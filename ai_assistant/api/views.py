...
def ask_financial_assistant(request):
    ...
    try:
        ...
        if not question:
            return Response(
                {"error": "Soru zorunludur."}, status=status.HTTP_400_BAD_REQUEST
            )
        nlp = LocalNLPService()
...
def voice_recognize(request):
    ...
    try:
        ...
        model_path = getattr(settings, "AI_STT_MODEL_PATH", None) or os.getenv(
            "VOSK_MODEL_PATH"
        )
        if not model_path:
            return Response(
                {
                    "error": "STT model yolu tanımlı değil (AI_STT_MODEL_PATH veya VOSK_MODEL_PATH)."
                },
                status=500,
            )
...
def qa_grounded(request):
    ...
    try:
        ...
        index_path = (
            getattr(settings, "AI_KB_INDEX_PATH", None)
            or os.getenv("AI_KB_INDEX_PATH")
            or os.path.join(settings.BASE_DIR, "media", "ai_kb", "index.json")
        )
        ...
        return Response(
            {
                "added": added,
                "index_path": out_path,
                "docs_loaded": [d["path"] for d in docs],
            }
        )
    except Exception as e:
        return Response({"error": str(e)}, status=500)
...
def voucher_from_voice(request):
    ...
    try:
        ...
        if not model_path:
            return Response(
                {
                    "error": "STT model yolu tanımlı değil (AI_STT_MODEL_PATH/VOSK_MODEL_PATH)."
                },
                status=500,
            )
        stt = LocalSTTService(model_path=model_path)
...
def voucher_confirm(request):
    ...
    try:
        ...
        return Response(
            {"status": "created", "voucher_id": getattr(voucher, "id", None)},
            status=201,
        )
    except Exception as e:
        return Response({"error": str(e)}, status=500)
