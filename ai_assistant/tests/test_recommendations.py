...
from ai_assistant.models import UserPreference, AIInsight
from ai_assistant.services import RecommendationService
...
class TestRecommendationService(TestCase):
    ...
    async def test_generate_recommendations(self):
        ...
        with patch("openai.OpenAI") as mock_openai_class:
            ...
            insights_count = await sync_to_async(
                lambda: AIInsight.objects.filter(user=self.user).count()
            )()
            assert insights_count == len(recommendations)
    ...
    def test_priority_calculation(self):
        ...
        for rec, expected_priority in test_cases:
            priority = self.service._calculate_priority(
                rec["title"], rec["recommendations"]
            )
    ...
    def test_action_requirement(self):
        ...
        test_cases = [
            ...
            (
                {"title": "Piyasa Analizi", "recommendations": ["Trend devam ediyor"]},
                False,
            ),
        ]
    ...
    async def test_recommendation_persistence(self):
        ...
        from ai_assistant.services.recommendation_service import GeneratedRecommendation
        ...
        insight = await sync_to_async(
            lambda: AIInsight.objects.filter(user=self.user).first()
        )()
