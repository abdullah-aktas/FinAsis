...
from ai_assistant.models import UserPreference, AIInsight, UserInteraction
...
class TestRecommendationService(TestCase):
    ...
    async def test_generate_recommendations(self):
        ...
        with patch('openai.OpenAI') as mock_openai_class:
            ...
            insights_count = await sync_to_async(lambda: AIInsight.objects.filter(user=self.user).count())()
    ...
    async def test_recommendation_persistence(self):
        ...
        from asgiref.sync import sync_to_async
        insight = await sync_to_async(lambda: AIInsight.objects.filter(user=self.user).first())()
