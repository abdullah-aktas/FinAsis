...
class AnomalyDetection(models.Model):
    ...
    def __str__(self):
        return f"{self.detection_type} - {self.created_at}"
...
class TrendAnalysis(models.Model):
    ...
    def __str__(self):
        return f"{self.analysis_type} ({self.start_date} - {self.end_date})"
...
class CashFlowForecaster:
    ...
    def forecast(self, periods=90):
        ...
        if self.model is None:
            raise ValueError("Model önce eğitilmelidir (train metodu ile).")
