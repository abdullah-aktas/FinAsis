...
class AIModel(models.Model):
    ...
    model_type = models.CharField(
        max_length=20, choices=MODEL_TYPES, verbose_name=_("Model Tipi")
    )
...
class UserInteraction(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, verbose_name=_("Kullanıcı")
    )
    interaction_type = models.CharField(
        max_length=20, choices=INTERACTION_TYPES, verbose_name=_("Etkileşim Tipi")
    )
    content = models.TextField(
        verbose_name=_("İçerik")
    )  # Kullanıcının gönderdiği içerik
    ai_response = models.TextField(verbose_name=_("AI Yanıtı"))  # AI'ın verdiği yanıt
    feedback = models.IntegerField(
        _("Geri Bildirim"), null=True, blank=True
    )  # 1-5 arası puanlama
    feedback_text = models.TextField(_("Geri Bildirim Metni"), blank=True)
    processing_time = models.FloatField(
        _("İşlem Süresi"), help_text=_("Saniye cinsinden")
    )
    created_at = models.DateTimeField(
        auto_now_add=True, verbose_name=_("Oluşturulma Tarihi")
    )
...
class FinancialPrediction(models.Model):
    ...
    prediction_type = models.CharField(
        max_length=50, choices=PREDICTION_TYPES, verbose_name=_("Tahmin Tipi")
    )
    input_data = models.JSONField(verbose_name=_("Girdi Verileri"))
    prediction_result = models.TextField(
        verbose_name=_("Tahmin Sonucu")
    )  # AI'ın ürettiği tahmin sonucu
...
class AIFeedback(models.Model):
    ...
    model = models.ForeignKey(
        AIModel, on_delete=models.CASCADE, verbose_name=_("Model")
    )
...
class AnomalyDetection(models.Model):
    ...
    detection_type = models.CharField(
        _("Tespit Tipi"), max_length=20, choices=DETECTION_TYPES
    )
...
class UserPreference(models.Model):
    ...
    risk_tolerance = models.CharField(
        _("Risk Toleransı"),
        max_length=20,
        choices=[("low", _("Düşük")), ("medium", _("Orta")), ("high", _("Yüksek"))],
    )
    investment_horizon = models.CharField(
        _("Yatırım Vadesi"),
        max_length=20,
        choices=[("short", _("Kısa")), ("medium", _("Orta")), ("long", _("Uzun"))],
    )
    ...
    preferred_model = models.ForeignKey(
        ...
        null=True,
        verbose_name=_("Tercih Edilen Model"),
    )
...
class AIInsight(models.Model):
    ...
    insight_type = models.CharField(
        max_length=50, choices=INSIGHT_TYPES, verbose_name=_("İçgörü Türü")
    )
...
class SectorBenchmark(models.Model):
    ...
    sector_key = models.CharField(
        max_length=50, unique=True, verbose_name=_("Sektör Anahtarı (slug)")
    )
    display_name = models.CharField(max_length=100, verbose_name=_("Görünen Ad"))
    margin_min = models.FloatField(
        null=True, blank=True, verbose_name=_("Hedef Net Kar Marjı Min (%)")
    )
    current_ratio_min = models.FloatField(
        null=True, blank=True, verbose_name=_("Hedef Cari Oran Min")
    )
    dte_max = models.FloatField(
        null=True, blank=True, verbose_name=_("Hedef Borç/Özsermaye Max")
    )
    opex_ratio_max = models.FloatField(
        null=True, blank=True, verbose_name=_("Hedef Faaliyet Gider Oranı Max")
    )
...
class CashFlowForecaster:
    ...
    def train(self, data):
        ...
        prophet_df = df[["date", "net_cash"]].rename(
            columns={"date": "ds", "net_cash": "y"}
        )
...
class CustomerRiskScorer:
    ...
    def _generate_explanation(self, customer_data, risk_score):
        ...
        if customer_data.get("payment_delay_count", 0) > 0:
            explanations.append(
                f"Son dönemde {customer_data['payment_delay_count']} kez ödeme gecikmesi yaşandı"
            )

        if customer_data.get("payment_delay_avg", 0) > 15:
            explanations.append(
                f"Ortalama ödeme gecikmesi {customer_data['payment_delay_avg']:.1f} gün"
            )
        ...
        if customer_data.get("days_since_last_payment", 0) > 30:
            explanations.append(
                f"Son ödemeden bu yana {customer_data['days_since_last_payment']} gün geçti"
            )
...
class SentimentAnalysis(models.Model):
    ...
    source_type = models.CharField(
        max_length=30, choices=SOURCE_TYPES, verbose_name=_("Kaynak Tipi")
    )
    text_content = models.TextField(verbose_name=_("Analiz Edilen Metin"))
    sentiment = models.CharField(
        max_length=20, choices=SENTIMENT_CHOICES, verbose_name=_("Duygu")
    )
    confidence_score = models.FloatField(
        verbose_name=_("Güven Skoru"), help_text=_("0.0 - 1.0 arası")
    )
    ...
    keywords = models.JSONField(
        default=list, blank=True, verbose_name=_("Anahtar Kelimeler")
    )
...
class DocumentSummary(models.Model):
    ...
    document_type = models.CharField(
        max_length=30, choices=DOCUMENT_TYPES, verbose_name=_("Doküman Tipi")
    )
    ...
    key_points = models.JSONField(
        default=list, blank=True, verbose_name=_("Ana Noktalar")
    )
    entities_mentioned = models.JSONField(
        default=list, blank=True, verbose_name=_("Bahsedilen Varlıklar")
    )
...
class AutoGeneratedReport(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="auto_reports"
    )
    report_type = models.CharField(
        max_length=30, choices=REPORT_TYPES, verbose_name=_("Rapor Tipi")
    )
    ...
    charts_data = models.JSONField(
        default=dict, blank=True, verbose_name=_("Grafik Verileri")
    )
