...
class SecurityMixin:
    ...
    def _validate_request(self, user, data):
        ...
        # expire yok, timeout için tekrar set et
        cache.set(cache_key, cache.get(cache_key), timeout=60)
...
class FinancialAIService(BaseAIService):
    ...
    def analyze_financial_data(self, user, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            prediction = FinancialPrediction.objects.create(
                user=user,
                prediction_type='market_trend',  # veya uygun bir değer
                input_data=data,
                prediction_result=analysis_result,
                confidence=1.0  # dummy değer, gerçek analizde değiştirin
            )
    ...
    def get_personalized_recommendations(self, user, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            recommendations = response.choices[0].message.content if response.choices and response.choices[0].message.content else ""
    ...
    def predict_market_trends(self, user, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            trends_val = trends if trends else ""
            if not trends_val:
                return {"trends": {}, "timestamp": data.get('timestamp')}
            return {
                "trends": self._parse_trends(trends_val),
...
class ChatAIService(BaseAIService):
    ...
    async def get_response(self, user, query: str) -> str:
        ...
        try:
            ...
            messages = [{"role": "system", "content": "Sen bir finansal danışman asistanısın."}]
            ...
            ai_response = response.choices[0].message.content if response.choices and response.choices[0].message.content else ""
...
class RecommendationService(BaseAIService):
    ...
    async def generate_recommendations(self, user, context: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Kullanıcının geçmiş etkileşimlerini al
            interactions = await sync_to_async(lambda: list(UserInteraction.objects.filter(
                user=user,
            ...
            rec_content = response.choices[0].message.content if response.choices and response.choices[0].message.content else ""
            if not rec_content:
                return []
            recommendations = self._parse_enhanced_recommendations(rec_content)
    ...
    def _create_enhanced_recommendation_prompt(self, **kwargs) -> str:
        ...
        if user_preferences is None:
            risk_tolerance = "orta"
            investment_horizon = "orta"
            language = "tr"
        else:
            risk_tolerance = getattr(user_preferences, 'risk_tolerance', 'orta')
            investment_horizon = getattr(user_preferences, 'investment_horizon', 'orta')
            language = getattr(user_preferences, 'language', 'tr')
        return f"""
        KULLANICI PROFİLİ:
        - Risk Toleransı: {risk_tolerance}
        - Yatırım Vadesi: {investment_horizon}
