...
class OCRService:
    def __init__(self, use_google_vision: bool = False):
            ...
            if pytesseract is not None:
                ...
                if cmd:
                    try:
                        ...
                        logger.debug(
                            "Tesseract yolu ayarlanamadı, PATH üzerinden deneniyor: %s",
                            cmd,
                        )
                else:
                    # Hiçbiri bulunamazsa PATH üzerinden çalışmayı dener
                    logger.debug(
                        "Tesseract varsayılan yolu bulunamadı; PATH kullanılacak."
                    )
        ...
        if use_google_vision:
            if vision is None:
                raise RuntimeError(
                    "'google-cloud-vision' paketi kurulu değil. 'pip install google-cloud-vision' ile yükleyin veya use_google_vision=False kullanın."
                )
    ...
    def _extract_with_tesseract(self, image_path: str) -> Dict[str, str]:
        ...
        try:
            if pytesseract is None or cv2 is None:
                raise RuntimeError(
                    "Tesseract için gerekli bağımlılıklar eksik: 'pytesseract' ve/veya 'opencv-python'."
                )
            # Görüntüyü oku
    ...
    def _parse_invoice_text(self, text: str) -> Dict[str, str]:
        ...
        tax_patterns = [r"KDV[:\s]*%?([\d.,]+)", r"KDV Oranı[:\s]*%?([\d.,]+)"]

        # Firma adı için regex
        company_patterns = [r"Firma[:\s]*([^\n]+)", r"Ünvan[:\s]*([^\n]+)"]
