...
class DocumentSummarizationService:
    ...
    def summarize_text(
        ...
        summary_sentences = [sent for sent, score in sorted(top_sentences, key=lambda x: sentences.index(x[0]))]
        ...
        return {
            ...
            'compression_ratio': round(len(summary_words) / len(original_words), 2) if original_words else 0,
    ...
    def _calculate_word_frequencies(cls, text: str, language: str) -> Dict[str, int]:
        ...
        stop_words_tr = {
            ...
            'mi', 'mı', 'mu', 'mü', 'gibi', 'kadar', 'daha', 'çok', 'az',
            'var', 'yok', 'ama', 'fakat', 'ancak', 'ne', 'neden', 'nasıl'
        }
        ...
        filtered_words = [
            ...
            if len(word) > 2 and word not in stop_words_tr
        ]
        
        return dict(Counter(filtered_words))
    
    @classmethod
    def _score_sentences(cls, sentences: List[str], word_frequencies: Dict[str, int]) -> Dict[str, float]:
        """Her cümleyi skorlar"""
        scores = {}
