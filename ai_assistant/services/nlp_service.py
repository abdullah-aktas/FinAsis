...
class LocalNLPService:
    ...
    def respond(self, user: AbstractBaseUser | Any, query: str) -> Dict[str, Any]:
        ...
        if intent == 'cash_flow':
            ...
            def _find_net_cash_value(rows):
                candidates = {
                    'net nakit akis', 'net nakit akisi', 'net nakit akışı',
                    'net cash flow', 'net cash', 'net nakit', 'net cashflow'
                }
                for r in rows or []:
                    ...
                    for nk, orig in norm_map.items():
                        if 'net' in nk and ('nakit' in nk or 'cash' in nk) and ('ak' in nk or 'flow' in nk):
    ...
    def _parse_number(self, val: Any) -> float | None:
        ...
        try:
            ...
            import re as _re
            s_clean = _re.sub(r"[\s\u20BA₺TLtl]", "", s)  # remove spaces and TL symbols/letters
