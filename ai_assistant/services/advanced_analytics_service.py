...
class AdvancedAnalyticsService:
    ...
    def revenue_forecast(cls, user, forecast_days: int = 30) -> AdvancedAnalytics:
        ...
        try:
            ...
            for inv in invoices:
                ...
                daily_revenue[date_key] = daily_revenue.get(date_key, 0) + float(
                    inv.total_amount
                )
    ...
    def customer_segmentation(cls, user) -> AdvancedAnalytics:
        ...
        try:
            ...
            customers = Customer.objects.filter(company__users=user).annotate(
                total_spent=Sum("invoice__total_amount"), invoice_count=Count("invoice")
            )
            ...
            for customer in customers:
                ...
                recency = (
                    (timezone.now().date() - last_invoice.issue_date).days
                    if last_invoice
                    else 999
                )
            ...
            if customer_data:
                sorted_by_monetary = sorted(
                    customer_data, key=lambda x: x["monetary"], reverse=True
                )
    ...
    def abc_analysis(cls, user) -> AdvancedAnalytics:
        ...
        try:
            ...
            customers = (
                Customer.objects.filter(company__users=user)
                .annotate(total_revenue=Sum("invoice__total_amount"))
