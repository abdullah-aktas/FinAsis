...
from .services.ml_service import RiskScoringService, FinancialForecastService, RecommendationService
import numpy as np
import pandas as pd
...
def risk_score_api(request):
    ...
    try:
        ...
        if not features or not isinstance(features, list):
            return Response({'error': _('Geçerli bir feature listesi giriniz.')}, status=400)
        ...
        return Response({'error': _('Risk skoru hesaplanırken hata oluştu: ') + str(e)}, status=500)
...
def financial_forecast_api(request):
    ...
    try:
        ...
        if 'ds' not in df or 'y' not in df:
            return Response({'error': _('Veri formatı yanlış. ds ve y sütunları olmalı.')}, status=400)
        ...
        return Response({'error': _('Tahmin yapılırken hata oluştu: ') + str(e)}, status=500)
