...
from .models import Customer, Contact, Opportunity, Activity, Document, Sale, SaleItem, EDocumentStatus, CustomerAcquisitionAnalytics, Campaign, CampaignUsage, ReferralProgram, PremiumPackage, ConsultingService, TrainingProgram, APIPricing, ServiceSubscription, Lead, CustomerNote, CustomerDocument
...
class LeadViewSet(viewsets.ModelViewSet):
    ...
    def convert(self, request, pk=None):
        ...
        # Gerçek uygulamada burada daha fazla iş yapılır
        ...
        return Response({'status': 'success', 'customer_id': customer.id})
...
class CustomerViewSet(viewsets.ModelViewSet):
    ...
    def documents(self, request, pk=None):
        ...
        return Response({
            ...
            'data': [{'id': d.id, 'title': d.title, 'url': d.file.url} for d in documents]
        })

class OpportunityViewSet(viewsets.ModelViewSet):
    """API viewset for opportunities"""
...
def dashboard(request):
    ...
    upcoming_activities = Activity.objects.filter(completed=False).order_by('due_date')[:5]
    
    # Satış istatistikleri
    sales_by_status = Opportunity.objects.values('status').annotate(
        count=Count('id'),
        value=Sum('value')
    )
...
def lead_create(request):
    ...
    if request.method == 'POST':
        ...
        messages.success(request, 'Aday başarıyla oluşturuldu.')
...
def lead_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        lead.position = request.POST.get('position')
        ...
        messages.success(request, 'Aday başarıyla güncellendi.')
        return redirect('crm:lead_detail', pk=lead.pk)
    
    return render(request, 'crm/lead_form.html', {'lead': lead})
...
def lead_delete(request, pk):
    ...
    if request.method == 'POST':
        lead.delete()
        messages.success(request, 'Aday başarıyla silindi.')
...
def lead_convert(request, pk):
    """Adayı müşteriye dönüştürme view"""
    lead = get_object_or_404(Lead, pk=pk)
    ...
    if request.method == 'POST':
        ...
        # İsteğe bağlı olarak lead'i silebilir veya statüsünü değiştirebilirsiniz
        lead.status = 'won'
        lead.save()
        
        messages.success(request, 'Aday başarıyla müşteriye dönüştürüldü.')
        return redirect('crm:customer_detail', pk=customer.pk)
    
    return render(request, 'crm/lead_convert.html', {'lead': lead})

# Benzer şekilde diğer view'lar da buraya eklenecek (Fırsatlar, Aktiviteler, vs.)
@login_required
def opportunity_list(request):
    """Fırsat listesi view"""
    opportunities = Opportunity.objects.all()
...
@login_required
def opportunity_create(request):
    """Fırsat oluşturma view"""
    # İlgili kodlar burada
    pass
...
def activity_edit(request, pk):
    """Aktivite güncelleme view"""
    # İlgili kodlar burada
    pass

@login_required
def activity_complete(request, pk):
    """Aktiviteyi tamamlama view"""
    activity = get_object_or_404(Activity, pk=pk)
    ...
    if request.method == 'POST':
        ...
        messages.success(request, 'Aktivite başarıyla tamamlandı.')
        return redirect('crm:activity_list')
    
    return render(request, 'crm/activity_complete.html', {'activity': activity})
