...
class SecurityPolicy(models.Model):
    ...
    policy_type = models.CharField(max_length=20, choices=POLICY_TYPES, verbose_name=_('Politika Tipi'))
...
class SecurityIncident(models.Model):
    ...
    incident_type = models.CharField(max_length=30, choices=INCIDENT_TYPES, verbose_name=_('Olay Tipi'))
    severity = models.CharField(max_length=20, choices=SEVERITY_LEVELS, verbose_name=_('Ciddiyet'))
    ...
    # Hedef
    affected_user = models.ForeignKey(settings.AUTH_USER_MODEL, null=True, blank=True, on_delete=models.SET_NULL, related_name='security_incidents', verbose_name=_('Etkilenen Kullanıcı'))
    affected_resource = models.CharField(max_length=200, blank=True, verbose_name=_('Etkilenen Kaynak'))
    ...
    resolved_by = models.ForeignKey(settings.AUTH_USER_MODEL, null=True, blank=True, on_delete=models.SET_NULL, related_name='resolved_incidents', verbose_name=_('Çözen'))
...
class IPWhitelist(models.Model):
    ...
    # Hedef
    user = models.ForeignKey(settings.AUTH_USER_MODEL, null=True, blank=True, on_delete=models.CASCADE, verbose_name=_('Kullanıcı'))
    ...
    # Geçerlilik
    valid_until = models.DateTimeField(null=True, blank=True, verbose_name=_('Geçerlilik Süresi'))
    
    created_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='created_whitelists', verbose_name=_('Oluşturan'))
    created_at = models.DateTimeField(auto_now_add=True)
...
class IPBlacklist(models.Model):
    ...
    ip_address = models.GenericIPAddressField(unique=True, verbose_name=_('IP Adresi'))
    reason = models.CharField(max_length=30, choices=BLOCK_REASONS, verbose_name=_('Engelleme Sebebi'))
    description = models.TextField(blank=True, verbose_name=_('Açıklama'))
    
    # İstatistikler
    blocked_attempts = models.IntegerField(default=0, verbose_name=_('Engellenen Deneme'))
    first_blocked_at = models.DateTimeField(verbose_name=_('İlk Engelleme'))
    last_attempt_at = models.DateTimeField(null=True, blank=True, verbose_name=_('Son Deneme'))
    
    # Durum
    is_active = models.BooleanField(default=True, verbose_name=_('Aktif'))
    auto_unblock_at = models.DateTimeField(null=True, blank=True, verbose_name=_('Otomatik Kaldırma Zamanı'))
    
    created_by = models.ForeignKey(settings.AUTH_USER_MODEL, null=True, blank=True, on_delete=models.SET_NULL, verbose_name=_('Oluşturan'))
    created_at = models.DateTimeField(auto_now_add=True)
