...
def rate_limit_login(fn):
    ...
    """
    return ratelimit(key="ip", rate="5/m", method="POST", block=True)(fn)
...
def rate_limit_api_user(fn):
    ...
    100 istek / dakika
    """
    return ratelimit(key="user", rate="100/m", method=ratelimit.ALL, block=True)(fn)
...
def rate_limit_api_anon(fn):
    ...
    20 istek / dakika
    """
    return ratelimit(key="ip", rate="20/m", method=ratelimit.ALL, block=True)(fn)
...
def rate_limit_registration(fn):
    ...
    3 kayıt / saat / IP
    """
    return ratelimit(key="ip", rate="3/h", method="POST", block=True)(fn)
...
def rate_limit_file_upload(fn):
    ...
    10 dosya / dakika
    """
    return ratelimit(key="user_or_ip", rate="10/m", method="POST", block=True)(fn)
...
def rate_limit_search(fn):
    ...
    30 arama / dakika
    """
    return ratelimit(key="user_or_ip", rate="30/m", method="GET", block=True)(fn)
...
def rate_limit_payment(fn):
    ...
    """
    return ratelimit(key="user", rate="5/m", method="POST", block=True)(fn)
...
def rate_limit_ai_assistant(fn):
    ...
    """
    return ratelimit(key="user", rate="20/m", method="POST", block=True)(fn)
...
def rate_limit_report_generation(fn):
    ...
    """
    return ratelimit(key="user", rate="10/m", method="POST", block=True)(fn)
        ...
        def wrapper(request, *args, **kwargs):
            if request.user.is_authenticated:
                ...
                limited_fn = ratelimit(
                    key="user", rate=user_rate, method=ratelimit.ALL, block=True
                )(fn)
            else:
                # Anonymous kullanıcı
                limited_fn = ratelimit(
                    key="ip", rate=anon_rate, method=ratelimit.ALL, block=True
                )(fn)
