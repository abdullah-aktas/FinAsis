...
def fix_file_encoding(file_path):
    ...
    try:
        ...
        if detected_encoding and detected_encoding != 'utf-8':
            ...
            logger.info(f"Düzeltildi: {file_path} ({detected_encoding} -> utf-8)")
            return True
        elif detected_encoding:
            logger.debug(f"Zaten UTF-8: {file_path}")
        else:
            logger.warning(f"Kodlama tespit edilemedi: {file_path}")
...
def process_directory(directory):
    ...
    for root, dirs, files in os.walk(directory):
        ...
        for file in files:
            # Python, HTML, JavaScript, CSS, PO ve diğer metin dosyalarını kontrol et
            if file.endswith(('.py', '.html', '.js', '.css', '.json', '.xml', '.txt', '.md', '.po')):
...
if __name__ == '__main__':
    ...
    fixed_files = process_directory(current_dir)
