    ...
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            if not has_permission:
                default_message = f"Bu işlem için '{permission_attr}' iznine sahip olmanız gerekir."
...
def requires_plan_feature(feature_attr, error_message=None):
    ...
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            if not has_feature:
                default_message = f"Bu özellik sizin planınızda bulunmuyor. Planınızı yükseltmek için tıklayın."
                message = error_message or default_message
...
def can_access_company(user, company):
    ...
    if (user_profile.role.name == 'financial_advisor' and 
        hasattr(company, 'financial_advisor') and 
        company.financial_advisor == user):
        return True
