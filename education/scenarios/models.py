...
class Scenario(models.Model):
    ...
    difficulty = models.CharField(
        _("Zorluk Seviyesi"), max_length=20, choices=DIFFICULTY_LEVELS
    )
    category = models.CharField(_("Kategori"), max_length=20, choices=CATEGORIES)
...
class ScenarioStep(models.Model):
    ...
    hints = models.TextField(
        _("İpuçları"), blank=True, help_text=_("Öğrenciye yardımcı olacak ipuçları")
    )
...
class CaseStudy(models.Model):
    ...
    company_profile = models.TextField(
        _("Şirket Profili"), help_text=_("Vaka çalışmasındaki şirketin detaylı profili")
    )
    ...
    questions = models.TextField(
        _("Sorular"), help_text=_("Öğrencilerin cevaplaması gereken sorular")
    )
    solution_guide = models.TextField(
        _("Çözüm Rehberi"), help_text=_("Vaka çalışmasının çözümü için rehber")
    )
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
