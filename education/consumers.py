...
class MeetingConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def receive_json(self, content: Dict[str, Any], **kwargs: Any) -> None:  # noqa: ANN401
        ...
        if msg_type not in {"join", "leave", "offer", "answer", "ice", "chat", "moderate", "whiteboard", "hand", "cobrowse"}:
            return
        user = self.scope.get("user")
        ...
        payload: Dict[str, Any] = {
            ...
            "from": user_id,
            "to": content.get("to"),
            "data": data if isinstance(data, (dict, list, str, int, float, type(None))) else None,
        }
    ...
    def _user_can_join(self, meeting_id: int) -> bool:
        from django.contrib.auth import get_user_model  # lazy import for app registry safety
        user = self.scope.get("user")
        try:
            meeting = Meeting.objects.select_related("organizer").prefetch_related("participants").get(pk=meeting_id)
    ...
    def _is_presenter_or_organizer(self) -> bool:
        ...
        try:
            meeting_id = int(self.room_name)
            meeting = Meeting.objects.only('id', 'organizer_id', 'presenter_id').get(pk=meeting_id)
        except Exception:
            return False
    ...
    def _mark_presence_join(self) -> None:
        ...
        if not user or not getattr(user, 'pk', None):
            return
        MeetingPresence.objects.create(meeting_id=meeting_id, user_id=user.pk, client_id=self.channel_name)
    ...
    def _mark_presence_leave(self) -> None:
        ...
        if not user or not getattr(user, 'pk', None):
            return
        qs = MeetingPresence.objects.filter(meeting_id=meeting_id, user_id=user.pk, client_id=self.channel_name, left_at__isnull=True).order_by('-joined_at')
        obj = qs.first()
        if obj:
