...
def student_dashboard(request):
    ...
    recent_grades = (
        StudentAssignment.objects.filter(student=request.user, status="graded")
        .select_related("assignment")
...
class AssignmentListView(ListView):
    ...
    def get_queryset(self):
        return (
            StudentAssignment.objects.filter(student=self.request.user)
            .select_related("assignment")
...
class CourseProgressView(ListView):
    ...
    def get_queryset(self):
        return (
            StudentProgress.objects.filter(student=self.request.user)
            .select_related("course")
...
def submit_assignment(request, pk):
    ...
    student_assignment = get_object_or_404(
        StudentAssignment, pk=pk, student=request.user
    )
...
def profile_update(request):
    ...
    return render(
        request, "student/profile_update.html", {"student_profile": student_profile}
    )
