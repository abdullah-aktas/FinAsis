...
class StudentProfile(models.Model):
    ...
    student_number = models.CharField(_("Öğrenci Numarası"), max_length=20, unique=True)
    department = models.CharField(
        _("Bölüm"), max_length=100, help_text=_("Öğrencinin okuduğu bölüm")
    )
    grade = models.CharField(
        _("Sınıf"), max_length=10, help_text=_("Öğrencinin sınıfı")
    )
    enrollment_date = models.DateField(_("Kayıt Tarihi"), auto_now_add=True)
    graduation_date = models.DateField(_("Mezuniyet Tarihi"), null=True, blank=True)
    gpa = models.DecimalField(
        _("Genel Not Ortalaması"), max_digits=3, decimal_places=2, default=0.00
    )
    completed_credits = models.IntegerField(_("Tamamlanan Krediler"), default=0)
    total_credits = models.IntegerField(_("Toplam Krediler"), default=240)
...
class StudentAssignment(models.Model):
    ...
    submission_date = models.DateTimeField(_("Teslim Tarihi"), null=True, blank=True)
    last_activity = models.DateTimeField(_("Son Aktivite"), auto_now=True)
    completion_percentage = models.DecimalField(
        _("Tamamlanma Yüzdesi"), max_digits=5, decimal_places=2, default=0
    )
    grade = models.DecimalField(
        _("Not"), max_digits=5, decimal_places=2, null=True, blank=True
    )
...
class StudentProgress(models.Model):
    ...
    attendance = models.DecimalField(
        _("Devam Durumu"), max_digits=5, decimal_places=2, default=100
    )
    midterm_grade = models.DecimalField(
        _("Vize Notu"), max_digits=5, decimal_places=2, null=True, blank=True
    )
    final_grade = models.DecimalField(
        _("Final Notu"), max_digits=5, decimal_places=2, null=True, blank=True
    )
    average_grade = models.DecimalField(
        _("Ortalama Not"), max_digits=5, decimal_places=2, null=True, blank=True
    )
    letter_grade = models.CharField(_("Harf Notu"), max_length=2, null=True, blank=True)
    status = models.CharField(
        _("Durum"),
        max_length=20,
        choices=(
