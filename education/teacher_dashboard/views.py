...
def teacher_dashboard(request):
    ...
    context = {
        ...
        "recent_assignments": Assignment.objects.filter(teacher=request.user).order_by(
            "-created_at"
        )[:5],
...
class AssignmentGradeView(UpdateView):
    ...
    def get_object(self):
        assignment = get_object_or_404(
            Assignment, pk=self.kwargs["pk"], teacher=self.request.user
        )
        return get_object_or_404(
            StudentProgress, assignment=assignment, student=self.request.user
        )
...
class StudentProgressView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["progress"] = StudentProgress.objects.filter(
            student=self.object, assignment__teacher=self.request.user
