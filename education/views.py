...
COURSE_MARKETING_PAGES: Dict[str, CourseMarketingPage] = {
    "finance-literacy": CourseMarketingPage(
        ...
        hero_subtitle=_(
            "Öğrenciler, girişimciler ve genç profesyoneller için 6 haftada kapsamlı finans okuryazarlığı."
        ),
        hero_highlights=[
        ...
        modules=[
            {
                ...
                "description": _(
                    "Gelir-gider analizi, hedef bazlı bütçeleme, acil durum fonu inşası."
                ),
            },
            {
                "icon": "bi-graph-up",
                ...
                "description": _(
                    "Hisse senedi, fon, tahvil, kripto; risk-getiri dengesi ve portföy oluşturma."
                ),
            },
            {
                "icon": "bi-piggy-bank",
                ...
                "description": _(
                    "Kredi skor yönetimi, borç kapama stratejileri, psikolojik finans."
                ),
            },
            {
                "icon": "bi-lightbulb",
                ...
                "description": _(
                    "İş planı finansalları, nakit akışı projeksiyonu, yatırımcı sunumu hazırlama."
                ),
            },
        ],
        outcomes=[
        ...
        instructor={
            ...
            "bio": _(
                "12 yıllık finans direktörlüğü deneyimi, finansal okuryazarlık programları ve girişim hızlandırıcılarında mentor."
            ),
        },
        syllabus=[
            {
                "week": _("1. Hafta"),
                "topic": _("Finansal hedefler ve bütçe incelemesi"),
                "format": _("Canlı ders + FinQuest görevleri"),
            },
            {
                "week": _("2. Hafta"),
                "topic": _("Gelir-gider analizi, bütçe şablonları"),
                "format": _("Vaka analizi + quiz"),
            },
            {
                "week": _("3. Hafta"),
                "topic": _("Yatırım araçları ve risk yönetimi"),
                "format": _("Portföy simülasyonu"),
            },
            {
                "week": _("4. Hafta"),
                "topic": _("Borç ve kredi planlaması"),
                "format": _("Senaryo çalışması + AI raporu"),
            },
            {
                "week": _("5. Hafta"),
                "topic": _("Finansal tabloları okuma"),
                "format": _("Bilanço ve nakit akışı analizi"),
            },
            {
                "week": _("6. Hafta"),
                "topic": _("Girişim finansalları ve yatırımcı sunumu"),
                "format": _("Pitch hazırlığı + bire bir mentorluk"),
            },
        ],
        faqs=[
            {
                ...
                "answer": _(
                    "Tüm canlı oturumlar kayıt altına alınır ve LMS üzerinden 48 saat içinde yayınlanır."
                ),
            },
            {
                "question": _("Sertifika veriliyor mu?"),
                "answer": _(
                    "Tüm görevleri tamamlayan ve final simülasyonunu bitiren katılımcılar FinAsis sertifikası alır."
                ),
            },
            {
                "question": _("Kurumsal paket var mı?"),
                "answer": _(
                    "Kurumsal ekipler için özelleştirilmiş içerik ve mentorluk sağlıyoruz. İletişim sayfasından bize ulaşabilirsiniz."
                ),
            },
        ],
    ),
...
def quick_prep(request):
    ...
    total_questions = (
        Question.objects.filter(exams__course__in=user_courses).distinct().count()
    ...
    def perform_create(self, serializer):
        ...
        if course and course.teacher_id != self.request.user.pk:
            raise PermissionDenied(
                "Only course teacher can create tournaments for a course."
            )
        serializer.save()
...
class CheatingIncidentViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if submission.exam.course.teacher_id != self.request.user.pk:
            raise PermissionDenied(
                "Only course teacher can log incidents for submissions in their course."
            )
        serializer.save()
...
class MeetingDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):  # type: ignore[override]
        ...
        try:
            ctx["ice_servers_json"] = _json.dumps(
                ice_servers
            )  # string for safe embedding
        except Exception:
        ...
        ctx["is_presenter"] = bool(
            user
            and (
                user == getattr(obj, "organizer", None)
                or getattr(user, "pk", None) == presenter_id
            )
        )
        # recordings list
        try:
            rec_qs = (
                MeetingRecording.objects.select_related("owner")
                .filter(meeting=obj)
                .order_by("-created_at")
            )
            ctx["recordings"] = list(
                rec_qs[:10]
            )  # show recent 10; rest is in paginated view
        except Exception:
            ctx["recordings"] = []
        ...
        try:
            ...
            def name(u):
                return (
                    ...
                    or getattr(u, "get_username", lambda: "")()
                )
        ...
        if (
            mode == "sfu"
            and jitsi_domain
            and getattr(settings, "JITSI_JWT_ENABLED", False)
        ):
            ...
            if secret:
                try:
                    ...
                    if app_id:
                        headers["kid"] = app_id
                    token = jwt.encode(
                        payload, secret, algorithm="HS256", headers=headers
                    )
                    ctx["jitsi_jwt"] = token
                except Exception:
                    ctx["jitsi_jwt"] = ""
...
def meeting_set_presenter(request, pk: int):
    ...
    if request.method == "POST":
        ...
        if uid:
            ...
            if (
                uid == getattr(meeting.organizer, "pk", None)
                or meeting.participants.filter(pk=uid).exists()
            ):
                    ...
                    if channel_layer is not None:
                        async_to_sync(channel_layer.group_send)(
                            f"meeting_{meeting.pk}",
                            {
                                ...
                                "payload": {
                                    "type": "moderate",
                                    "from": getattr(request.user, "pk", None),
                                    "data": {"action": "set-presenter", "user_id": uid},
                                },
                                "sender": "server",
                            },
                        )
...
def meeting_upload_recording(request, pk: int):
    ...
    if not (
        user == meeting.organizer or meeting.participants.filter(pk=user.pk).exists()
    ):
        return HttpResponse(status=403)
    if request.method != "POST":
...
def meeting_recordings(request, pk: int):
    ...
    return render(
        request,
        "education/meetings_recordings.html",
        {
            "meeting": meeting,
            "page_obj": page_obj,
            "paginator": paginator,
...
def meeting_presence(request, pk: int):
    ...
    now = dj_tz.now()
    qs = (
        MeetingPresence.objects.select_related("user")
        .filter(meeting=meeting)
    ...
    for p in qs:
        ...
        if left is None:
            ongoing_sessions += 1
        duration_seconds = (
            int((effective_left - joined).total_seconds())
            if (joined and effective_left)
            else 0
        )
        total_seconds_all += max(duration_seconds, 0)
        # Per-user totals
        uname = (
            getattr(p.user, "get_full_name", lambda: "")()
            or getattr(p.user, "get_username", lambda: "")()
        )
        uid = getattr(p.user, "pk", None)
        per_user.setdefault(
            uid, {"user_display": uname, "sessions": 0, "total_seconds": 0}
        )
    ...
    return render(
        request,
        "education/meetings_presence.html",
        {
            "meeting": meeting,
...
def meeting_presence_csv(request, pk: int):
    ...
    writer.writerow(
        ["user_id", "user", "joined_at", "left_at", "duration_seconds", "duration_hms"]
    )
    for p in presences:
        joined = p.joined_at
        left = p.left_at
...
def meeting_presence_totals_csv(request, pk: int):
    ...
    for p in qs:
        ...
        duration_seconds = (
            int((left - joined).total_seconds()) if (joined and left) else 0
        )
...
def meeting_invite(request, pk: int):
    ...
    return render(
        request,
        "education/meeting_invite_form.html",
