...
class InteractiveExercise(models.Model):
    ...
    exercise_type = models.CharField(
        _("Alıştırma Tipi"), max_length=20, choices=EXERCISE_TYPES
    )
    content = models.JSONField(
        _("İçerik"), help_text=_("Alıştırmanın içeriği (JSON formatında)")
    )
    correct_answers = models.JSONField(
        _("Doğru Cevaplar"), help_text=_("Doğru cevaplar (JSON formatında)")
    )
...
class ExerciseAttempt(models.Model):
    ...
    answers = models.JSONField(
        _("Cevaplar"), help_text=_("Öğrencinin verdiği cevaplar (JSON formatında)")
    )
    score = models.DecimalField(
        _("Puan"), max_digits=5, decimal_places=2, null=True, blank=True
    )
    feedback = models.TextField(
        _("Geri Bildirim"), blank=True, help_text=_("Öğrenciye verilecek geri bildirim")
    )
...
class ExerciseGroup(models.Model):
    ...
    exercises = models.ManyToManyField(
        InteractiveExercise, related_name="groups", verbose_name=_("Alıştırmalar")
    )
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
