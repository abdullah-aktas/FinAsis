...
class MeetingRecording(models.Model):
    ...
    meeting = models.ForeignKey(Meeting, on_delete=models.CASCADE, related_name='recordings', verbose_name=_('Toplantı'))
    owner = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name='meeting_recordings', verbose_name=_('Yükleyen'))
    file = models.FileField(upload_to='meeting_recordings/', verbose_name=_('Kayıt Dosyası'))
    kind = models.CharField(max_length=20, blank=True, verbose_name=_('Tür'))  # screen|camera
    duration_ms = models.BigIntegerField(null=True, blank=True, verbose_name=_('Süre (ms)'))
    created_at = models.DateTimeField(auto_now_add=True, verbose_name=_('Yükleme Zamanı'))
    title = models.CharField(max_length=200, blank=True, default="", verbose_name=_('Başlık'))
...
class CourseCategory(models.Model):
    ...
    parent = models.ForeignKey('self', null=True, blank=True, on_delete=models.CASCADE, related_name='subcategories', verbose_name=_('Üst Kategori'))
...
class EnhancedCourse(models.Model):
    ...
    course = models.OneToOneField(Course, on_delete=models.CASCADE, related_name='enhanced_info', verbose_name=_('Ders'))
    category = models.ForeignKey(CourseCategory, null=True, blank=True, on_delete=models.SET_NULL, related_name='courses', verbose_name=_('Kategori'))
    
    # Görsel ve medya
    thumbnail = models.ImageField(upload_to='course_thumbnails/', null=True, blank=True, verbose_name=_('Kapak Görseli'))
    ...
    difficulty_level = models.CharField(max_length=20, choices=[
        ...
        ('advanced', _('İleri')),
        ('expert', _('Uzman'))
    ], default='beginner', verbose_name=_('Zorluk Seviyesi'))
    
    estimated_hours = models.IntegerField(default=0, verbose_name=_('Tahmini Süre (saat)'))
    prerequisites = models.ManyToManyField(Course, blank=True, related_name='required_for', verbose_name=_('Ön Koşul Dersler'))
    
    # İstatistikler
    total_enrolled = models.IntegerField(default=0, verbose_name=_('Toplam Kayıt'))
    average_rating = models.DecimalField(max_digits=3, decimal_places=2, default=Decimal('0.00'), verbose_name=_('Ortalama Puan'))
    total_reviews = models.IntegerField(default=0, verbose_name=_('Toplam Değerlendirme'))
    
    # Yayın durumu
    is_published = models.BooleanField(default=True, verbose_name=_('Yayında'))
    published_at = models.DateTimeField(null=True, blank=True, verbose_name=_('Yayın Tarihi'))
...
class LearningPath(models.Model):
    ...
    description = models.TextField(verbose_name=_('Açıklama'))
    thumbnail = models.ImageField(upload_to='learning_paths/', null=True, blank=True, verbose_name=_('Görsel'))
    courses = models.ManyToManyField(Course, through='PathCourse', related_name='learning_paths', verbose_name=_('Dersler'))
    created_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='created_paths', verbose_name=_('Oluşturan'))
    students = models.ManyToManyField(settings.AUTH_USER_MODEL, related_name='enrolled_paths', blank=True, verbose_name=_('Kayıtlı Öğrenciler'))
    ...
    estimated_weeks = models.IntegerField(default=0, verbose_name=_('Tahmini Süre (hafta)'))
...
class PathCourse(models.Model):
    ...
    course = models.ForeignKey(Course, on_delete=models.CASCADE, related_name='path_memberships')
    order = models.IntegerField(default=0, verbose_name=_('Sıra'))
...
class Certificate(models.Model):
    ...
    student = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='certificates', verbose_name=_('Öğrenci'))
    course = models.ForeignKey(Course, null=True, blank=True, on_delete=models.CASCADE, related_name='certificates', verbose_name=_('Ders'))
    learning_path = models.ForeignKey(LearningPath, null=True, blank=True, on_delete=models.CASCADE, related_name='certificates', verbose_name=_('Öğrenme Yolu'))
    
    certificate_type = models.CharField(max_length=20, choices=CERTIFICATE_TYPES, default='completion')
    certificate_id = models.CharField(max_length=50, unique=True, verbose_name=_('Sertifika No'))
    
    # Puanlama
    final_score = models.DecimalField(max_digits=5, decimal_places=2, verbose_name=_('Final Puanı'))
    grade = models.CharField(max_length=5, blank=True, verbose_name=_('Harf Notu'))
    
    # Dosya
    file = models.FileField(upload_to='certificates/', null=True, blank=True, verbose_name=_('PDF Dosyası'))
    
    # Metadata
    issued_at = models.DateTimeField(auto_now_add=True, verbose_name=_('Veriliş Tarihi'))
    issued_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, related_name='issued_certificates', verbose_name=_('Veren'))
    
    # Doğrulama
    verification_code = models.CharField(max_length=100, blank=True, verbose_name=_('Doğrulama Kodu'))
...
class Announcement(models.Model):
    ...
    course = models.ForeignKey(Course, on_delete=models.CASCADE, related_name='announcements', verbose_name=_('Ders'))
    title = models.CharField(max_length=200, verbose_name=_('Başlık'))
    content = models.TextField(verbose_name=_('İçerik'))
    author = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='announcements', verbose_name=_('Yazar'))
    ...
    priority = models.CharField(max_length=20, choices=[
        ...
        ('high', _('Yüksek')),
        ('urgent', _('Acil'))
    ], default='normal', verbose_name=_('Öncelik'))
    ...
    # Bildirim
    send_notification = models.BooleanField(default=True, verbose_name=_('Bildirim Gönder'))
    notification_sent = models.BooleanField(default=False, verbose_name=_('Bildirim Gönderildi'))
...
class LessonResource(models.Model):
    ...
    lesson = models.ForeignKey(Lesson, on_delete=models.CASCADE, related_name='resources', verbose_name=_('Ders'))
    title = models.CharField(max_length=200, verbose_name=_('Başlık'))
    resource_type = models.CharField(max_length=20, choices=RESOURCE_TYPES, default='pdf')
    
    # Dosya veya bağlantı
    file = models.FileField(upload_to='lesson_resources/', null=True, blank=True, verbose_name=_('Dosya'))
    ...
    requires_completion = models.BooleanField(default=False, verbose_name=_('Tamamlama Gerektirir'))
    
    uploaded_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, related_name='uploaded_resources', verbose_name=_('Yükleyen'))
    created_at = models.DateTimeField(auto_now_add=True)
...
class StudentGoal(models.Model):
    ...
    student = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='learning_goals', verbose_name=_('Öğrenci'))
    ...
    # Hedef detayları
    course = models.ForeignKey(Course, null=True, blank=True, on_delete=models.CASCADE, related_name='student_goals', verbose_name=_('Ders'))
    target_date = models.DateField(verbose_name=_('Hedef Tarihi'))
    target_metric = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True, verbose_name=_('Hedef Metrik'))
    current_progress = models.DecimalField(max_digits=5, decimal_places=2, default=Decimal('0.00'), verbose_name=_('Mevcut İlerleme'))
    ...
    # Hatırlatıcılar
    reminder_enabled = models.BooleanField(default=True, verbose_name=_('Hatırlatıcı Aktif'))
...
class CourseReview(models.Model):
    ...
    course = models.ForeignKey(Course, on_delete=models.CASCADE, related_name='reviews', verbose_name=_('Ders'))
    student = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='course_reviews', verbose_name=_('Öğrenci'))
    
    # Puanlama (1-5)
    rating = models.IntegerField(choices=[(i, str(i)) for i in range(1, 6)], verbose_name=_('Puan'))
    
    # Detaylı puanlama
    content_rating = models.IntegerField(null=True, blank=True, choices=[(i, str(i)) for i in range(1, 6)], verbose_name=_('İçerik'))
    instructor_rating = models.IntegerField(null=True, blank=True, choices=[(i, str(i)) for i in range(1, 6)], verbose_name=_('Eğitmen'))
    difficulty_rating = models.IntegerField(null=True, blank=True, choices=[(i, str(i)) for i in range(1, 6)], verbose_name=_('Zorluk'))
...
class StudySession(models.Model):
    ...
    student = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='study_sessions', verbose_name=_('Öğrenci'))
    course = models.ForeignKey(Course, null=True, blank=True, on_delete=models.CASCADE, related_name='study_sessions', verbose_name=_('Ders'))
    lesson = models.ForeignKey(Lesson, null=True, blank=True, on_delete=models.CASCADE, related_name='study_sessions', verbose_name=_('Konu'))
    ...
    duration_minutes = models.IntegerField(null=True, blank=True, verbose_name=_('Süre (dk)'))
    
    # Notlar
    notes = models.TextField(blank=True, verbose_name=_('Notlar'))
    focus_score = models.IntegerField(null=True, blank=True, choices=[(i, str(i)) for i in range(1, 6)], verbose_name=_('Konsantrasyon'))
