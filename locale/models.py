...
class Language(models.Model):
    ...
    flag_emoji = models.CharField(
        max_length=10, blank=True, verbose_name=_("Bayrak Emoji")
    )
    ...
    # İstatistikler
    translation_completeness = models.DecimalField(
        max_digits=5, decimal_places=2, default=0, verbose_name=_("Çeviri Tamamlanma %")
    )
    total_strings = models.IntegerField(default=0, verbose_name=_("Toplam String"))
    translated_strings = models.IntegerField(
        default=0, verbose_name=_("Çevrilmiş String")
    )
...
class TranslationString(models.Model):
    ...
    key = models.CharField(
        max_length=200,
        unique=True,
        verbose_name=_("Çeviri Anahtarı"),
        help_text="common.save, auth.login",
    )
    context = models.CharField(
        max_length=20, choices=CONTEXT_CHOICES, default="UI", verbose_name=_("Bağlam")
    )
    ...
    requires_review = models.BooleanField(
        default=False, verbose_name=_("İnceleme Gerekli")
    )
...
class Translation(models.Model):
    ...
    is_machine_translated = models.BooleanField(
        default=False, verbose_name=_("Makine Çevirisi")
    )
    ...
    reviewed_at = models.DateTimeField(
        null=True, blank=True, verbose_name=_("İnceleme Tarihi")
    )
...
class LocalizedContent(models.Model):
    ...
    language = models.ForeignKey(
        Language, on_delete=models.CASCADE, verbose_name=_("Dil")
    )
...
class TranslationMemory(models.Model):
    ...
    source_language = models.ForeignKey(
        ...
        verbose_name=_("Kaynak Dil"),
    )
    target_language = models.ForeignKey(
        Language,
        on_delete=models.CASCADE,
        related_name="target_memories",
        verbose_name=_("Hedef Dil"),
    )
...
class UserLanguagePreference(models.Model):
    ...
    # Fallback diller
    fallback_languages = models.JSONField(
        default=list, blank=True, verbose_name=_("Yedek Diller")
    )

    # Bölgesel ayarlar
    timezone = models.CharField(
        max_length=100, default="Europe/Istanbul", verbose_name=_("Zaman Dilimi")
    )
    date_format = models.CharField(
        max_length=50, default="DD.MM.YYYY", verbose_name=_("Tarih Formatı")
    )
    time_format = models.CharField(
        max_length=50, default="HH:mm", verbose_name=_("Saat Formatı")
    )
    number_format = models.CharField(
        max_length=50, default="1.234,56", verbose_name=_("Sayı Formatı")
    )
    currency_symbol = models.CharField(
        max_length=10, default="₺", verbose_name=_("Para Birimi Sembolü")
    )

    # Auto-translate
    auto_translate = models.BooleanField(
        default=False, verbose_name=_("Otomatik Çeviri")
    )
...
class TranslationJob(models.Model):
    ...
    target_languages = models.ManyToManyField(
        Language, related_name="target_jobs", verbose_name=_("Hedef Diller")
    )
    ...
    progress_percentage = models.DecimalField(
        max_digits=5, decimal_places=2, default=0, verbose_name=_("İlerleme %")
    )
    ...
    started_at = models.DateTimeField(
        null=True, blank=True, verbose_name=_("Başlangıç")
    )
...
class LocaleAuditLog(models.Model):
    ...
    action = models.CharField(
        max_length=20, choices=ACTION_CHOICES, verbose_name=_("Aksiyon")
    )
    ...
    ip_address = models.GenericIPAddressField(
        null=True, blank=True, verbose_name=_("IP Adresi")
    )
...
class MissingTranslation(models.Model):
    ...
    detected_in = models.CharField(
        max_length=200, blank=True, verbose_name=_("Tespit Edilen Yer")
    )
    ...
    resolved_at = models.DateTimeField(
        null=True, blank=True, verbose_name=_("Çözüm Tarihi")
    )
