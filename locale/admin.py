...
from .models import (
    Language, TranslationString, Translation, LocalizedContent,
    TranslationMemory, UserLanguagePreference, TranslationJob,
    LocaleAuditLog, MissingTranslation
)


@admin.register(Language)
class LanguageAdmin(admin.ModelAdmin):
    list_display = ('code', 'name', 'native_name', 'flag_emoji', 'is_rtl', 'is_active', 'is_default', 'translation_completeness', 'order')
...
class TranslationStringAdmin(admin.ModelAdmin):
    list_display = ('key', 'context', 'module', 'is_translated', 'requires_review', 'usage_count', 'created_at')
...
class TranslationAdmin(admin.ModelAdmin):
    list_display = ('translation_string', 'language', 'status', 'is_machine_translated', 'confidence_score', 'translated_by', 'reviewed_by')
...
class LocalizedContentAdmin(admin.ModelAdmin):
    list_display = ('content_type', 'object_id', 'language', 'field_name', 'is_published', 'updated_at')
...
class TranslationMemoryAdmin(admin.ModelAdmin):
    list_display = ('source_language', 'target_language', 'source_text_preview', 'usage_count', 'quality_score', 'is_verified')
...
class UserLanguagePreferenceAdmin(admin.ModelAdmin):
    list_display = ('user', 'preferred_language', 'timezone', 'date_format', 'auto_translate', 'updated_at')
...
class TranslationJobAdmin(admin.ModelAdmin):
    list_display = ('job_name', 'source_language', 'status', 'progress_percentage', 'total_items', 'completed_items', 'assigned_to', 'created_at')
...
class MissingTranslationAdmin(admin.ModelAdmin):
    list_display = ('translation_string', 'language', 'priority', 'detected_in', 'is_resolved', 'detected_at')
    ...
    def mark_as_resolved(self, request, queryset):
        ...
        self.message_user(request, f'{updated} eksik çeviri çözüldü olarak işaretlendi.')
