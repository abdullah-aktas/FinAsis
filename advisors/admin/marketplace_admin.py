...
class ConsultantProfileAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("İletişim", {"fields": ("office_address", "city", "phone", "website")}),
        (
            "Uzmanlık",
            {
                "fields": (
        ...
        ("Fiyatlandırma", {"fields": ("hourly_rate", "currency", "commission_rate")}),
        (
            "Müsaitlik",
        ...
        ("Öne Çıkarma", {"fields": ("is_featured", "featured_until")}),
        ("Meta", {"fields": ("created_at", "updated_at")}),
    )

    actions = [
        "approve_consultants",
        "approve_and_blockchain",
        "verify_consultant_documents",
        "reject_consultants",
        "feature_consultants",
    ]
    ...
    def approve_consultants(self, request, queryset):
        ...
        count = 0
        for consultant in queryset.filter(
        ...
        if count > 0:
            self.message_user(
                request,
                f"{count} mali müşavir onaylandı ve blockchain anlaşması yapıldı.",
            )
    ...
    def reject_consultants(self, request, queryset):
        ...
        count = queryset.filter(approval_status__in=["pending", "under_review"]).update(
...
class ConsultantServiceAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Fiyatlandırma", {"fields": ("pricing_type", "price", "currency")}),
        (
            "Süre ve Teslimat",
            {"fields": ("duration_minutes", "estimated_delivery_days")},
        ),
        ("Detaylar", {"fields": ("includes", "is_active", "total_orders")}),
    )
...
class ConsultationPaymentAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            "Tutarlar",
            {"fields": ("amount", "commission", "consultant_amount", "currency")},
        ),
        ("Durum", {"fields": ("status", "paid_at", "payout_to_consultant_at")}),
        ("Gateway", {"fields": ("gateway_name", "transaction_id", "gateway_response")}),
        (
            "İade",
            {
...
class ConsultantContractAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            "İçerik",
            {"fields": ("scope_of_work", "terms_and_conditions", "deliverables")},
        ),
        (
            "Finansal",
            {"fields": ("contract_value", "payment_terms", "payment_schedule")},
        ),
        (
            "Tarihler",
        ...
        (
            "Dosyalar",
            {"fields": ("contract_document", "signed_document", "attachments")},
        ),
        (
            "Fesih",
            {
                "fields": ("termination_reason", "terminated_by", "terminated_at"),
                "classes": ("collapse",),
            },
        ),
    )
...
class ConsultantReviewAdmin(admin.ModelAdmin):
    ...
    list_filter = ["rating", "is_published", "is_verified", "is_featured", "created_at"]
    search_fields = ["consultant__display_name", "client__username", "title", "comment"]
    readonly_fields = [
    ...
    fieldsets = (
        ...
        ("Durum", {"fields": ("is_verified", "is_featured", "is_published")}),
        ("Faydalılık", {"fields": ("helpful_count", "not_helpful_count")}),
    )
...
class ConsultantDocumentAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Geçerlilik", {"fields": ("issue_date", "expiry_date")}),
    )
...
class ConsultantPayoutAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Mali Müşavir", {"fields": ("consultant", "amount", "currency")}),
        ("Dönem", {"fields": ("period_start", "period_end", "included_payments")}),
        ("Banka Bilgileri", {"fields": ("bank_name", "account_holder", "iban")}),
        (
            "İşlem",
            {"fields": ("status", "transaction_reference", "processed_at", "notes")},
        ),
    )
