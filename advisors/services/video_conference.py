...
class FinasisMeetingProvider(VideoConferenceProvider):
    ...
    def __init__(self):
        base = getattr(settings, 'FINASIS_MEETING_BASE_URL', '') or getattr(settings, 'SITE_BASE_URL', '')
...
class VideoConferenceFactory:
    ...
    def get_provider(cls, provider_name=None):
        ...
        if not provider_class:
            raise ValueError(f"Desteklenmeyen video konferans sağlayıcısı: {provider_name}")
    ...
    def create_consultation_meeting(cls, booking, provider_name=None):
        ...
        organizer_user = getattr(getattr(getattr(booking, 'consultant', None), 'advisor', None), 'user', None)
...
def create_meeting_for_booking(booking, provider_name=None):
    ...
    return VideoConferenceFactory.create_consultation_meeting(booking, provider_name=provider_name)
...
def cancel_meeting_for_booking(booking, provider_name=None):
    ...
    try:
        provider = VideoConferenceFactory.get_provider(provider_name or getattr(booking, 'video_provider', None))
