    ...
    if not created:
        ...
        if instance.status == 'completed' and not instance.payout_to_consultant_at:
            ...
            try:
                ...
                logger.info(f"Payout oluşturuldu/güncellendi: {payout.id} - {instance.consultant.display_name}")
            except Exception as e:
...
def consultant_review_post_save(sender, instance, created, **kwargs):
    ...
    try:
        ...
        logger.info(f"Rating güncellendi: {instance.consultant.display_name} - {instance.consultant.average_rating}")
    except Exception as e:
