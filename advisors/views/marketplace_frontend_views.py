...
def consultant_list(request):
    ...
    consultants = ConsultantProfile.objects.filter(
        approval_status="approved", accepts_new_clients=True
    ...
    cities = (
        ConsultantProfile.objects.filter(approval_status="approved")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if consultant.instant_booking:
                ...
                messages.success(
                    request, _("Randevu talebiniz gönderildi. Onay bekleniyor.")
                )

            return redirect(
                "advisors:marketplace:booking-detail", booking_id=booking.id
            )
    else:
