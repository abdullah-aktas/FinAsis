...
class ConsultantProfileListSerializer(serializers.ModelSerializer):
    ...
    advisor_name = serializers.CharField(
        source="advisor.user.get_full_name", read_only=True
    )
...
class ConsultantProfileCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "instant_booking",
            "diploma_document",
            "graduation_document",  # Zorunlu belgeler
        ]
...
class ConsultantServiceSerializer(serializers.ModelSerializer):
    ...
    consultant_name = serializers.CharField(
        source="consultant.display_name", read_only=True
    )
...
class ConsultationBookingSerializer(serializers.ModelSerializer):
    ...
    consultant_photo = serializers.ImageField(
        source="consultant.profile_photo", read_only=True
    )
    ...
    def validate(self, attrs):
        ...
        scheduled_datetime = timezone.datetime.combine(
            attrs["scheduled_date"], attrs["scheduled_time"]
        )
        ...
        if consultant and not consultant.is_available():
            raise serializers.ValidationError(
                "Bu mali müşavir şu anda müşteri kabul etmiyor."
            )
...
class ConsultationBookingUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_status(self, value):
        ...
        if value == "completed" and self.instance and not self.instance.actual_end_time:
            raise serializers.ValidationError(
                "Randevuyu tamamlamak için bitiş zamanı gereklidir."
            )
        return value
...
class ConsultantReviewSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if booking and booking.status != "completed":
            raise serializers.ValidationError(
                "Sadece tamamlanmış randevular için değerlendirme yapabilirsiniz."
            )

        # Müşteri daha önce değerlendirme yapmış mı?
        request = self.context.get("request")
        if request and hasattr(request, "user") and request.user.is_authenticated:
            if ConsultantReview.objects.filter(
                booking=booking, client=request.user
            ).exists():
                raise serializers.ValidationError(
                    "Bu randevu için zaten değerlendirme yaptınız."
                )
