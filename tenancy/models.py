...
class Company(models.Model):
    tenant = models.ForeignKey(
        Tenant, on_delete=models.CASCADE, related_name="companies"
    )
    name = models.CharField(max_length=255)
    is_active = models.BooleanField(default=True)
    created_at = models.DateTimeField(auto_now_add=True)
...
class TenantSettings(models.Model):
    ...
    tenant = models.OneToOneField(
        Tenant, on_delete=models.CASCADE, related_name="settings", verbose_name="Tenant"
    )

    # Branding
    logo = models.ImageField(
        upload_to="tenant_logos/", null=True, blank=True, verbose_name="Logo"
    )
    ...
    feature_limits = models.JSONField(
        default=dict,
        verbose_name="Özellik Limitleri",
        help_text='{"users": 50, "invoices": 1000}',
    )

    # Kotalar
    max_users = models.IntegerField(default=10, verbose_name="Maksimum Kullanıcı")
    max_storage_mb = models.IntegerField(
        default=1024, verbose_name="Maksimum Depolama (MB)"
    )
    max_api_calls_per_day = models.IntegerField(
        default=10000, verbose_name="Günlük Maksimum API Çağrısı"
    )
    ...
    language = models.CharField(
        max_length=10, default="tr", verbose_name="Varsayılan Dil"
    )

    # Güvenlik
    ip_whitelist = models.JSONField(
        default=list, blank=True, verbose_name="IP Beyaz Liste"
    )
...
class TenantBilling(models.Model):
    ...
    tenant = models.ForeignKey(
        Tenant, on_delete=models.CASCADE, related_name="billings", verbose_name="Tenant"
    )
    ...
    base_amount = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name="Taban Tutar"
    )
    usage_amount = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, verbose_name="Kullanım Tutarı"
    )
...
class TenantAudit(models.Model):
    ...
    action = models.CharField(
        max_length=30, choices=ACTION_TYPES, verbose_name="Aksiyon"
    )
