...
from .models import (
    Tenant, Company, UserTenantRole,
    TenantSettings, TenantUsage, TenantBilling, TenantAudit
)
...
class TenantSettingsAdmin(admin.ModelAdmin):
    list_display = ('tenant', 'max_users', 'max_storage_mb', 'language', 'timezone', 'require_2fa')
...
class TenantUsageAdmin(admin.ModelAdmin):
    list_display = ('tenant', 'date', 'active_users', 'storage_used_mb', 'api_calls', 'invoices_created', 'reports_generated')
...
class TenantBillingAdmin(admin.ModelAdmin):
    list_display = ('tenant', 'invoice_number', 'billing_period_start', 'billing_period_end', 'total_amount', 'status', 'due_date')
