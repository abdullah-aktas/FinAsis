...
from .models import Tenant, TenantSettings, TenantBilling, TenantAudit, TenantUsage
...
def tenant_settings(request, tenant_id):
    ...
    if not request.user.is_superuser:
        ...
        if (
            ...
            or request.user.company.tenant != tenant
        ):
...
def ajax_tenant_stats(request, tenant_id):
    ...
    """
    if not request.user.is_superuser:
        return JsonResponse({"success": False, "error": _("Yetkiniz yok")}, status=403)
    ...
    try:
        ...
        stats = {
            ...
            "active_users": User.objects.filter(
                company__tenant=tenant, is_active=True
            ).count(),
        ...
        return JsonResponse(
            {"success": False, "error": _("Tenant bulunamadı")}, status=404
        )
