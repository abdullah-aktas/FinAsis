...
def kobi_dashboard(request):
    ...
    try:
        ...
        ).aggregate(debit_total=Sum("debit_amount"), credit_total=Sum("credit_amount"))
        ...
        # Kar marjı hesaplama
        profit_margin = (
            (net_profit / monthly_revenue * 100) if monthly_revenue > 0 else 0
        )
            ...
            alerts.append(
                {
                    ...
                    "description": _(
                        "Nakit bakiyeniz negatif. Acil nakit akışı planlaması yapın."
                    ),
                    "priority": "high",
                    "priority_text": _("Yüksek"),
                    "action": _(
                        "Nakit akış tahmini oluşturun ve vadeli ödemeleri gözden geçirin."
                    ),
                }
            )
            ...
            alerts.append(
                {
                    ...
                    "description": _(
                        "Kar marjınız %5'in altında. Maliyet analizi yapmanız öneriyoruz."
                    ),
                    "priority": "medium",
                    "priority_text": _("Orta"),
                    "action": _(
                        "Gider kalemlerini detaylı analiz edin ve fiyatlandırma stratejinizi gözden geçirin."
                    ),
                }
            )
            ...
            alerts.append(
                {
                    ...
                    "description": _(
                        "Cari oranınız 1.0'ın altında. Kısa vadeli borç ödeme gücünüz sınırlı."
                    ),
                    "priority": "high",
                    "priority_text": _("Yüksek"),
                    "action": _(
                        "Alacak tahsilat süresini kısaltın veya kısa vadeli finansman alternatifleri değerlendirin."
                    ),
                }
            )
        ...
        # Son işlemler
        recent_transactions = (
            JournalEntry.objects.filter(voucher__company=company)
        ...
        for entry in recent_transactions:
            amount = (
                entry.debit_amount if entry.debit_amount > 0 else entry.credit_amount
            )
            formatted_transactions.append(
                {
                    ...
                    "type_text": getattr(
                        entry.voucher,
                        "get_voucher_type_display",
                        lambda: getattr(entry.voucher, "voucher_type", ""),
                    )(),
        ...
        context.update(
            {
                ...
                "income_expense_data": json.dumps(
                    [float(monthly_revenue), float(monthly_expenses)]
                ),
            }
        )
        ...
        if "BudgetPlan" in globals():  # güvenli kontrol
            current_budget = BudgetPlan.objects.filter(
                company=company, period_start__lte=today, period_end__gte=today
            ).first()
        ...
        if current_budget and BudgetItem:
            try:
                ...
                for item in budget_items:
                    acc_name = getattr(
                        item.account, "name", getattr(item.account, "code", "")
                    )
                    ...
                    actual_amount = (
                        ...
                        ).aggregate(total=Sum("debit_amount") - Sum("credit_amount"))[
                            "total"
                        ]
                        or 0
                    )
...
def financial_analysis_report(request):
    ...
    if not analysis:
        ...
        if analysis:
            ...
            messages.error(
                request, _("Analiz oluşturulamadı. Yeterli veri bulunmuyor.")
            )
            return redirect("finance:kobi_dashboard")
    ...
    context["ratios"] = {
        ...
        "profit_margin": {
            ...
            "status_text": get_ratio_status_text(
        ...
        "debt_to_equity": {
            ...
            "status_text": get_ratio_status_text(
                analysis.debt_to_equity_ratio, "debt_to_equity"
            ),
        },
        "debt_ratio": {
    ...
    context.update(
        {
            "profit_trend_labels": json.dumps(
                ["Oca", "Şub", "Mar", "Nis", "May", "Haz"]
            ),
            "revenue_data": json.dumps(
                [100000, 120000, 110000, 130000, 125000, 140000]
            ),
            ...
            "company_ratios": json.dumps(
                [80, 75, 85, 70, 60]
            ),  # Normalize edilmiş oranlar
            "sector_ratios": json.dumps([75, 70, 80, 65, 70]),
            "asset_labels": json.dumps(
                ["Dönen Varlıklar", "Sabit Varlıklar", "Maddi Olmayan"]
            ),
...
def get_health_description(score):
    if score >= 80:
        ...
        return _(
            "Finansal durumunuz orta seviyede. Bazı alanlarda iyileştirme gerekli."
        )
    elif score >= 50:
...
def calculate_current_assets(company, date):
    ...
    ).aggregate(total=Sum("debit_amount") - Sum("credit_amount"))["total"] or Decimal(
        "0"
    )
...
def calculate_current_liabilities(company, date):
    ...
    ).aggregate(total=Sum("credit_amount") - Sum("debit_amount"))["total"] or Decimal(
        "0"
    )
        ...
        recommendations.append(
            {
                ...
                "description": _(
                    "Cari oranınız düşük. Kısa vadeli borç ödeme gücünüzü artırmak için alacak tahsilat süresini kısaltın."
                ),
                "priority": "high",
                "priority_text": _("Yüksek"),
                "impact": _("Yüksek"),
        ...
        recommendations.append(
            {
                ...
                "description": _(
                    "Kar marjınız düşük. Maliyet yapısını gözden geçirin ve fiyatlandırma stratejinizi optimize edin."
                ),
                "priority": "medium",
                "priority_text": _("Orta"),
                "impact": _("Yüksek"),
        ...
        recommendations.append(
            {
                ...
                "description": _(
                    "Borç/özkaynak oranınız yüksek. Borç yapısını optimize edin veya özkaynak artırımı düşünün."
                ),
                "priority": "medium",
                "priority_text": _("Orta"),
                "impact": _("Orta"),
...
def ajax_cash_flow_data(request):
    ...
    return JsonResponse(
        {"labels": labels, "inflow_data": inflow_data, "outflow_data": outflow_data}
    )
