...
class BankAccountListView(ListView):
    ...
    def get_queryset(self):
        """Filter by user without additional queries."""
    ...
    def get_context_data(self, **kwargs):
        ...
        context["total_balance"] = (
            accounts.aggregate(Sum("balance"))["balance__sum"] or 0
        )
        return context
...
class BankAccountUpdateView(UpdateView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "finance:bank_account_detail", kwargs={"pk": self.kwargs["pk"]}
        )
...
class TransactionListView(ListView):
    ...
    def get_queryset(self):
        """Optimize with select_related for accounts and category."""
        queryset = (
            Transaction.objects.filter(
                Q(source_account__user=self.request.user)
                | Q(destination_account__user=self.request.user)
            )
            .select_related("source_account", "destination_account", "category")
            .order_by("-date")
        )
...
class TransactionUpdateView(UpdateView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "finance:transaction_detail", kwargs={"pk": self.kwargs["pk"]}
        )
...
def bank_import(request):
    ...
    accounts = CoreBankAccount.objects.filter(company=company, is_active=True).order_by(
        "bank__name"
    )
    ...
    if request.method == "POST" and request.FILES.get("file"):
        ...
        if request.POST.get("confirm") == "1":
            acc = get_object_or_404(
                CoreBankAccount, pk=request.POST.get("account_id"), company=company
            )
            for r in rows:
                raw_date = (
                    r.get("date") or r.get("tarih") or r.get("Date") or r.get("Tarih")
                )
                raw_desc = (
                    r.get("description")
                    or r.get("açıklama")
                ...
                if not (raw_date and raw_amount):
                    continue
            ...
            AuditLog.log_action(
                obj=acc,
                action="bank_csv_import",
                user=request.user,
                ip=request.META.get("REMOTE_ADDR"),
                payload={"rows": len(rows), "created": created},
            )
            messages.success(
                request,
                _("CSV içe aktarma tamamlandı. Oluşturulan işlem: {0}").format(created),
            )
            return redirect("finance:bank_summary")
    return render(
        request,
        "finance/bank_import.html",
        {"accounts": accounts, "preview": preview, "created": created},
    )
