...
from finance.enhanced_accounting_models import FiscalPeriod, TrialBalanceSnapshot
from finance.tasks import compute_monthly_depreciation
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--skip-depreciation", action="store_true", help="Amortismanı atla"
        )
    ...
    def handle(self, *args, **options):
        ...
        if fiscal_period.is_closed:
            self.stdout.write(
                self.style.WARNING(
                    "Fiscal period already closed; skipping month close logic."
                )
            )
            return
        ...
        # Mizan snapshot
        snapshot = TrialBalanceSnapshot.build_snapshot(
            company, fiscal_period, as_of_date=run_date, user=None
        )
        self.stdout.write(
            self.style.SUCCESS(
                f"Snapshot created with {len(snapshot.account_balances)} accounts."
            )
        )

        self.stdout.write(
            self.style.SUCCESS(
                "Month close base operations finished (period not marked closed yet)."
            )
        )
