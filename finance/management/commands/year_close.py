...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--period-id", type=int, help="Specific fiscal period id (opsiyonel)"
        )
    ...
    def handle(self, *args, **options):
        ...
        if options.get("period_id"):
            fiscal_period = FiscalPeriod.objects.filter(
                company=company, pk=options["period_id"]
            ).first()
        else:
            # Son açık dönem
            fiscal_period = (
                FiscalPeriod.objects.filter(company=company, is_closed=False)
                .order_by("-start_date")
                .first()
            )
        ...
        snapshot = TrialBalanceSnapshot.build_snapshot(
            company, fiscal_period, as_of_date=fiscal_period.end_date
        )
