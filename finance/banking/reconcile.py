...
def score_match(txn: BankTxn, cand: LedgerItem, *, amount_tol: Decimal = Decimal("0.01"), date_window_days: int = 3) -> float:
    score = 0.0
    ...
    # IBAN (20 pts)
    if txn.counter_iban and cand.counter_iban and txn.counter_iban.replace(' ', '') == cand.counter_iban.replace(' ', ''):
        score += 20.0
...
def reconcile(bank_txns: Iterable[BankTxn], ledger_items: Iterable[LedgerItem], *, min_score: float = 60.0) -> List[Tuple[str, str, float]]:
    ...
    for t in txns:
        ...
        for c in cands:
            ...
            s = score_match(t, c)
            if s >= min_score:
                if best is None or s > best[1] or (s == best[1] and abs((t.date - next(x for x in cands if x.id == (best[0] if best else c.id)).date).days) > abs((t.date - c.date).days)):
                    best = (c.id, s)
        if best:
            taken.add(best[0])
