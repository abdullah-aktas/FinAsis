...
from finance.models import BaseModel, AuditableMixin

from accounting.models import Company
from finance.models import Account, Voucher
...
class BankAccount(BaseModel, AuditableMixin):
    ...
    bank = models.ForeignKey(
        Bank, on_delete=models.PROTECT, related_name="accounts", verbose_name=_("Banka")
    )
    ...
    def current_balance(self):
        ...
        incoming = sum(
            t.amount for t in self.incoming_transactions.filter(is_reconciled=True)
        )
        outgoing = sum(
            t.amount for t in self.outgoing_transactions.filter(is_reconciled=True)
        )
...
class BankTransaction(BaseModel, AuditableMixin):
    ...
    reference_number = models.CharField(
        _("Referans No"), max_length=50, blank=True, null=True
    )
    description = models.CharField(_("Açıklama"), max_length=255)
    transaction_type = models.CharField(
        _("İşlem Tipi"), max_length=20, choices=TRANSACTION_TYPES
    )
    ...
    def get_transaction_type_display(self):
        ...
        return dict(self.TRANSACTION_TYPES).get(
