...
def banking_dashboard(request):
    ...
    recent_transactions = BankTransaction.objects.filter(company=company).order_by(
        "-transaction_date"
    )[:20]

    # Toplam bakiyeler
    total_balance = sum(
        acc.current_balance if isinstance(acc.current_balance, (int, float)) else 0
        for acc in accounts
    )
...
def transaction_list(request):
    ...
    if account_filter:
        ...
        transactions = transactions.filter(
            Q(source_account_id=account_filter)
            | Q(destination_account_id=account_filter)
        )
...
def ajax_dashboard_stats(request):
    ...
    stats = {
        ...
        "recent_transaction_count": BankTransaction.objects.filter(
            company=company, transaction_date__gte=timezone.now().date()
        ).count(),
    }
