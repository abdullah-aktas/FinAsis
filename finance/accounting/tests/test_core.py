...
def test_posting_rule_creates_balanced_voucher():
    ...
    VoucherType.objects.create(code="MA", name="Mahsup")
...
def test_vat_and_fifo_helpers():
    ...
    layers = [
        StockLayer(quantity=Decimal("5"), unit_cost=Decimal("10")),
        StockLayer(quantity=Decimal("10"), unit_cost=Decimal("12")),
    ]
    res = fifo_consume(layers, Decimal("8"))
    assert res.cost_of_issued == Decimal("5") * Decimal("10") + Decimal("3") * Decimal(
        "12"
    )
