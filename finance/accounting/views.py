...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["account_count"] = (
            Account.objects.filter(company=user_company).count() if user_company else 0
        )
        context["active_account_count"] = (
            Account.objects.filter(company=user_company, is_active=True).count()
            if user_company
            else 0
        )

        context["voucher_count"] = (
            Voucher.objects.filter(company=user_company).count() if user_company else 0
        )
        # Not: Voucher.STATE_CHOICES lower-case kullanıyor: ('draft','posted','cancelled')
        context["voucher_draft_count"] = (
            Voucher.objects.filter(company=user_company, state="draft").count()
            if user_company
            else 0
        )
        context["voucher_posted_count"] = (
            Voucher.objects.filter(company=user_company, state="posted").count()
            if user_company
            else 0
        )
        ...
        if v_total:
            context["posted_percent"] = round(
                (context["voucher_posted_count"] / v_total) * 100
            )
            context["draft_percent"] = round(
                (context["voucher_draft_count"] / v_total) * 100
            )
        else:
        ...
        context["recent_vouchers"] = (
            Voucher.objects.filter(company=user_company).order_by(
                "-date", "-created_at"
            )[:10]
            if user_company
            else []
...
class AccountListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return (
            Account.objects.filter(company=company).order_by("code")
            if company
...
class VoucherListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        company = getattr(self.request.user, "company", None)
        return (
            Voucher.objects.filter(company=company).order_by("-date", "-created_at")
            if company
            else Voucher.objects.none()
        )
...
class VoucherDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["voucher_lines"] = VoucherLine.objects.filter(voucher=obj).order_by(
            "line_no"
        )
        return context
...
class VoucherUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        if obj.state != "draft":
            messages.error(
                self.request, _("Sadece taslak durumdaki fişler düzenlenebilir.")
            )
        ...
        with transaction.atomic():
            ...
            if formset.is_valid():
                ...
                for i, line in enumerate(
                    VoucherLine.objects.filter(voucher=self.object).order_by("line_no")
                ):
                    line.line_no = i + 1
                    line.save()
...
def post_voucher(request, pk):
    ...
    voucher = (
        get_object_or_404(Voucher, pk=pk, company=company)
        if company
        else get_object_or_404(Voucher, pk=pk)
    )
...
def cancel_voucher(request, pk):
    ...
    if voucher.state != "draft":
        messages.error(
            request,
            _(
                "Sadece taslak durumdaki fişler iptal edilebilir. Onaylanmış fişler için ters kayıt oluşturun."
            ),
        )
        return redirect("accounting:voucher_detail", pk=voucher.pk)
...
def create_reverse_voucher(request, pk):
    ...
    if source_voucher.state != "posted":
        messages.error(
            request, _("Sadece onaylanmış fişler için ters kayıt oluşturulabilir.")
        )
    ...
    for acc in accounts:
        debit = (
            VoucherLine.objects.filter(
                account=acc, voucher__date__gte=start_date, voucher__date__lte=end_date
    ...
    if format_ == "excel":
        ...
        for row in data:
            p.drawString(
                50,
                y,
                f"{row['Hesap Kodu']} - {row['Hesap Adı']} | Borç: {row['Borç']} | Alacak: {row['Alacak']} | Bakiye: {row['Bakiye']}",
            )
            y -= 20
            if y < 50:
                p.showPage()
    ...
    if format_ == "excel":
        ...
        for row in data:
            p.drawString(
                50,
                y,
                f"{row['Fiş No']} | {row['Tarih']} | {row['Açıklama']} | {row['Hesap Kodu']} - {row['Hesap Adı']} | Borç: {row['Borç']} | Alacak: {row['Alacak']}",
            )
            y -= 20
            if y < 50:
                p.showPage()
...
def export_general_ledger(request):
    ...
    for acc in accounts:
        lines = VoucherLine.objects.filter(
            account=acc, voucher__date__gte=start_date, voucher__date__lte=end_date
        ).order_by("voucher__date", "voucher__number")
        for line in lines:
            data.append(
                {
                    "Hesap Kodu": acc.code,
    ...
    if format_ == "excel":
        ...
        for row in data:
            p.drawString(
                50,
                y,
                f"{row['Hesap Kodu']} - {row['Hesap Adı']} | {row['Fiş No']} | {row['Tarih']} | {row['Açıklama']} | Borç: {row['Borç']} | Alacak: {row['Alacak']}",
            )
            y -= 20
            if y < 50:
                p.showPage()
