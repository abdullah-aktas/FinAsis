    ...
    def post_vouchers(self, request, queryset):
        ...
        for voucher in queryset:
            try:
                ...
                error_count += 1
                self.message_user(
                    request, _(f"Hata: {voucher.number} - {str(e)}"), level="ERROR"
                )

        if posted_count:
            self.message_user(
                request, _(f"{posted_count} fiş başarıyla onaylandı."), level="SUCCESS"
            )
...
class CurrencyAdmin(admin.ModelAdmin):
    ...
    def set_as_default(self, request, queryset):
        ...
        if queryset.count() != 1:
            self.message_user(
                request, _("Lütfen sadece bir para birimi seçin."), level="ERROR"
            )
            return
        ...
        self.message_user(
            request,
            _(f"{currency.code} varsayılan para birimi olarak ayarlandı."),
            level="SUCCESS",
        )
