...
from finance.accounting.models import Voucher, VoucherLine, VoucherType, Account
    ...
    def handle(self, *args, **options):
        ...
        if acc_exp and acc_dep:
            VoucherLine.objects.create(
                voucher=v, line_no=1, account=acc_exp, debit_amount=Decimal("1000.00")
            )
            VoucherLine.objects.create(
                voucher=v, line_no=2, account=acc_dep, credit_amount=Decimal("1000.00")
            )
        if v.is_balanced():
            v.post()
            self.stdout.write(
                self.style.SUCCESS("Dönem sonu fişi oluşturuldu ve onaylandı.")
            )
        else:
            self.stdout.write(
                self.style.WARNING(
                    "Dönem sonu fişi denksiz; lütfen hesapları kontrol edin."
                )
            )
