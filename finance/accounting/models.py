...
class Account(models.Model):
    ...
    def get_balance(self, start_date=None, end_date=None):
        ...
        totals = query.aggregate(total_debit=Sum('debit_amount'), total_credit=Sum('credit_amount'))
    ...
    def clean(self):
        ...
        if strict:
            ...
            if not pattern.match(code):
                raise ValidationError({'code': _('Hesap kodu TDHP formatında olmalıdır (örn: 100, 100.01, 120.01.001).')})
        return super().clean()
...
class AccountingPeriod(models.Model):
    ...
    closed_by = models.ForeignKey(dj_settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, blank=True, null=True, verbose_name=_('Kapatılan Kullanıcı'))
...
class VoucherLine(models.Model):
    ...
    debit_amount = models.DecimalField(_('Borç Tutarı'), max_digits=15, decimal_places=2, default=Decimal('0.00'))
    credit_amount = models.DecimalField(_('Alacak Tutarı'), max_digits=15, decimal_places=2, default=Decimal('0.00'))
...
class AutoBookingRule(models.Model):
    ...
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name='auto_booking_rules', verbose_name=_('Şirket'))
    name = models.CharField(max_length=100, verbose_name=_('Kural Adı'))
    keyword_pattern = models.CharField(max_length=255, verbose_name=_('Anahtar Kelime / Regex'))
    nature = models.CharField(max_length=20, choices=NATURE_CHOICES, default='expense', verbose_name=_('İşlem Türü'))
    debit_account_code = models.CharField(max_length=20, blank=True, null=True, verbose_name=_('Borç Hesap Kodu'))
    credit_account_code = models.CharField(max_length=20, blank=True, null=True, verbose_name=_('Alacak Hesap Kodu'))
    kdv_account_code = models.CharField(max_length=20, blank=True, null=True, verbose_name=_('KDV Hesap Kodu'))
    priority = models.PositiveIntegerField(default=100, verbose_name=_('Öncelik (küçük önce)'))
