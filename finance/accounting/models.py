...
"""FinAsis Muhasebe Modülü - Modeller (tekil ve temiz sürüm)"""
...
# Dış bağımlılıklar (opsiyonel)
try:  # opsiyonel banka entegrasyonu
    from finance.bank_integration.models import IntegratedBankAccount as FinanceBankAccount  # type: ignore
except Exception:
    FinanceBankAccount = None

from accounting.models import Company, Customer

try:
    from company.models import FiscalYear  # type: ignore
except Exception:

    class FiscalYear(models.Model):  # minimal fallback
        ...
        class Meta:
            app_label = "finance_accounting"
            verbose_name = _("Mali Yıl")
            verbose_name_plural = _("Mali Yıllar")


# Hint for type checkers: the symbol exists at runtime via import or fallback above
if TYPE_CHECKING:  # pragma: no cover - static analysis aid only
    from company.models import FiscalYear as _FiscalYear  # type: ignore

    FiscalYear = _FiscalYear  # noqa: F811 - intentional re-alias for type checkers
...
class Account(models.Model):
    ...
    def get_balance(self, start_date=None, end_date=None):
        ...
        return (totals["total_debit"] or Decimal("0")) - (
            totals["total_credit"] or Decimal("0")
        )

    def clean(self):
        strict = getattr(dj_settings, "ACCOUNT_CODE_TDHP_STRICT", False)
        if strict:
            import re

            code = (self.code or "").strip()
            pattern = re.compile(r"^(\d{3})([\.-]\d{2,3}){0,3}$")
            if not pattern.match(code):
                raise ValidationError(
                    {
                        "code": _(
                            "Hesap kodu TDHP formatında olmalıdır (örn: 100, 100.01, 120.01.001)."
                        )
                    }
                )
        return super().clean()
...
class AccountingPeriod(models.Model):
    ...
    status = models.CharField(
        max_length=10, choices=STATUS_CHOICES, default="open", verbose_name=_("Durum")
    )
    closed_at = models.DateTimeField(
        blank=True, null=True, verbose_name=_("Kapanış Zamanı")
    )
    closed_by = models.ForeignKey(
        ...
        blank=True,
        null=True,
        verbose_name=_("Kapatılan Kullanıcı"),
    )
    ...
    def __str__(self):
        return (
            f"{self.company} {self.year}-{self.month:02d} ({self.get_status_display()})"
        )
...
class Voucher(models.Model):
    ...
    state = models.CharField(
        _("Durum"), max_length=10, choices=STATE_CHOICES, default="draft"
    )
    ...
    exchange_rate = models.DecimalField(
        _("Kur"), max_digits=18, decimal_places=6, default=Decimal("1.000000")
    )
    ...
    def is_balanced(self):
        ...
        return (totals["debit_sum"] or Decimal("0")) == (
            totals["credit_sum"] or Decimal("0")
        )

    def post(self):
        if self.total_amount > Decimal("100000") and (
            not self.reference or "APPROVED2" not in self.reference
        ):
        ...
        period = AccountingPeriod.objects.filter(
            company=self.company, year=self.date.year, month=self.date.month
        ).first()
        if period and period.status == "closed":
            raise ValidationError(
                _("Seçili fiş tarihi kapalı bir döneme ait. Önce dönemi açın.")
            )
        self.state = "posted"
        self.save()
...
class VoucherLine(models.Model):
    voucher = models.ForeignKey(
        Voucher, on_delete=models.CASCADE, related_name="lines", verbose_name=_("Fiş")
    )
    ...
    debit_amount = models.DecimalField(
        _("Borç Tutarı"), max_digits=15, decimal_places=2, default=Decimal("0.00")
    )
    credit_amount = models.DecimalField(
        _("Alacak Tutarı"), max_digits=15, decimal_places=2, default=Decimal("0.00")
    )
    ...
    def clean(self):
        ...
        if self.debit_amount == 0 and self.credit_amount == 0:
            raise ValidationError(
                _("Borç veya alacak tutarından biri doldurulmalıdır.")
            )
...
class GLBalance(models.Model):
    ...
    begin_balance = models.DecimalField(
        ...
        decimal_places=2,
        default=Decimal("0.00"),
        verbose_name=_("Açılış Bakiyesi"),
    )
    debit_total = models.DecimalField(
        max_digits=18,
        decimal_places=2,
        default=Decimal("0.00"),
        verbose_name=_("Borç Toplam"),
    )
    credit_total = models.DecimalField(
        max_digits=18,
        decimal_places=2,
        default=Decimal("0.00"),
        verbose_name=_("Alacak Toplam"),
    )
    end_balance = models.DecimalField(
        max_digits=18,
        decimal_places=2,
        default=Decimal("0.00"),
        verbose_name=_("Kapanış Bakiyesi"),
    )
...
class Invoice(models.Model):
    customer = models.ForeignKey(
        Customer, on_delete=models.CASCADE, verbose_name=_("Müşteri")
    )
    ...
    total_amount = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name=_("Toplam Tutar")
    )
    status = models.CharField(
        max_length=20,
        choices=[
...
class AutoBookingRule(models.Model):
    """Şirket bazlı otomatik fişleme kuralı (TDHP eşleme)."""
    ...
    keyword_pattern = models.CharField(
        max_length=255, verbose_name=_("Anahtar Kelime / Regex")
    )
    ...
    debit_account_code = models.CharField(
        max_length=20, blank=True, null=True, verbose_name=_("Borç Hesap Kodu")
    )
    credit_account_code = models.CharField(
        max_length=20, blank=True, null=True, verbose_name=_("Alacak Hesap Kodu")
    )
    kdv_account_code = models.CharField(
        max_length=20, blank=True, null=True, verbose_name=_("KDV Hesap Kodu")
    )
    priority = models.PositiveIntegerField(
        default=100, verbose_name=_("Öncelik (küçük önce)")
    )
...
# Explicit re-export list for static analyzers and star imports
__all__ = [
    "FiscalYear",
    "AccountType",
    "Account",
