...
class ChartOfAccounts(models.Model):
    ...
    code = models.CharField(
        _("Hesap Kodu"), max_length=20, help_text="Örn: 100, 120.01"
    )
    ...
    parent = models.ForeignKey(
        ...
        verbose_name=_("Ana Hesap"),
    )
    level = models.PositiveIntegerField(_("Seviye"), default=1)
    is_detail_account = models.BooleanField(
        _("Detay Hesap"),
        default=False,
    ...
    def get_balance(self, start_date=None, end_date=None):
        ...
        aggregated = entries.aggregate(
            debit_total=Sum("debit_amount"), credit_total=Sum("credit_amount")
        )
        ...
        return (
            (debits - credits)
            if self.account_type in ["1", "2", "7", "8"]
            else (credits - debits)
        )
...
class FiscalPeriod(models.Model):
    ...
    name = models.CharField(
        _("Dönem Adı"), max_length=100, help_text="Örn: 2024 Mali Yılı"
    )
    ...
    closed_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        verbose_name=_("Kapatan Kullanıcı"),
    )
    ...
    def clean(self):
        if self.start_date >= self.end_date:
            raise ValidationError(
                _("Başlangıç tarihi bitiş tarihinden küçük olmalıdır.")
            )
...
class JournalVoucher(models.Model):
    ...
    company = models.ForeignKey(
        Company, on_delete=models.CASCADE, related_name="journal_vouchers"
    )
    voucher_number = models.CharField(
        _("Fiş Numarası"), max_length=20, help_text="Şirket bazında benzersiz"
    )
    ...
    total_credit = models.DecimalField(
        _("Toplam Alacak"), max_digits=15, decimal_places=2, default=Decimal("0")
    )
    ...
    def clean(self):
        ...
        if self.fiscal_period and (
            self.date < self.fiscal_period.start_date
            or self.date > self.fiscal_period.end_date
        ):
            raise ValidationError(
                _("Fiş tarihi seçili mali dönem içerisinde olmalıdır.")
            )
    ...
    def save(self, *args, **kwargs):  # noqa: D401
        ...
        if self.pk and self.fiscal_period and self.fiscal_period.is_closed:
            raise ValidationError(
                _("Kapalı dönemde fiş üzerinde değişiklik yapılamaz.")
            )
...
class JournalEntry(models.Model):
    ...
    exchange_rate = models.DecimalField(
        _("Döviz Kuru"), max_digits=10, decimal_places=4, default=Decimal("1")
    )
    ...
    def clean(self):
        ...
        if self.debit_amount == 0 and self.credit_amount == 0:
            raise ValidationError(
                _("Borç veya alacak tutarından en az biri girilmelidir.")
            )
...
class DepreciationMethod(models.Model):
    ...
    company = models.ForeignKey(
        Company, on_delete=models.CASCADE, related_name="depreciation_methods"
    )
...
class FixedAsset(models.Model):
    ...
    purchase_cost = models.DecimalField(
        _("Satın Alma Maliyeti"), max_digits=15, decimal_places=2
    )
    useful_life_years = models.PositiveIntegerField(_("Faydalı Ömür (Yıl)"))
    salvage_value = models.DecimalField(
        _("Hurda Değeri"), max_digits=15, decimal_places=2, default=Decimal("0")
    )
    ...
    status = models.CharField(
        _("Durum"), max_length=20, choices=ASSET_STATUS, default="ACTIVE"
    )
...
class DepreciationEntry(models.Model):
    ...
    fiscal_period = models.ForeignKey(
        FiscalPeriod, on_delete=models.PROTECT, related_name="depreciation_entries"
    )
    date = models.DateField(_("Tarih"))
    amount = models.DecimalField(
        _("Amortisman Tutarı"), max_digits=15, decimal_places=2
    )
    voucher = models.ForeignKey(
        JournalVoucher,
        on_delete=models.PROTECT,
    ...
    is_automatic = models.BooleanField(
        _("Otomatik"), default=True, help_text="Sistem tarafından otomatik hesaplanan"
    )
...
class TaxRate(models.Model):
    ...
    company = models.ForeignKey(
        Company, on_delete=models.CASCADE, related_name="tax_rates"
    )
...
class CostCenter(models.Model):
    ...
    parent = models.ForeignKey(
        ...
        verbose_name=_("Ana Masraf Merkezi"),
    )
...
class InventoryItem(models.Model):
    ...
    company = models.ForeignKey(
        Company, on_delete=models.CASCADE, related_name="inventory_items"
    )
    ...
    inventory_type = models.CharField(
        _("Stok Tipi"), max_length=20, choices=INVENTORY_TYPES
    )
    ...
    current_quantity = models.DecimalField(
        _("Mevcut Miktar"), max_digits=15, decimal_places=3, default=Decimal("0")
    )
    minimum_quantity = models.DecimalField(
        _("Minimum Miktar"), max_digits=15, decimal_places=3, default=Decimal("0")
    )
    maximum_quantity = models.DecimalField(
        _("Maksimum Miktar"), max_digits=15, decimal_places=3, default=Decimal("0")
    )

    # Fiyat bilgileri
    last_purchase_price = models.DecimalField(
        _("Son Alış Fiyatı"), max_digits=15, decimal_places=4, default=Decimal("0")
    )
    average_cost = models.DecimalField(
        _("Ortalama Maliyet"), max_digits=15, decimal_places=4, default=Decimal("0")
    )
    standard_selling_price = models.DecimalField(
        _("Standart Satış Fiyatı"),
        max_digits=15,
        decimal_places=4,
        default=Decimal("0"),
    )
    ...
    def update_average_cost(self):
        ...
        total_quantity = movements.aggregate(total=Sum("quantity"))["total"] or Decimal(
            "0"
        )
...
class StockMovement(models.Model):
    ...
    movement_type = models.CharField(
        _("Hareket Tipi"), max_length=20, choices=MOVEMENT_TYPES
    )
    date = models.DateField(_("Tarih"))
    ...
    total_cost = models.DecimalField(
        _("Toplam Maliyet"), max_digits=15, decimal_places=2
    )
    ...
    description = models.TextField(_("Açıklama"), blank=True)
    created_by = models.ForeignKey(
        dj_settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True
    )
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return (
            f"{self.inventory_item.item_code} - {self.movement_type} - {self.quantity}"
        )
    ...
    def save(self, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            self.inventory_item.refresh_from_db(
                fields=["current_quantity", "last_purchase_price"]
            )
...
class FinancialStatementGenerator:
    ...
    def generate_balance_sheet(self):
        ...
        totals_map = {
            row["account_id"]: (
                row["debit_total"] or Decimal("0"),
                row["credit_total"] or Decimal("0"),
            )
            for row in entries
        }
        ...
        for acc in detail_accounts:
            ...
            balance = (
                (debits - credits)
                if acc.account_type in ["1", "2"]
                else (credits - debits)
            )
            if acc.account_type == "1":
                balance_sheet["aktif"]["donen_varliklar"][acc.code] = {
                    "name": acc.name,
                    "balance": balance,
                }
                balance_sheet["aktif"]["toplam_aktif"] += balance
            elif acc.account_type == "2":
                balance_sheet["aktif"]["duran_varliklar"][acc.code] = {
                    "name": acc.name,
                    "balance": balance,
                }
                balance_sheet["aktif"]["toplam_aktif"] += balance
            elif acc.account_type == "3":
                balance_sheet["pasif"]["kisa_vadeli_borclar"][acc.code] = {
                    "name": acc.name,
                    "balance": balance,
                }
                balance_sheet["pasif"]["toplam_pasif"] += balance
            elif acc.account_type == "4":
                balance_sheet["pasif"]["uzun_vadeli_borclar"][acc.code] = {
                    "name": acc.name,
                    "balance": balance,
                }
                balance_sheet["pasif"]["toplam_pasif"] += balance
            elif acc.account_type == "5":
                balance_sheet["pasif"]["ozkaynaklar"][acc.code] = {
                    "name": acc.name,
                    "balance": balance,
                }
                balance_sheet["pasif"]["toplam_pasif"] += balance
    ...
    def generate_income_statement(self):
        ...
        for acc in accounts:
            ...
            if acc.account_type == "6":
                ...
                income_statement["gelirler"][acc.code] = {
                    "name": acc.name,
                    "balance": balance,
                }
                ...
                income_statement["giderler"][acc.code] = {
                    "name": acc.name,
                    "balance": balance,
                }
                income_statement["toplam_gider"] += balance

        income_statement["net_kar_zarar"] = (
            income_statement["toplam_gelir"] - income_statement["toplam_gider"]
        )
...
class TrialBalanceSnapshot(models.Model):
    ...
    company = models.ForeignKey(
        Company, on_delete=models.CASCADE, related_name="trial_balance_snapshots"
    )
    fiscal_period = models.ForeignKey(
        FiscalPeriod, on_delete=models.CASCADE, related_name="trial_balance_snapshots"
    )
    ...
    class Meta:
        ...
        ordering = ["-as_of_date"]
        indexes = [models.Index(fields=["company", "as_of_date"])]
    ...
    def build_snapshot(cls, company, fiscal_period, as_of_date=None, user=None):
        ...
        for row in entries:
            ...
            balance = (
                (debit - credit)
                if row["account__account_type"] in ["1", "2", "7", "8"]
                else (credit - debit)
            )
