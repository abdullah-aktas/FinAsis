...
class Invoice(BaseModel):
    ...
    invoice_number = models.CharField(
        max_length=50, unique=True, verbose_name=_("Fatura Numarası")
    )
    customer = models.ForeignKey(
        Customer, on_delete=models.PROTECT, verbose_name=_("Müşteri")
    )
...
class Budget(BaseModel):
    ...
    amount = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Bütçe Tutarı")
    )
...
class FinancialReport(BaseModel):
    ...
    type = models.CharField(
        max_length=20, choices=TYPE_CHOICES, verbose_name=_("Rapor Tipi")
    )
    start_date = models.DateField(verbose_name=_("Başlangıç Tarihi"))
    end_date = models.DateField(verbose_name=_("Bitiş Tarihi"))
...
class Tax(BaseModel):
    ...
    type = models.CharField(
        max_length=20, choices=TYPE_CHOICES, verbose_name=_("Vergi Tipi")
    )
    description = models.TextField(blank=True, verbose_name=_("Açıklama"))
...
class CashFlow(BaseModel):
    ...
    period = models.CharField(
        max_length=20, choices=PERIOD_CHOICES, verbose_name=_("Dönem")
    )
    start_date = models.DateField(verbose_name=_("Başlangıç Tarihi"))
    end_date = models.DateField(verbose_name=_("Bitiş Tarihi"))
    opening_balance = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name=_("Açılış Bakiyesi")
    )
    closing_balance = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name=_("Kapanış Bakiyesi")
    )
    total_income = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name=_("Toplam Gelir")
    )
    total_expense = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name=_("Toplam Gider")
    )
    net_cash_flow = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name=_("Net Nakit Akışı")
    )
...
class IncomeStatement(BaseModel):
    ...
    period = models.CharField(
        max_length=20, choices=PERIOD_CHOICES, verbose_name="Dönem"
    )
    ...
    cost_of_goods_sold = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name="Satılan Malın Maliyeti"
    )
    gross_profit = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name="Brüt Kar"
    )
    operating_expenses = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name="İşletme Giderleri"
    )
    operating_income = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name="İşletme Karı"
    )
    other_income = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name="Diğer Gelirler"
    )
    other_expenses = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name="Diğer Giderler"
    )
    net_income = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name="Net Kar"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        # Net kar hesaplama
        self.net_income = (
            self.operating_income + self.other_income - self.other_expenses
        )
...
class EInvoice(BaseModel):
    ...
    invoice_type = models.CharField(
        max_length=20, choices=INVOICE_TYPE_CHOICES, verbose_name=_("Fatura Tipi")
    )
    issue_date = models.DateField(verbose_name=_("Düzenleme Tarihi"))
    due_date = models.DateField(verbose_name=_("Vade Tarihi"))
    customer = models.ForeignKey(
    ...
    subtotal = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Ara Toplam")
    )
    tax_total = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Vergi Toplamı")
    )
    total = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Genel Toplam")
    )
    currency = models.CharField(
        max_length=3, default="TRY", verbose_name=_("Para Birimi")
    )
    ...
    sent_at = models.DateTimeField(
        null=True, blank=True, verbose_name=_("Gönderim Zamanı")
    )
    accepted_at = models.DateTimeField(
        null=True, blank=True, verbose_name=_("Kabul Zamanı")
    )
...
class EInvoiceItem(BaseModel):
    ...
    quantity = models.DecimalField(
        max_digits=12, decimal_places=3, verbose_name=_("Miktar")
    )
    unit = models.CharField(max_length=10, default="ADET", verbose_name=_("Birim"))
    unit_price = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Birim Fiyat")
    )
    tax_rate = models.DecimalField(
        max_digits=5, decimal_places=2, verbose_name=_("Vergi Oranı (%)")
    )
    tax_amount = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Vergi Tutarı")
    )
    line_total = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Satır Toplamı")
    )
    description = models.CharField(
        max_length=255, blank=True, verbose_name=_("Açıklama")
    )
...
class PaymentTerm(BaseModel):
    ...
    term_type = models.CharField(
        max_length=20, choices=TERM_TYPES, verbose_name=_("Vade Tipi")
    )
...
class RecurringTransaction(BaseModel):
    ...
    account = models.ForeignKey(
        Account, on_delete=models.PROTECT, verbose_name=_("Hesap")
    )
    category = models.ForeignKey(
        TransactionCategory, on_delete=models.PROTECT, verbose_name=_("Kategori")
    )

    # Tekrarlama ayarları
    frequency = models.CharField(
        max_length=20, choices=FREQUENCY_CHOICES, verbose_name=_("Sıklık")
    )
    start_date = models.DateField(verbose_name=_("Başlangıç Tarihi"))
...
class FinancialMetric(BaseModel):
    ...
    metric_type = models.CharField(
        max_length=20, choices=METRIC_TYPES, verbose_name=_("Metrik Tipi")
    )
    value = models.DecimalField(
        max_digits=15, decimal_places=4, verbose_name=_("Değer")
    )
    ...
    calculation_data = models.JSONField(
        default=dict, blank=True, verbose_name=_("Hesaplama Verisi")
    )
...
class AssetDepreciation(BaseModel):
    ...
    purchase_cost = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Satın Alma Maliyeti")
    )
    salvage_value = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=Decimal("0.00"),
        verbose_name=_("Hurda Değeri"),
    )
    ...
    annual_depreciation = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Yıllık Amortisman")
    )
    accumulated_depreciation = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=Decimal("0.00"),
        verbose_name=_("Birikmiş Amortisman"),
    )
    book_value = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Net Defter Değeri")
    )

    # Durum
    is_fully_depreciated = models.BooleanField(
        default=False, verbose_name=_("Tamamen Amortismana Tabi Tutuldu")
    )
    last_depreciation_date = models.DateField(
        null=True, blank=True, verbose_name=_("Son Amortisman Tarihi")
    )
...
class Loan(BaseModel):
    ...
    loan_type = models.CharField(
        max_length=20, choices=LOAN_TYPES, verbose_name=_("Kredi Tipi")
    )
    lender = models.CharField(max_length=200, verbose_name=_("Borç Veren"))

    # Kredi detayları
    principal_amount = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Ana Para")
    )
    interest_rate = models.DecimalField(
        max_digits=5, decimal_places=2, verbose_name=_("Faiz Oranı (%)")
    )
    ...
    payment_frequency = models.CharField(
        ...
        verbose_name=_("Ödeme Sıklığı"),
    )
    installment_amount = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Taksit Tutarı")
    )

    # Güncel durum
    outstanding_balance = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Kalan Bakiye")
    )
    total_paid = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=Decimal("0.00"),
        verbose_name=_("Toplam Ödenen"),
    )
    total_interest_paid = models.DecimalField(
        max_digits=15,
        decimal_places=2,
        default=Decimal("0.00"),
        verbose_name=_("Ödenen Faiz"),
    )

    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default="ACTIVE", verbose_name=_("Durum")
    )

    # İlişkili hesap
    liability_account = models.ForeignKey(
        Account, on_delete=models.PROTECT, verbose_name=_("Borç Hesabı")
    )
...
class Investment(BaseModel):
    ...
    investment_type = models.CharField(
        max_length=20, choices=INVESTMENT_TYPES, verbose_name=_("Yatırım Tipi")
    )

    # Yatırım detayları
    purchase_date = models.DateField(verbose_name=_("Alım Tarihi"))
    purchase_price = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Alım Fiyatı")
    )
    quantity = models.DecimalField(
        max_digits=15,
        decimal_places=4,
    ...
    current_price = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Güncel Fiyat")
    )

    # Hesaplanan değerler
    total_cost = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Toplam Maliyet")
    )
    current_value = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Güncel Değer")
    )
    unrealized_gain_loss = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Gerçekleşmemiş Kar/Zarar")
    )
    return_percentage = models.DecimalField(
        max_digits=8, decimal_places=2, verbose_name=_("Getiri (%)")
    )

    # İlişkili hesap
    investment_account = models.ForeignKey(
        Account, on_delete=models.PROTECT, verbose_name=_("Yatırım Hesabı")
    )
    ...
    realized_gain_loss = models.DecimalField(
        ...
        null=True,
        blank=True,
        verbose_name=_("Gerçekleşen Kar/Zarar"),
    )
...
class Reconciliation(BaseModel):
    ...
    reconciliation_type = models.CharField(
        max_length=20, choices=RECONCILIATION_TYPES, verbose_name=_("Mutabakat Tipi")
    )
    account = models.ForeignKey(
        Account,
        null=True,
    ...
    system_balance = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Sistem Bakiyesi")
    )
    statement_balance = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Ekstre Bakiyesi")
    )
    difference = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name=_("Fark")
    )
    ...
    reconciliation_data = models.JSONField(
        default=dict, blank=True, verbose_name=_("Mutabakat Verisi")
    )
    notes = models.TextField(blank=True, verbose_name=_("Notlar"))
    ...
    reconciled_by = models.ForeignKey(
        ...
        verbose_name=_("Mutabakat Yapan"),
    )
