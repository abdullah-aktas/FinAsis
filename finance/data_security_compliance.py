...
try:
    ...
    class Fernet:  # minimal stub fallback
        def __init__(self, *args, **kwargs):
            raise ImportError(
                "cryptography paketi yüklü değil: pip install cryptography"
            )
...
class PersonalDataCategory(models.Model):
    ...
    company = models.ForeignKey(
        Company, on_delete=models.CASCADE, related_name="data_categories"
    )
    ...
    sensitivity_level = models.CharField(
        _("Hassasiyet Seviyesi"), max_length=20, choices=SENSITIVITY_LEVELS
    )

    # KVKK gereksinimleri
    processing_purpose = models.CharField(
        _("İşleme Amacı"), max_length=20, choices=PROCESSING_PURPOSES
    )
    ...
    # Saklama süresi
    retention_period_months = models.PositiveIntegerField(
        _("Saklama Süresi (Ay)"), help_text="0 = Süresiz"
    )
    deletion_required = models.BooleanField(_("Otomatik Silme Gerekli"), default=True)

    # Veri sahipleri bilgilendirildi mi?
    data_subjects_informed = models.BooleanField(
        _("Veri Sahipleri Bilgilendirildi"), default=False
    )
    information_method = models.CharField(
        _("Bilgilendirme Yöntemi"),
        max_length=100,
        blank=True,
...
class PersonalDataRecord(models.Model):
    ...
    company = models.ForeignKey(
        Company, on_delete=models.CASCADE, related_name="personal_data_records"
    )
    data_category = models.ForeignKey(
        PersonalDataCategory,
        on_delete=models.PROTECT,
        related_name="data_records",
        verbose_name=_("Veri Kategorisi"),
    )
    ...
    data_subject_type = models.CharField(
        _("Veri Sahibi Tipi"), max_length=50, help_text="customer, employee, vendor vb."
    )

    # İşleme faaliyeti
    processing_activity = models.CharField(
        _("İşleme Faaliyeti"), max_length=20, choices=PROCESSING_ACTIVITIES
    )
    ...
    # İşleme detayları
    data_fields_processed = models.JSONField(
        _("İşlenen Veri Alanları"),
        default=list,
    ...
    scheduled_deletion_date = models.DateField(
        _("Planlanan Silme Tarihi"), null=True, blank=True
    )
...
class DataSubjectRequest(models.Model):
    ...
    company = models.ForeignKey(
        Company, on_delete=models.CASCADE, related_name="data_subject_requests"
    )

    # Talep bilgileri
    request_id = models.UUIDField(_("Talep ID"), default=uuid.uuid4, unique=True)
    request_type = models.CharField(
        _("Talep Tipi"), max_length=20, choices=REQUEST_TYPES
    )
    ...
    verification_method = models.CharField(
        _("Doğrulama Yöntemi"), max_length=100, blank=True
    )

    # Talep detayları
    request_description = models.TextField(_("Talep Açıklaması"))
    specific_data_requested = models.JSONField(
        _("Talep Edilen Belirli Veriler"), default=list, blank=True
    )
    preferred_response_method = models.CharField(
        _("Tercih Edilen Yanıt Yöntemi"),
        max_length=50,
        choices=[
            ("EMAIL", _("E-posta")),
            ("POST", _("Posta")),
    ...
    status = models.CharField(
        _("Durum"), max_length=30, choices=STATUS_CHOICES, default="RECEIVED"
    )
    ...
    supporting_documents = models.JSONField(
        _("Destekleyici Belgeler"), default=list, blank=True
    )
...
class EncryptionKey(models.Model):
    ...
    company = models.ForeignKey(
        Company, on_delete=models.CASCADE, related_name="encryption_keys"
    )
    ...
    expiration_date = models.DateTimeField(
        _("Son Kullanma Tarihi"), null=True, blank=True
    )
    ...
    last_used_date = models.DateTimeField(
        _("Son Kullanım Tarihi"), null=True, blank=True
    )
...
class DataEncryption(models.Model):
    ...
    company = models.ForeignKey(
        Company, on_delete=models.CASCADE, related_name="data_encryptions"
    )
    ...
    # Durum
    is_active = models.BooleanField(_("Aktif"), default=True)
    decryption_date = models.DateTimeField(
        _("Şifre Çözme Tarihi"), null=True, blank=True
    )
...
class DataBackup(models.Model):
    ...
    company = models.ForeignKey(
        Company, on_delete=models.CASCADE, related_name="data_backups"
    )
    ...
    excluded_tables = models.JSONField(
        _("Hariç Tutulan Tablolar"), default=list, blank=True
    )
    ...
    # Durum ve süre
    status = models.CharField(
        _("Durum"), max_length=20, choices=BACKUP_STATUS, default="IN_PROGRESS"
    )
    ...
    duration_seconds = models.PositiveIntegerField(
        _("Süre (Saniye)"), null=True, blank=True
    )

    # Saklama bilgileri
    retention_period_days = models.PositiveIntegerField(
        _("Saklama Süresi (Gün)"), default=365
    )
    ...
    last_verification_date = models.DateTimeField(
        _("Son Doğrulama Tarihi"), null=True, blank=True
    )
    ...
    def calculate_deletion_date(self):
        ...
        if self.backup_date:
            self.scheduled_deletion_date = self.backup_date + timedelta(
    ...
    def verify_integrity(self):
        ...
        if current_hash == self.file_hash:
            ...
            self.verification_status = "VERIFICATION_FAILED"
            self.error_message = (
                "Dosya hash değeri eşleşmiyor - dosya bozulmuş olabilir"
            )
...
class SecurityIncident(models.Model):
    ...
    incident_type = models.CharField(
        _("Olay Tipi"), max_length=30, choices=INCIDENT_TYPES
    )
    ...
    estimated_records_affected = models.PositiveIntegerField(
        _("Etkilenen Kayıt Sayısı (Tahmini)"), null=True, blank=True
    )

    # Veri ihlali detayları (KVKK Uyumu)
    is_personal_data_involved = models.BooleanField(
        _("Kişisel Veri İçeriyor"), default=False
    )
    personal_data_categories = models.JSONField(
        _("Etkilenen Kişisel Veri Kategorileri"), default=list, blank=True
    )
    data_subjects_affected = models.PositiveIntegerField(
        _("Etkilenen Veri Sahibi Sayısı"), null=True, blank=True
    )

    # Bildirimi gerekli mi? (KVKK m.12: 72 saat içinde VVK'ya bildirim)
    kvkk_notification_required = models.BooleanField(
        _("KVKK Bildirimi Gerekli"), default=False
    )
    kvkk_notification_sent = models.BooleanField(
        _("KVKK Bildirimi Gönderildi"), default=False
    )
    kvkk_notification_date = models.DateTimeField(
        _("KVKK Bildirim Tarihi"), null=True, blank=True
    )

    # Veri sahipleri bilgilendirildi mi?
    data_subjects_notification_required = models.BooleanField(
        _("Veri Sahipleri Bildirimi Gerekli"), default=False
    )
    data_subjects_notification_sent = models.BooleanField(
        _("Veri Sahipleri Bildirimi Gönderildi"), default=False
    )
    data_subjects_notification_date = models.DateTimeField(
        _("Veri Sahipleri Bildirim Tarihi"), null=True, blank=True
    )

    # Durum takibi
    status = models.CharField(
        _("Durum"), max_length=20, choices=STATUS_CHOICES, default="OPEN"
    )
    assigned_to = models.ForeignKey(
        dj_settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
    ...
    resolution_time_hours = models.DecimalField(
        _("Çözüm Süresi (Saat)"), max_digits=8, decimal_places=2, null=True, blank=True
    )

    # Mali etki
    estimated_cost = models.DecimalField(
        _("Tahmini Maliyet"), max_digits=15, decimal_places=2, null=True, blank=True
    )
    actual_cost = models.DecimalField(
        _("Gerçek Maliyet"), max_digits=15, decimal_places=2, null=True, blank=True
    )
    ...
    reported_by = models.ForeignKey(
        ...
        verbose_name=_("Raporlayan"),
    )
    external_authorities_notified = models.JSONField(
        _("Bilgilendirilen Dış Otoriteler"), default=list, blank=True
    )
...
class GDPRComplianceChecker:
    ...
    def generate_compliance_report(self):
        ...
        overall_score = self._calculate_compliance_score(
            ...
            consent_check,
            requests_check,
            overdue_kvkk_notifications.count(),
        )
        ...
        return {
            ...
            "recommendations": self._generate_recommendations(
                retention_check, consent_check, requests_check
            ),
        }
    ...
    def _calculate_average_response_time(self):
        ...
        completed_requests = DataSubjectRequest.objects.filter(
            company=self.company, response_date__isnull=False
        )
    ...
    def _calculate_compliance_score(
        self, retention_check, consent_check, requests_check, overdue_notifications
    ):
