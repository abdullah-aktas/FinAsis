...
from .models import (
    Plan, Price, Module, PlanModule, SubscriptionProfile, Transaction, 
    BankTransfer, EnterpriseInquiry, Invoice, PaymentGateway, Discount,
    DiscountUsage, PaymentAttempt
)
...
class InvoiceAdmin(admin.ModelAdmin):
    list_display = ('invoice_number', 'subscription', 'invoice_date', 'due_date', 'total_amount', 'status', 'paid_at')
...
class PaymentGatewayAdmin(admin.ModelAdmin):
    list_display = ('name', 'gateway_type', 'is_test_mode', 'is_active', 'total_transactions', 'successful_transactions', 'failed_transactions')
    search_fields = ('name', 'merchant_id')
    list_filter = ('gateway_type', 'is_test_mode', 'is_active')
    readonly_fields = ('total_transactions', 'successful_transactions', 'failed_transactions', 'created_at', 'updated_at')


@admin.register(Discount)
class DiscountAdmin(admin.ModelAdmin):
    list_display = ('code', 'name', 'discount_type', 'discount_value', 'valid_from', 'valid_until', 'times_used', 'is_active')
...
class DiscountUsageAdmin(admin.ModelAdmin):
    list_display = ('discount', 'user', 'original_amount', 'discount_amount', 'final_amount', 'used_at')
...
class PaymentAttemptAdmin(admin.ModelAdmin):
    list_display = ('transaction', 'attempt_number', 'status', 'gateway', 'response_code', 'attempted_at')
