...
from billing.models import Plan, Price, PlanGroup
    ...
    def handle(self, *args, **options):
        ...
        for plan in Plan.objects.all():
            ...
            if not prices.exists():
                ok = False
                self.stdout.write(
                    self.style.ERROR(f"Plan {plan.code} has no active prices")
                )
            if not PlanGroup.objects.filter(plan=plan).exists():
                ok = False
                self.stdout.write(
                    self.style.ERROR(f"Plan {plan.code} has no PlanGroup mapping")
                )
            ...
            if not st:
                self.stdout.write(
                    self.style.WARNING(
                        f"No matching SubscriptionType for plan code {plan.code}"
                    )
                )
        if ok:
