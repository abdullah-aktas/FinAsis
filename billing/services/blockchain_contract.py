...
class SubscriptionBlockchainService:
    ...
    def should_create_contract(subscription_profile):
        ...
        try:
            ...
            if (
                monthly_price
                and monthly_price.amount
                >= SubscriptionBlockchainService.BLOCKCHAIN_THRESHOLD
            ):
                return True

            # Yıllık fiyat kontrolü
            yearly_price = subscription_profile.plan.prices.filter(
                period="year", is_active=True
            ).first()

            if (
                yearly_price
                and yearly_price.amount
                >= SubscriptionBlockchainService.BLOCKCHAIN_THRESHOLD
            ):
                return True
    ...
    def create_subscription_contract(subscription_profile, transaction=None):
        ...
        if not SubscriptionBlockchainService.should_create_contract(
        ...
        try:
            ...
            contract_terms = {
                ...
                "pricing": {
                    "base_amount": str(
                        plan.prices.filter(is_active=True).first().amount
                        if plan.prices.filter(is_active=True).exists()
                        else 0
                    ),
                    "currency": "TRY",
                    "beta_discount": str(plan.beta_discount_percent)
                    if plan.is_beta_plan
                    else "0",
                },
                "beta_membership": {
    ...
    def get_user_contracts(user):
        ...
        contracts = SmartContract.objects.filter(
            deployed_by=user, contract_type="subscription", is_active=True
