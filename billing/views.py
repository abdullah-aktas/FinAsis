...
from typing import Optional, cast, Iterable, Dict, List, Tuple
...
from accounting.models import Invoice, Customer, Company
...
def _build_region_context(request: HttpRequest) -> Tuple[str, List[Dict[str, str]], Dict[str, object]]:
    region = resolve_region(request)
    supported = get_supported_regions()
    labels = get_region_labels()
    region_options = [{"code": code, "label": labels.get(code, code)} for code in supported]
...
def _prepare_plan_display(plans_list: List[Plan], region: str) -> Tuple[List[Dict[str, object]], Dict[str, Dict[str, object]], List[Dict[str, object]]]:
    ...
    plan_cards = [{"plan": plan, "card": card_map.get(plan.code)} for plan in plans_list]
...
def plans_kobi(request: HttpRequest) -> HttpResponse:
    ...
    price_qs = Price.objects.filter(is_active=True).order_by('period')
    qs = (
        Plan.objects.filter(is_active=True, audience='sme')
        .prefetch_related(
            Prefetch('prices', queryset=price_qs),
            'plan_modules__module'
        )
    ...
    return render(request, 'billing/plans_kobi.html', {
        'plans': plans_list,
...
def plans_education(request: HttpRequest) -> HttpResponse:
    ...
    price_qs = Price.objects.filter(is_active=True).order_by('period')
    qs = (
        Plan.objects.filter(is_active=True, audience='edu')
        .prefetch_related(
            Prefetch('prices', queryset=price_qs),
            'plan_modules__module'
        )
    ...
    return render(request, 'billing/plans_education.html', {
        'plans': plans_list,
        'plan_cards': plan_cards,
        'period': period,
        'feature_rows': feature_rows,
...
def plans_games(request: HttpRequest) -> HttpResponse:
    ...
    price_qs = Price.objects.filter(is_active=True).order_by('period')
    qs = (
        Plan.objects.filter(is_active=True, audience='games')
        .prefetch_related(
            Prefetch('prices', queryset=price_qs),
            'plan_modules__module'
        )
    ...
    return render(request, 'billing/plans_games.html', {
        'plans': plans_list,
        'plan_cards': plan_cards,
        'period': period,
        'feature_rows': feature_rows,
...
def plans_by_category(request: HttpRequest, audience: str, category_name: str) -> HttpResponse:
    ...
    if audience == 'sme':
        ...
        games_map = {code: 20 + i + 1 for i, code in enumerate(order_cfg.get('games', []))}
    ...
    module_to_plans = {row['name']: set(row.get('included', [])) for row in feature_rows}
    ...
    region_currency = region_config.get('currency', getattr(settings, 'BASE_PRICING_CURRENCY', 'TRY'))
    region_tax_summary = None
    if 'vat_rate' in region_config:
        try:
            rate = Decimal(str(region_config['vat_rate'])) * Decimal('100')
            region_tax_summary = _("KDV oranı %(rate)s%%") % {'rate': rate.quantize(Decimal('0.01'))}
        ...
        try:
            ...
            region_tax_summary = _("GST oranı %(rate)s%%") % {'rate': rate.quantize(Decimal('0.01'))}
        except (InvalidOperation, ValueError):
            region_tax_summary = _("GST uygulanır.")
    elif region_config.get('sales_tax'):
        region_tax_summary = _("Satış vergisi: %(note)s") % {'note': region_config['sales_tax']}
    return render(request, 'billing/plans_category.html', {
        'plans': plans_list,
        'plan_cards': plan_cards,
