...
def role_assignment_view(request):
    ...
    recent_users = User.objects.select_related('user_type').prefetch_related('groups').order_by('-date_joined')[:10]
...
def assign_single_user(request, user_id):
    ...
    try:
        ...
        if result['success']:
            messages.success(request, 
                f'{user.username} kullanıcısına rol atama başarılı!')
            return JsonResponse({'success': True, 'result': result})
        else:
...
admin_url_patterns = [
    ...
    path('role-assignment/assign-all/', assign_all_users, name='role_assignment_assign_all'),
    path('role-assignment/assign-user/<int:user_id>/', assign_single_user, name='role_assignment_assign_user'),
    path('role-assignment/create-groups/', create_groups, name='role_assignment_create_groups'),
]