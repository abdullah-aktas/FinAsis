...
def users_without_groups(request):
    ...
    try:
        ...
        return Response(
            {"success": True, "data": {"count": len(user_data), "users": user_data}}
        )
    except Exception as e:
        return Response(
            {"success": False, "error": str(e)},
...
def group_statistics(request):
    ...
    try:
        ...
        for group in Group.objects.all():
            users_in_group = User.objects.filter(groups=group).select_related(
                "user_type"
            )
        ...
        return Response(
            {
                "success": True,
                "data": {"total_groups": len(groups_data), "groups": groups_data},
            }
        )
    except Exception as e:
        return Response(
