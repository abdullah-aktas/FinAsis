...
def setup_company_cache_invalidation(Company):
    ...
    def invalidate_company_cache(sender, instance, created, **kwargs):
        patterns = [
            ...
            f'user_{instance.owner_id}_companies' if hasattr(instance, 'owner_id') else None,
            'companies_list',
            'companies_stats',
        ]
...
def setup_accounting_cache_invalidation(GLAccount, GLJournal):
    ...
    def invalidate_glaccount_cache(sender, instance, **kwargs):
        patterns = [
            ...
            f'company_{instance.company_id}_accounts' if hasattr(instance, 'company_id') else None,
            'chart_of_accounts',
        ]
    ...
    def invalidate_journal_cache(sender, instance, **kwargs):
        patterns = [
            ...
            f'company_{instance.company_id}_journals' if hasattr(instance, 'company_id') else None,
...
def setup_edoc_cache_invalidation(EInvoice, ELedger):
    ...
    def invalidate_einvoice_cache(sender, instance, **kwargs):
        patterns = [
            ...
            f'company_{instance.company_id}_einvoices' if hasattr(instance, 'company_id') else None,
            'einvoices_pending',
            'einvoices_stats',
        ]
        patterns = [p for p in patterns if p]
    ...
    def invalidate_eledger_cache(sender, instance, **kwargs):
        patterns = [
            ...
            f'company_{instance.company_id}_eledgers' if hasattr(instance, 'company_id') else None,
            'eledgers_stats',
        ]
        patterns = [p for p in patterns if p]
        invalidate_cache(*patterns)
