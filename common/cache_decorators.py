...
def cache_view(timeout: int = 300, key_prefix: str = '', vary_on: Optional[List[str]] = None):
    """
...
def invalidate_cache(*patterns: str):
    ...
    try:
        ...
        for pattern in patterns:
            full_pattern = f"finasis:{pattern}" if not pattern.startswith('finasis:') else pattern
            for key in list(redis_conn.scan_iter(match=full_pattern)):
                redis_conn.delete(key)
                deleted_count += 1
        logger.info(f"Invalidated {deleted_count} cache keys matching patterns: {patterns}")
                ...
                if isinstance(internal, dict):
                    ...
                    for key in keys:
                        ...
                        if k.startswith(internal_prefix):
                            ...
                            deleted_total += 1
            logger.info(f"[fallback] Invalidated ~{deleted_total} cache keys matching patterns: {patterns}")
...
def cache_on_signal(signal, sender, timeout: int = 300, invalidate_on: Optional[List[str]] = None):
    """
