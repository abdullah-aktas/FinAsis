...
ADMIN_USER_RULES = {
    'superuser': {
        'groups': ['Süper Yöneticiler', 'Sistem Yöneticileri', 'Finans Yöneticisi', 'Eğitim Yöneticisi', 'Rapor Görüntüleyici'],
        'custom_role': 'admin',
...
def assign_roles_to_user(user, force=False):
    ...
    try:
        ...
        if user.is_superuser:
            ...
            logger.info(f"SuperUser {user.username} için roller atandı: {assigned_groups}")
        ...
        # 3. UserType bazlı rol atama
            ...
            if user_type_code in AUTO_ROLE_RULES:
                ...
                assigned_custom_role = rule['custom_role']
                assigned_user_role = rule.get('user_role')
                logger.info(f"UserType {user_type_code} için {user.username} kullanıcısına roller atandı: {assigned_groups}")
        ...
        # 4. Email domain bazlı rol atama (fallback)
            ...
            if email_domain and email_domain in EMAIL_DOMAIN_RULES:
                ...
                assigned_custom_role = rule['custom_role']
                assigned_user_role = rule.get('user_role')
                logger.info(f"Email domain {email_domain} için {user.username} kullanıcısına roller atandı: {assigned_groups}")
            ...
            # Varsayılan rol (hiçbiri uymazsa)
                ...
                logger.info(f"Varsayılan rol için {user.username} kullanıcısına roller atandı: {assigned_groups}")
        
        # CustomUser.role güncelle
        if assigned_custom_role and (force or not user.role):
            user.role = assigned_custom_role
            user.save(update_fields=['role'])
            logger.info(f"Kullanıcı {user.username} rolü güncellendi: {assigned_custom_role}")
...
def bulk_assign_roles(users=None, force=False):
    ...
    logger.info(f"Toplu rol atama tamamlandı - Başarılı: {success_count}, Hata: {error_count}")
...
def auto_assign_roles_on_user_create(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        logger.info(f"Yeni kullanıcı {instance.username} için otomatik rol atama tamamlandı")
...
def auto_assign_roles_on_user_update(sender, instance, created, **kwargs):
    ...
    if not created:
        ...
        if hasattr(instance, '_state') and instance._state.adding is False:
            ...
            if (hasattr(instance, 'user_type') or 
                instance.is_staff or 
                instance.is_superuser):
                assign_roles_to_user(instance, force=False)
                logger.info(f"Kullanıcı {instance.username} güncellemesi için rol kontrolü yapıldı")
...
def get_role_assignment_summary():
    ...
    return {
        ...
        'coverage_percentage': round((users_with_groups / total_users * 100) if total_users > 0 else 0, 2)
    }