...
class ErrorLogAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("System Info", {"fields": ("system_info",)}),
        (
            "Occurrence",
            {
                "fields": (
    ...
    def last_seen_relative(self, obj):
        ...
        if diff.total_seconds() < 60:
            return format_html(
                '<span style="color: #dc2626; font-weight: bold;">Just now</span>'
            )
        elif diff.total_seconds() < 3600:
            minutes = int(diff.total_seconds() / 60)
            return format_html(
                '<span style="color: #f59e0b;">{} min ago</span>', minutes
            )
    ...
    def resend_notification(self, request, queryset):
        ...
        self.message_user(
            request, f"Notifications resent for {queryset.count()} error(s)."
        )
