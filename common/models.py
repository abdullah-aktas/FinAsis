...
class ApprovalRequest(models.Model):
    ...
    content_type = models.ForeignKey(
        ContentType, on_delete=models.CASCADE, related_name="common_approval_requests"
    )
    object_id = models.CharField(max_length=64)
    content_object = GenericForeignKey("content_type", "object_id")
...
class SystemSetting(models.Model):
    ...
    value_type = models.CharField(
        max_length=20, choices=SETTING_TYPES, default="string", verbose_name="Tip"
    )
...
class FileUpload(models.Model):
    ...
    original_filename = models.CharField(
        max_length=255, verbose_name="Orijinal Dosya Adı"
    )
    ...
    category = models.CharField(
        max_length=20, choices=FILE_CATEGORIES, default="other", verbose_name="Kategori"
    )
...
class EmailLog(models.Model):
    ...
    sent_at = models.DateTimeField(
        null=True, blank=True, verbose_name="Gönderim Zamanı"
    )
    opened_at = models.DateTimeField(
        null=True, blank=True, verbose_name="Açılma Zamanı"
    )
    clicked_at = models.DateTimeField(
        null=True, blank=True, verbose_name="Tıklama Zamanı"
    )
...
class APIKey(models.Model):
    ...
    allowed_ips = models.TextField(
        blank=True, verbose_name="İzin Verilen IP'ler", help_text="Her satırda bir IP"
    )
...
class Webhook(models.Model):
    ...
    event_type = models.CharField(
        max_length=50, choices=EVENT_TYPES, verbose_name="Event Tipi"
    )
    ...
    last_called_at = models.DateTimeField(
        null=True, blank=True, verbose_name="Son Çağrı"
    )
...
class WebhookLog(models.Model):
    ...
    webhook = models.ForeignKey(
        Webhook, on_delete=models.CASCADE, related_name="logs", verbose_name="Webhook"
    )
    ...
    response_status = models.IntegerField(
        null=True, blank=True, verbose_name="Durum Kodu"
    )
    response_body = models.TextField(blank=True, verbose_name="Yanıt")
    response_time_ms = models.IntegerField(
        null=True, blank=True, verbose_name="Yanıt Süresi (ms)"
    )
...
class ScheduledTask(models.Model):
    ...
    task_type = models.CharField(
        max_length=20, choices=TASK_TYPES, verbose_name="Görev Tipi"
    )
    ...
    execution_time_ms = models.IntegerField(
        null=True, blank=True, verbose_name="Çalışma Süresi (ms)"
    )
...
class SupportTicket(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default="open", verbose_name="Durum"
    )
    ...
    resolved_at = models.DateTimeField(
        null=True, blank=True, verbose_name="Çözüm Tarihi"
    )
