...
"""
    ...
    def notify_payment_received(payment, user):
        ...
        action_url = (
            reverse("accounting:payment_detail", args=[payment.id])
    ...
    def notify_expense_created(expense, user):
        ...
        action_url = (
            reverse("accounting:expense_detail", args=[expense.id])
    ...
    def notify_financial_report_generated(report, user):
        ...
        action_url = (
            reverse("finance:report_detail", args=[report.id])
    ...
    def notify_low_balance(bank_account, user, threshold):
        ...
        action_url = (
            reverse("finance:bank_account_detail", args=[bank_account.id])
            if hasattr(bank_account, "id")
            else ""
        )
    ...
    def notify_invoice_due_soon(invoice, user, days_until_due: int):
        ...
        action_url = (
            f"/accounting/invoice/{invoice.id}/"
            if hasattr(invoice, "id")
            else "/accounting/invoices/"
        )
    ...
    def notify_low_stock(product, user, current_stock: int, min_stock: int):
        ...
        action_url = (
            f"/accounting/product/{product.id}/"
            if hasattr(product, "id")
            else "/accounting/products/"
        )
    ...
    def notify_customer_added(customer, user):
        ...
        action_url = (
            f"/accounting/customer/{customer.id}/"
            if hasattr(customer, "id")
            else "/accounting/customers/"
        )
