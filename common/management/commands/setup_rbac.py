...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS("FinAsis RBAC (Role-Based Access Control) Kurulumu")
        )
        self.stdout.write(self.style.SUCCESS("=" * 80))
        ...
        self.stdout.write(
            self.style.SUCCESS(f"   OK: {created_count} yeni grup olusturuldu")
        )
            ...
            try:
                ...
                if not ct:
                    if verbose:
                        self.stdout.write(
                            self.style.WARNING(
                                f"      WARNING: ContentType bulunamadi: {app_name}"
                            )
                        )
                    continue
                    ...
                    for role in roles:
                        ...
                        for group_name in ROLE_CATEGORIES[role]["groups"]:
                            try:
                                ...
                                if verbose:
                                    self.stdout.write(
                                        f"      OK: {group_name} -> {perm_name}"
                                    )
                            except Group.DoesNotExist:
                                if verbose:
                                    self.stdout.write(
                                        self.style.WARNING(
                                            f"      WARNING: Grup bulunamadi: {group_name}"
                                        )
                                    )
    ...
    def _assign_roles_to_users(self, verbose):
        ...
        for user in users:
            try:
                ...
                if verbose:
                    self.stdout.write(
                        self.style.ERROR(f"   ERROR: {user.username}: {e}")
                    )

        self.stdout.write(
            self.style.SUCCESS(f"   OK: {success_count} kullaniciya rol atandi")
        )
        if error_count > 0:
            self.stdout.write(
                self.style.WARNING(f"   WARNING: {error_count} kullanicida hata olustu")
            )
    ...
    def _print_summary(self):
        ...
        if users_without_groups > 0:
            self.stdout.write(
                self.style.WARNING(f"   WARNING: Grupsuz: {users_without_groups}")
            )
        ...
        for role_name, role_info in ROLE_CATEGORIES.items():
            self.stdout.write(
                f'   - {role_name} (Level {role_info["level"]}) - {role_info["description"]}'
            )
