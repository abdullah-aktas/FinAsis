...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i, error in enumerate(top_errors, 1):
            severity_color = self.style.ERROR if error.severity == 'CRITICAL' else self.style.WARNING
            self.stdout.write(
        ...
        if critical_errors.exists():
            ...
            for i, error in enumerate(critical_errors, 1):
                ...
                self.stdout.write(f'   User: {error.user.email if error.user else "Anonymous"}')
        
        # Unnotified errors
        unnotified = errors.filter(admin_notified=False, severity__in=['CRITICAL', 'ERROR'])
        if unnotified.exists():
            self.stdout.write(self.style.WARNING('\n' + '=' * 80))
            self.stdout.write(self.style.WARNING(f'⚠️  {unnotified.count()} error(s) not yet notified to admins'))
            self.stdout.write(self.style.WARNING('=' * 80))
