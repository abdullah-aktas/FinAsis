...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["user"]:
            self.assign_single_user(
                options["user"], options["force"], options["dry_run"]
            )
            return

        if options["user_type"]:
            self.assign_by_user_type(
                options["user_type"], options["force"], options["dry_run"]
            )
            return
    ...
    def show_summary(self):
        ...
        if summary["group_stats"]:
            ...
            for group_stat in summary["group_stats"]:
                self.stdout.write(
                    f"  • {group_stat['name']}: {group_stat['user_count']} kullanıcı"
                )
    ...
    def assign_single_user(self, username, force=False, dry_run=False):
        ...
        if dry_run:
            ...
            self.stdout.write(
                f"Mevcut gruplar: {list(user.groups.values_list('name', flat=True))}"
            )
            user_type_obj = getattr(user, "user_type", None)
            self.stdout.write(
                f"UserType: {user_type_obj.code if user_type_obj else 'None'}"
            )
        ...
        if result["success"]:
            self.stdout.write(self.style.SUCCESS("✅ Rol atama başarılı:"))
            self.stdout.write(f"  👤 Kullanıcı: {result['username']}")
            self.stdout.write(f"  🎭 Atanan rol: {result['assigned_role']}")
            self.stdout.write(
                f"  🏷️  Atanan gruplar: {', '.join(result['assigned_groups'])}"
            )
        else:
            self.stdout.write(
    ...
    def assign_by_user_type(self, user_type_code, force=False, dry_run=False):
        ...
        if not users.exists():
            self.stdout.write(
                self.style.WARNING(
                    f"⚠️  {user_type_code} user_type'ına sahip kullanıcı bulunamadı"
                )
            )
            return
    ...
    def print_help(self, prog_name, subcommand):
        ...
        examples = [
            ...
            (
                "Tüm kullanıcılara rol ata",
                "python manage.py auto_assign_roles --assign",
            ),
            (
                "Tüm rolleri sıfırla ve yeniden ata",
                "python manage.py auto_assign_roles --assign --force",
            ),
            (
                "Belirli kullanıcıya rol ata",
                "python manage.py auto_assign_roles --user admin",
            ),
            (
                "UserType bazlı rol ata",
                "python manage.py auto_assign_roles --user-type kobi_owner",
            ),
            (
                "Dry run (simülasyon)",
                "python manage.py auto_assign_roles --assign --dry-run",
            ),
        ]
