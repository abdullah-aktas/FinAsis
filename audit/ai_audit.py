...
class AuditAIAnalyzer:
            ...
            anomalies.append(
                {
                    ...
                    "risk_score": min(
                        100,
                        int(
                            (float(event.financial_impact) / float(threshold_high)) * 80
                        ),
                    ),
                    "details": {
    ...
    def _detect_pattern_anomalies(cls, events) -> List[Dict]:
        ...
        failed_logins = events.filter(
            action="login", severity__in=["high", "critical"]
    ...
    def _detect_geographic_anomalies(cls, events) -> List[Dict]:
        ...
        user_ips = (
            events.filter(ip__isnull=False)
            .values("actor_username")
            .annotate(unique_ips=Count("ip", distinct=True))
            ...
            anomalies.append(
                {
                    ...
                    "details": {"unique_ip_count": item["unique_ips"]},
                }
            )
    ...
    def calculate_risk_score(cls, entity_data: Dict) -> Tuple[int, Dict]:
        ...
        # 1. Sıklık Riski
        frequency_score = cls._calculate_frequency_risk(
            entity_data.get("transaction_count", 0)
        )
        scores["frequency"] = frequency_score

        # 2. Ciddiyet Riski
        severity_score = cls._calculate_severity_risk(
            entity_data.get("high_severity_count", 0)
        )
        scores["severity"] = severity_score

        # 3. Finansal Risk
        financial_score = cls._calculate_financial_risk(
            entity_data.get("total_financial_impact", 0)
        )
    ...
    def generate_recommendations(
        cls, anomalies: List[Dict], risk_score: int
    ) -> List[Dict]:
        """
