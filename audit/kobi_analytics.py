...
class KOBIAuditAnalytics:
    ...
    @classmethod
    def get_kobi_health_score(cls, company, audit_events, financial_data: Dict = None) -> Dict:
        """
        ...
        return {
            ...
            'recommendations': cls._generate_health_recommendations(scores, total_score)
        }
    ...
    def _calculate_compliance_score(cls, events) -> float:
        ...
        critical_compliance = compliance_events.filter(severity__in=['high', 'critical'])
    ...
    def _calculate_risk_impact(cls, events) -> float:
        ...
        # Finansal etki
        total_financial_impact = events.aggregate(
    ...
    def _generate_health_recommendations(cls, scores: Dict, total: float) -> List[str]:
        ...
        if scores.get('audit_quality', 0) < 15:
            recommendations.append("📋 Audit kayıtlarınızı düzenli tutun ve detaylandırın")
        
        if scores.get('financial_health', 0) < 15:
            recommendations.append("💰 Finansal göstergelerinizi iyileştirin, nakit akışını optimize edin")
        
        if scores.get('compliance', 0) < 15:
            recommendations.append("⚖️ Uyumluluk sorunlarını acilen çözün, yasal danışman desteği alın")
        
        if scores.get('operational_efficiency', 0) < 10:
            recommendations.append("⚙️ Operasyonel süreçlerinizi gözden geçirin, otomasyonu artırın")
        
        if scores.get('risk_management', 0) < 10:
            recommendations.append("🛡️ Risk yönetimi stratejinizi güçlendirin, sigorta değerlendirin")
    ...
    @classmethod
    def get_sector_benchmarking(cls, company, audit_events, sector: str = 'genel') -> Dict:
        """
        ...
        return {
            ...
            'comparison': {
                'audit_quality': {
                    ...
                    'status': 'üstünde' if company_audit > sector_avg['audit_quality'] else 'altında'
                },
                'compliance': {
                    'company': round(company_compliance, 1),
                    'sector_avg': sector_avg['compliance_score'],
                    'difference': round(company_compliance - sector_avg['compliance_score'], 1),
                    'status': 'üstünde' if company_compliance > sector_avg['compliance_score'] else 'altında'
                },
                'risk_management': {
                    'company': round(100 - company_risk, 1),
                    'sector_avg': 100 - sector_avg['risk_score'],
                    'difference': round((100 - company_risk) - (100 - sector_avg['risk_score']), 1),
                    'status': 'üstünde' if company_risk < sector_avg['risk_score'] else 'altında'
                },
                'operational_efficiency': {
                    'company': round(company_operational, 1),
                    'sector_avg': sector_avg['operational_efficiency'],
                    'difference': round(company_operational - sector_avg['operational_efficiency'], 1),
                    'status': 'üstünde' if company_operational > sector_avg['operational_efficiency'] else 'altında'
                }
            },
            'insights': cls._generate_benchmark_insights(
                company_audit, company_compliance, company_risk, company_operational, sector_avg
            )
        }
    
    @classmethod
    def _generate_benchmark_insights(cls, audit, compliance, risk, operational, sector_avg) -> List[str]:
    ...
    @classmethod
    def get_cost_optimization_opportunities(cls, audit_events, financial_data: Dict = None) -> List[Dict]:
        """
    ...
    def generate_executive_summary(cls, company, audit_events, period_days=30) -> Dict:
        ...
        # Finansal etki
        financial_impact = recent_events.aggregate(
            total=Sum('financial_impact')
        )['total'] or 0
        
        # Trend
        previous_period = audit_events.filter(
            created_at__gte=cutoff - timedelta(days=period_days),
            created_at__lt=cutoff
        ).count()
        
        trend = 'artış' if total_events > previous_period else 'azalış' if total_events < previous_period else 'stabil'
        trend_percent = abs(((total_events - previous_period) / max(previous_period, 1)) * 100)
        ...
        return {
            ...
            'key_highlights': [
                ...
                f"{'⚠️ ' if critical_events > 0 else '✅ '}{critical_events} kritik olay" + (" - Acil dikkat gerekli!" if critical_events > 5 else ""),
                f"Finansal etki: {financial_impact:,.2f} TL"
            ],
            'action_items': cls._generate_action_items(critical_events, high_events, health),
            'next_steps': [
    ...
    @classmethod
    def _generate_action_items(cls, critical: int, high: int, health: Dict) -> List[str]:
        ...
        if high > 5:
            actions.append(f"⚠️ YÜKSEK ÖNCELİK: {high} yüksek öncelikli olay takip edilmeli")
        
        if health['overall_score'] < 60:
            actions.append("📊 DİKKAT: Genel performans düşük, iyileştirme planı hazırlayın")
