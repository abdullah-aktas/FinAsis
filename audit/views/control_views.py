...
from finance.internal_control_system import (
    ControlActivity,
    AuditTrail,
    RiskAssessment,
    ApprovalWorkflow,
...
def control_dashboard(request):
    ...
    if cached_compliance is None:
        ...
        if total_controls > 0:
            effective_controls = active_controls.filter(
            ...
            cached_compliance = {
                ...
                "completed_audits": AuditTrail.objects.filter(
                    company=company, action_type="CREATE"
                ).count(),
    ...
    control_ids = list(
        active_controls.order_by("-updated_at")[:6].values_list("id", flat=True)
    )
    executions_map = {
    ...
    for control in ControlActivity.objects.filter(id__in=control_ids):
        ...
        if not last_test_date:
            last_test_date = (
                getattr(control, "last_performed_date", None)
                or getattr(control, "created_at", timezone.now()).date()
            )
        freq = getattr(control, "frequency", "MONTHLY") or "MONTHLY"
        next_test_due = (
            last_test_date + timedelta(days=freq_days.get(freq, 30))
            if last_test_date
            else None
        )
        ...
        eff_for_score = (
            "EFFECTIVE" if effectiveness == "EFFECTIVE" else "NEEDS_IMPROVEMENT"
        )
    ...
    if cached_risk is None:
        ...
        cached_risk = {
            ...
            "high_critical": sum(
                ...
                if r["overall_risk_level"] in ["HIGH", "VERY_HIGH"]
            ),
    ...
    risk_assessments = []
    for risk in RiskAssessment.objects.filter(company=company).order_by(
        "-assessment_date"
    )[:5]:
...
def risk_assessment_view(request):
    ...
    if request.method == "POST":
        try:
            ...
            assessment = RiskAssessment.objects.create(
                ...
                potential_financial_impact=request.POST.get(
            ...
            messages.error(
                request, f"Risk değerlendirmesi kaydedilirken hata oluştu: {str(e)}"
            )
    ...
    risk_level_distribution = (
        assessments.values("overall_risk_level")
        .annotate(count=Count("id"))
        .order_by("overall_risk_level")
    )
    ...
    risk_category_distribution = (
        assessments.values("risk_category")
        .annotate(count=Count("id"))
        .order_by("-count")
    )
...
def control_create(request):
    ...
    if request.method == "POST":
        try:
            control_id = (
                request.POST.get("control_id")
                or f"CTRL-{int(timezone.now().timestamp())}"
            )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if test_result in ("EFFECTIVE", "INEFFECTIVE"):
                ...
                control.save(
                    update_fields=["operating_effectiveness", "last_performed_date"]
                )
            AuditTrail.log_action(
                user=request.user,
                company=company,
...
def audit_trail_report(request):
    ...
    # Temel sorgu
    trails = (
        AuditTrail.objects.filter(company=company)
        .select_related("user")
        .order_by("-timestamp")
    )
    ...
    action_distribution = (
        trails.values("action_type").annotate(count=Count("id")).order_by("-count")
    )
    ...
    for i in range(7):
        ...
        daily_stats.append({"date": day_start.strftime("%d %b"), "count": count})
...
def compliance_report(request):
    ...
    # Etkinlik dağılımı
    effectiveness_stats = (
        controls.values("operating_effectiveness")
        .annotate(count=Count("id"))
    ...
    effective_percentage = effectiveness_breakdown.get("EFFECTIVE", {}).get(
        "percentage", 0
    )
    ...
    risk_stats = (
        risks.values("overall_risk_level")
        .annotate(count=Count("id"))
        .order_by("overall_risk_level")
    )
    ...
    for stat in risk_stats:
        ...
        count = stat["count"]
        percentage = (count / total_risks * 100) if total_risks > 0 else 0
        risk_breakdown[level] = {"count": count, "percentage": round(percentage, 1)}

    # Yüksek risk sayısı
    high_risk_count = risk_breakdown.get("HIGH", {}).get(
        "count", 0
    ...
    audit_stats = (
        audits.values("action_type").annotate(count=Count("id")).order_by("-count")
    )
    ...
    for stat in audit_stats:
        ...
        audit_breakdown[action] = {"count": count, "percentage": round(percentage, 1)}
    ...
    workflow_stats = (
        workflows.values("is_active").annotate(count=Count("id")).order_by("-is_active")
    )

    active_workflows = sum(
        stat["count"] for stat in workflow_stats if stat["is_active"]
    )
    ...
    for i in range(6):
        ...
        count = AuditTrail.objects.filter(
            company=company, timestamp__gte=month_start, timestamp__lt=month_end
        ).count()
        monthly_audits.append({"month": month_start.strftime("%b"), "count": count})
...
def audit_trail_export_pdf(request):
    ...
    html = (
        "<html><body><h1>Audit Trail PDF Export</h1><p>Export soon...</p></body></html>"
    )
...
def control_test_all(request):  # Basit yönlendirme placeholder
    ...
    messages.success(
        request,
        _("%(count)d kontrol için test kaydı oluşturuldu.") % {"count": created},
    )
    return redirect("audit:control_dashboard")
...
def ajax_test_control(request, control_id):  # AJAX placeholder
    if request.method != "POST":
        return JsonResponse(
            {"success": False, "message": _("Yalnızca POST desteklenir.")}, status=405
        )
    try:
        control = ControlActivity.objects.select_related("company").get(id=control_id)
    except ControlActivity.DoesNotExist:
        return JsonResponse(
            {"success": False, "message": _("Kontrol bulunamadı.")}, status=404
        )

    company = getattr(request.user, "company", None)
    if company is not None and getattr(control.company, "id", None) != getattr(
        company, "id", company
    ):
        return JsonResponse(
            {"success": False, "message": _("Yetkisiz şirket erişimi.")}, status=403
        )

    allowed_roles = {"Admin", "Accountant", "Auditor"}
    user_roles = set(request.user.groups.values_list("name", flat=True))
    has_role_access = request.user.is_superuser or bool(
        user_roles.intersection(allowed_roles)
    )

    if (
        not has_role_access
        and getattr(control, "control_owner_id", None) != request.user.id
    ):
        return JsonResponse(
            {"success": False, "message": _("Bu işlem için yetkiniz yok.")}, status=403
        )
...
def ajax_control_detail(request, control_id):
    ...
    try:
        ...
        executions_qs = getattr(control, "executions", None)
        tests = (
            executions_qs.order_by("-execution_date")[:5]
            if executions_qs is not None
            else []
        )
        html = render(
            request,
            "audit/partials/control_detail.html",
            {"control": control, "tests": tests},
...
def ajax_workflow_approve(request, workflow_id):
    ...
    try:
        workflow = ApprovalWorkflow.objects.select_related("company").get(
            id=workflow_id
        )
    except ApprovalWorkflow.DoesNotExist:
        return JsonResponse(
            {"success": False, "message": _("İş akışı bulunamadı.")}, status=404
        )
    # Company tutarlılığı: Kullanıcının şirketi varsa doğrula
    user_company = getattr(request.user, "company", None)
    if user_company is not None and getattr(workflow.company, "id", None) != getattr(
        user_company, "id", user_company
    ):
        return JsonResponse(
            {"success": False, "message": _("Yetkisiz şirket erişimi.")}, status=403
        )
...
def get_compliance_description(score):
    ...
    if score >= 90:
        return _(
            "Kontrol sistemleriniz çok iyi çalışıyor. Mevcut performansınızı koruyun."
        )
    elif score >= 80:
        return _(
            "Kontrol sistemleriniz iyi durumda. Küçük iyileştirmeler yapabilirsiniz."
        )
    elif score >= 70:
        return _(
            "Kontrol sistemleriniz orta seviyede. Bazı alanlarda iyileştirme gerekli."
        )
    elif score >= 60:
