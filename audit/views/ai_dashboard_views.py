...
def ai_dashboard(request):
    ...
    entity_data = {
        ...
        'high_severity_count': audit_events.filter(severity__in=['high', 'critical']).count(),
        'total_financial_impact': sum([
            float(e.financial_impact) for e in audit_events.filter(financial_impact__isnull=False)
        ]),
...
def kobi_health_dashboard(request):
    ...
    if not company:
        return render(request, 'audit/kobi_health.html', {
    ...
    # Sağlık skoru
    health_score = KOBIAuditAnalytics.get_kobi_health_score(
        company, audit_events
    )
...
def blockchain_verification_view(request):
    ...
    # Son 10 blok
    recent_blocks = blockchain_manager.blockchain.chain[-10:] if len(blockchain_manager.blockchain.chain) > 1 else []
    ...
    for i in range(days):
        ...
        if day_events.exists():
            entity_data = {
                ...
                'high_severity_count': day_events.filter(severity__in=['high', 'critical']).count(),
                'total_financial_impact': sum([
                    float(e.financial_impact) for e in day_events.filter(financial_impact__isnull=False)
                ]),
                'off_hours_count': 0
            }
