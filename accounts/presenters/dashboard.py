...
class UserDashboardPresenter(BasePresenter):
    ...
    def get_context_data(self) -> MutableMapping[str, Any]:
        ...
        if role_code == "oyuncu":
            ...
            context["simulation_cards"] = task_engine.get_tasks(
                audience="gamer", kind="simulation", limit=3
            ) or [
                {
                    "title": "Trade Sim",
            ...
            progress_pct = (
                min(100, int((total_xp % 250) / 250 * 100)) if total_xp else 0
            )
            ...
            context["ai_cards"] = prompt_registry.get_prompts_for_role(
                "oyuncu", limit=3
            ) or [
                {
    ...
    def _resolve_role_redirect(self) -> Optional[HttpResponse]:
        ...
        if code == "oyuncu":
            return render(
                self.request, "accounts/dashboard_oyuncu.html", self.get_context_data()
            )
    ...
    def _build_financial_summary(self, company) -> FinancialSummary:
        invoice_total = Invoice.objects.filter(company=company).aggregate(
            toplam=Sum("total_amount")
        )["toplam"] or Decimal("0")
        expense_total = Expense.objects.filter(company=company).aggregate(
            toplam=Sum("amount")
        )["toplam"] or Decimal("0")
        balance_total = BankAccount.objects.filter(company=company).aggregate(
            toplam=Sum("balance")
        )["toplam"] or Decimal("0")
    ...
    def _get_recent_bank_transactions(self, company) -> Sequence[BankTransaction]:
        ...
        return BankTransaction.objects.filter(account__in=accounts).order_by("-date")[
            :5
        ]
    ...
    def _get_trend_data(self, company) -> List[Dict[str, Any]]:
        ...
        for offset in range(5, -1, -1):
            ...
            ).aggregate(toplam=Sum("total_amount"))["toplam"] or Decimal("0")
            expense = Expense.objects.filter(
                company=company,
                expense_date__gte=month_start,
                expense_date__lte=month_end,
            ).aggregate(toplam=Sum("amount"))["toplam"] or Decimal("0")
    ...
    def _inject_role_specific_cards(self, context: MutableMapping[str, Any]) -> None:
        ...
        prompts = (
            prompt_registry.get_prompts_for_role(prompt_role, limit=3)
            if prompt_role
            else []
        )
        if prompts:
