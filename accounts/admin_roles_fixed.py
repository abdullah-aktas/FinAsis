...
class UserRoleAdmin(ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if obj.name == "super_admin" and obj.hierarchy_level != 0:
            messages.warning(
                request, "Süper admin hiyerarşi seviyesi 0 olarak ayarlandı."
            )
...
class SubscriptionPlanAdmin(ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            "Fiyatlandırma",
            {"fields": ("price_monthly", "price_yearly"), "classes": ("wide",)},
        ),
        (
            "Limitler",
            {
    ...
    def price_display(self, obj):
        ...
        yearly_info = (
            f" (Yıllık: ₺{obj.price_yearly} - %{yearly_discount} indirim)"
            if yearly_discount > 0
            else ""
        )

        return format_html("<strong>₺{}/ay</strong>{}", obj.price_monthly, yearly_info)
...
class UserSubscriptionAdmin(ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Ödeme", {"fields": ("amount_paid", "currency", "auto_renew")}),
        (
        ...
        ("Notlar", {"fields": ("notes",), "classes": ("collapse",)}),
    )
    ...
    def user_link(self, obj):
        ...
        try:
            ...
            url_name = (
                f"admin:{UserModel._meta.app_label}_{UserModel._meta.model_name}_change"
            )
    ...
    def usage_summary(self, obj):
        ...
        if obj.plan.max_transactions == -1:
            ...
            transaction_pct = (
                f"{obj.current_month_transactions}/{obj.plan.max_transactions}"
            )

        storage_pct = f"{obj.storage_used_mb//1024}GB/{obj.plan.storage_gb}GB"

        return format_html("📊 {}<br/>💾 {}", transaction_pct, storage_pct)
    ...
    def activate_subscription(self, request, queryset):
        ...
        updated = queryset.filter(status__in=["pending", "suspended"]).update(
            status="active"
        )
        self.message_user(
            request, f"{updated} abonelik aktifleştirildi.", messages.SUCCESS
        )
    ...
    def suspend_subscription(self, request, queryset):
        ...
        self.message_user(
            request, f"{updated} abonelik askıya alındı.", messages.WARNING
        )
...
class UserProfileAdmin(ModelAdmin):
    ...
    fieldsets = (
        ("Kullanıcı & Rol", {"fields": ("user", "role")}),
        (
            "Kişisel Bilgiler",
            {"fields": ("phone", "tc_no", "birth_date"), "classes": ("wide",)},
        ),
        (
        ...
        (
            "Hesap Ayarları",
            {"fields": ("timezone", "language"), "classes": ("collapse",)},
        ),
        (
            "Güvenlik",
            {
    ...
    def user_link(self, obj):
        ...
        return format_html('<a href="{}">{} {}</a>', url, status_icon, full_name)
    ...
    def enable_2fa(self, request, queryset):
        ...
        self.message_user(
            request, f"{updated} kullanıcı için 2FA aktifleştirildi.", messages.SUCCESS
        )
    ...
    def unlock_users(self, request, queryset):
        ...
        self.message_user(
            request, f"{updated} kullanıcı hesabının kilidi açıldı.", messages.SUCCESS
        )
...
class UserProfileInline(admin.StackedInline):
    ...
    fieldsets = (
        ("Rol & Şirket", {"fields": ("role", "company_name", "job_title")}),
        ("İletişim", {"fields": ("phone",)}),
        (
            "Güvenlik",
            {"fields": ("two_factor_enabled", "is_locked"), "classes": ("collapse",)},
        ),
    )
...
class EnhancedUserAdmin(BaseUserAdmin):
    inlines = (
        UserProfileInline,
    )  # UserSubscriptionInline'ı kaldırdık çakışma olmaması için
    ...
    list_filter = list(BaseUserAdmin.list_filter) + ["is_active", "date_joined"]
...
def role_statistics_view(request):
    ...
    stats = UserRole.objects.annotate(user_count=Count("userprofile")).values(
        "display_name", "user_count", "hierarchy_level"
    )
...
def subscription_analytics_view(request):
    ...
    analytics = SubscriptionPlan.objects.annotate(
        active_subscribers=Count(
            "usersubscription", filter=models.Q(usersubscription__status="active")
        ),
