...
def panel_data_api(request):
    ...
    if user_company:
        ...
        try:
            ...
            for i in range(5, -1, -1):
                ...
                expense_total = (
                    Expense.objects.filter(
                        company=user_company, date__gte=month_start, date__lt=month_end
                    ).aggregate(total=Sum("amount"))["total"]
                    or 0
                )

                invoice_trend.append(
                    {"month": month_start.strftime("%b"), "total": float(invoice_total)}
                )
                expense_trend.append(
                    {"month": month_start.strftime("%b"), "total": float(expense_total)}
                )
            ...
            # Son işlemler
            recent_invoices = Invoice.objects.filter(company=user_company).order_by(
                "-issue_date"
            )[:3]

            recent_expenses = Expense.objects.filter(company=user_company).order_by(
                "-date"
            )[:3]
