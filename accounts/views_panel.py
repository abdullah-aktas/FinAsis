...
def _get_user_modules(user):
    ...
    # Muhasebe Modülü
    if (user.has_perm('accounting.view_invoice') or 
        user.has_perm('accounting.view_expense') or 
        'accountant' in user_roles or 
    ...
    # Blockchain Modülü
    if (user.has_perm('blockchain.view_blockchainrecord') or 
        'manager' in user_roles):
        try:
            modules.append({
                'name': 'Blockchain',
