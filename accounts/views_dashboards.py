...
from finance.models import Invoice, Expense, BankAccount
...
def kobi_dashboard(request):
    ...
    # Son işlemler
    son_faturalar = (
        Invoice.objects.filter(company=company).order_by("-issue_date")[:5]
        if company
        else []
    )
    son_giderler = (
        Expense.objects.filter(company=company).order_by("-expense_date")[:5]
        if company
        else []
    )
    ...
    ai_cards = prompt_registry.get_prompts_for_role("mali_musavir", limit=3) or [
        {
            ...
            "body": _(
                "Son 30 gündeki nakit akışı düşen müşteriler için uyarı raporu hazır."
            ),
...
def egitimci_dashboard_new(request):
    ...
    my_courses = (
        Course.objects.filter(instructor=request.user)[:5]
    ...
    onboarding_steps = task_engine.get_tasks(
        audience="teacher", kind="onboarding", limit=3
    ) or [
        {
    ...
    gamification = {
        ...
        "delta": _("Toplam XP: %(xp)s")
        % {"xp": teacher_brief.get("total_reward_xp", 0)},
    }
    ai_cards = prompt_registry.get_prompts_for_role("egitimci", limit=3) or [
        {
            "title": _("AI Asistan: Ders özetleri"),
            "body": _(
                "Son derslerin özetini öğrencilerle paylaşmaya hazır bir metin oluşturuldu."
            ),
...
def ogrenci_dashboard_new(request):
    ...
    onboarding_steps = task_engine.get_tasks(
        audience="student", kind="onboarding", limit=3
    ) or [
        {
    ...
    gamification = {
        ...
        "delta": _("Toplam XP: %(xp)s")
        % {"xp": student_brief.get("total_reward_xp", 0)},
    }
    ai_cards = prompt_registry.get_prompts_for_role("ogrenci", limit=3) or [
        {
            "title": _("AI Asistan: Çalışma planı"),
            "body": _(
                "Bu hafta tamamlaman gereken dersler için günlük plan önerisi hazır."
            ),
