...
def user_role_card(user):
    ...
    try:
        ...
        context = {
            ...
            'expires_soon': subscription.days_remaining <= 7 if subscription.is_active else False
        }
...
def navigation_items(user):
    ...
    try:
        ...
        nav_items = [
            {'name': 'dashboard', 'title': 'Ana Sayfa', 'icon': '🏠', 'url': '/dashboard/', 'allowed': True},
        ]
        
        # Mali modüller
        if role.can_view_all_finances or role.can_edit_finances:
            nav_items.append({
                'name': 'accounting', 'title': 'Muhasebe', 'icon': '📚', 
                'url': '/accounting/', 'allowed': subscription.plan.has_accounting
            })
            nav_items.append({
                'name': 'finance', 'title': 'Finans', 'icon': '💼',
                'url': '/finance/', 'allowed': subscription.plan.has_finance
            })
        
        # Özel modüller
        if role.can_access_ai:
            nav_items.append({
                'name': 'ai', 'title': 'AI Asistan', 'icon': '🤖',
                'url': '/ai/', 'allowed': subscription.plan.has_ai_assistant
            })
        
        if role.can_use_education:
            nav_items.append({
                'name': 'education', 'title': 'Eğitim', 'icon': '🎓',
                'url': '/education/', 'allowed': subscription.plan.has_education
            })
            
        if role.can_use_blockchain:
            nav_items.append({
                'name': 'blockchain', 'title': 'Blockchain', 'icon': '⛓️',
                'url': '/blockchain/', 'allowed': subscription.plan.has_blockchain
            })
        
        # Yönetim
        if role.can_manage_users:
            nav_items.append({
                'name': 'admin', 'title': 'Yönetim', 'icon': '⚙️',
                'url': '/admin/', 'allowed': True
            })
        
        return [item for item in nav_items if item['allowed']]
        
    except:
        return [{'name': 'dashboard', 'title': 'Ana Sayfa', 'icon': '🏠', 'url': '/dashboard/', 'allowed': True}]