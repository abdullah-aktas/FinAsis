...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            'input_file',
            type=str,
            help='Import edilecek JSON dosyasının yolu'
        )
        parser.add_argument(
            '--update-existing',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'   Export tarihi: {export_data.get("export_date", "Bilinmiyor")}\n')
                    ...
                    try:
                        st, created = SubscriptionType.objects.update_or_create(
                            ...
                            defaults={
                                ...
                                'period_options': st_data.get('period_options', 'monthly'),
                        ...
                        if created:
                            ...
                            if not dry_run:
                                self.stdout.write(f'    ✓ Oluşturuldu: {st.name} ({st.code})')
                        else:
                            stats['subscription_types_updated'] += 1
                            if not dry_run:
                                self.stdout.write(f'    ↻ Güncellendi: {st.name} ({st.code})')
                ...
                for ut_data in export_data['user_types']:
                    try:
                        ...
                        if created:
                            ...
                            if not dry_run:
                                self.stdout.write(f'    ✓ Oluşturuldu: {ut.name} ({ut.code})')
                        else:
                            stats['user_types_updated'] += 1
                            if not dry_run:
                                self.stdout.write(f'    ↻ Güncellendi: {ut.name} ({ut.code})')
                    except Exception as e:
                ...
                for user_data in export_data['users']:
                    try:
                        ...
                        if existing_user and not update_existing:
                            ...
                            if not dry_run:
                                self.stdout.write(
                                    self.style.WARNING(f'    ⊘ Atlanıldı (zaten var): {username}')
                                )
                            continue
                        ...
                        if user_data.get('user_type_code'):
                            try:
                                user_type = UserType.objects.get(code=user_data['user_type_code'])
                            ...
                            if skip_passwords or not user_data.get('password'):
                                ...
                                if not dry_run:
                                    user = User.objects.create_user(
                                        ...
                                        **{k: v for k, v in user_defaults.items() if k != 'password'}
                                    )
                                    self.stdout.write(
                                        self.style.WARNING(
        ...
        self.stdout.write(f'      - UserType: +{stats["user_types_created"]} / ↻{stats["user_types_updated"]}')
        self.stdout.write(
