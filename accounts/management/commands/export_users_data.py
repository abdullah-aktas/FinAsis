    ...
    def handle(self, *args, **options):
        ...
        for ut in user_types:
            export_data['user_types'].append({
                'code': ut.code,
                'name': ut.name,
                'default_subscription_code': ut.default_subscription.code if ut.default_subscription else None,
            })
        self.stdout.write(self.style.SUCCESS(f'    ✓ {len(export_data["user_types"])} UserType export edildi'))
        ...
        for st in subscription_types:
            export_data['subscription_types'].append({
                ...
                'monthly_price': str(st.monthly_price) if st.monthly_price else None,
                'yearly_price': str(st.yearly_price) if st.yearly_price else None,
                'user_limit': st.user_limit,
                'features': st.features,
            })
        self.stdout.write(self.style.SUCCESS(f'    ✓ {len(export_data["subscription_types"])} SubscriptionType export edildi'))
        ...
        if include_companies:
            ...
            self.stdout.write(self.style.SUCCESS(f'    ✓ {len(export_data["companies"])} şirket export edildi'))

        # 4. Kullanıcıları export et
        self.stdout.write('  → Kullanıcılar export ediliyor...')
        users = User.objects.select_related('user_type', 'company').prefetch_related('groups').all()
        ...
        self.stdout.write(self.style.SUCCESS(f'    ✓ {len(export_data["users"])} kullanıcı export edildi'))
        ...
        self.stdout.write(f'      - SubscriptionType: {len(export_data["subscription_types"])}')
