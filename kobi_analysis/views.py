...
from .models import (
    ...
    CashFlowForecast, FinancialGoal, GoalProgress,
    IndustryBenchmark, CompetitorAnalysis, SWOTAnalysis,
    RiskAssessment, RiskMitigation, PerformanceMetric,
    FinancialAlert, AdvisoryReport, FinancialHealthSnapshot
)
from accounting.models import Company, Invoice, Expense
from audit.ai_audit import AuditAIAnalyzer
...
def kobi_dashboard(request):
    ...
    if not company:
        messages.warning(request, 'Şirket bilgisi bulunamadı. Lütfen önce şirket oluşturun.')
    ...
    # Son finansal analiz
    latest_analysis = KOBIFinancialAnalysis.objects.filter(
        company=company
    ...
    # Son raporlar
    recent_reports = AdvisoryReport.objects.filter(
        company=company
...
def budget_planning(request):
    ...
    # Taslak bütçeler
    draft_budgets = BudgetPlan.objects.filter(
        company=company,
        status='DRAFT'
    )
...
def risk_management(request):
    ...
    # Kritik riskler
    critical_risks = all_risks.filter(
        risk_score__gte=70,
        status='IDENTIFIED'
    ).order_by('-risk_score')
...
def competitor_analysis(request):
    ...
    # Rakip analizleri
    analyses = CompetitorAnalysis.objects.filter(
        company=company
...
def swot_analysis(request):
    ...
    # En son SWOT
    latest_swot = SWOTAnalysis.objects.filter(
        company=company
    ).order_by('-analysis_date').first()
    
    # Tüm SWOT'lar
    all_swots = SWOTAnalysis.objects.filter(
        company=company
    ).order_by('-analysis_date')
...
def performance_metrics(request):
    ...
    metrics = PerformanceMetric.objects.filter(
        company=company
    ).order_by('metric_category', 'metric_name')
...
def advisory_reports(request):
    ...
    # Raporlar
    reports = AdvisoryReport.objects.filter(
        company=company
...
def generate_analysis(request):
    ...
    if request.method == 'POST':
        ...
        if start_date and end_date:
            ...
            messages.success(request, f'Finansal analiz başarıyla oluşturuldu! Sağlık Skoru: {analysis.financial_health_score}')
...
def ajax_health_trend(request):
    ...
    snapshots = FinancialHealthSnapshot.objects.filter(
        company=company
