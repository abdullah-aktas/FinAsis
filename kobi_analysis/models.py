...
class BudgetPlan(models.Model):
    ...
    def variance_percentage(self):
        ...
        if self.planned_revenue > 0:
            return ((self.actual_revenue - self.planned_revenue) / self.planned_revenue) * 100
        return 0
...
class BudgetLineItem(models.Model):
    ...
    budget = models.ForeignKey(BudgetPlan, on_delete=models.CASCADE, related_name='line_items', verbose_name=_('Bütçe'))
    line_item_name = models.CharField(max_length=200, verbose_name=_('Kalem Adı'))
    category = models.CharField(max_length=20, choices=CATEGORY_CHOICES, verbose_name=_('Kategori'))
    
    planned_amount = models.DecimalField(max_digits=15, decimal_places=2, default=Decimal('0.00'), verbose_name=_('Planlanan Tutar'))
    actual_amount = models.DecimalField(max_digits=15, decimal_places=2, default=Decimal('0.00'), verbose_name=_('Gerçekleşen Tutar'))
    variance = models.DecimalField(max_digits=15, decimal_places=2, default=Decimal('0.00'), verbose_name=_('Varyans'))
...
class CashFlowForecast(models.Model):
    ...
    forecast_period_start = models.DateField(verbose_name=_('Tahmin Dönemi Başlangıç'), null=True, blank=True)
    forecast_period_end = models.DateField(verbose_name=_('Tahmin Dönemi Bitiş'), null=True, blank=True)
    confidence_level = models.CharField(max_length=10, choices=CONFIDENCE_CHOICES, default='MEDIUM', verbose_name=_('Güven Seviyesi'))
...
class ForecastScenario(models.Model):
    ...
    forecast = models.ForeignKey(CashFlowForecast, on_delete=models.CASCADE, related_name='scenarios', verbose_name=_('Tahmin'))
    scenario_name = models.CharField(max_length=100, verbose_name=_('Senaryo Adı'))
    scenario_type = models.CharField(max_length=20, choices=SCENARIO_TYPE_CHOICES, verbose_name=_('Senaryo Türü'))
    probability = models.DecimalField(max_digits=5, decimal_places=2, verbose_name=_('Olasılık %'))
    
    projected_revenue = models.DecimalField(max_digits=15, decimal_places=2, default=Decimal('0.00'), verbose_name=_('Tahmini Gelir'))
    projected_expenses = models.DecimalField(max_digits=15, decimal_places=2, default=Decimal('0.00'), verbose_name=_('Tahmini Gider'))
    projected_profit = models.DecimalField(max_digits=15, decimal_places=2, default=Decimal('0.00'), verbose_name=_('Tahmini Kar'))
...
class FinancialGoal(models.Model):
    ...
    goal_type = models.CharField(max_length=20, choices=GOAL_TYPE_CHOICES, verbose_name=_('Hedef Türü'))
    
    target_amount = models.DecimalField(max_digits=15, decimal_places=2, verbose_name=_('Hedef Tutar'))
    current_amount = models.DecimalField(max_digits=15, decimal_places=2, default=Decimal('0.00'), verbose_name=_('Mevcut Tutar'))
    achievement_percentage = models.DecimalField(max_digits=5, decimal_places=2, default=Decimal('0.00'), verbose_name=_('Başarım %'))
    
    start_date = models.DateField(verbose_name=_('Başlangıç Tarihi'))
    deadline = models.DateField(verbose_name=_('Termin'))
    
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='NOT_STARTED', verbose_name=_('Durum'))
    priority = models.CharField(max_length=10, choices=PRIORITY_CHOICES, default='MEDIUM', verbose_name=_('Öncelik'))
    ...
    def update_achievement(self):
        ...
        if self.target_amount > 0:
            self.achievement_percentage = (self.current_amount / self.target_amount) * 100
...
class GoalProgress(models.Model):
    ...
    goal = models.ForeignKey(FinancialGoal, on_delete=models.CASCADE, related_name='progress_records', verbose_name=_('Hedef'))
    progress_date = models.DateField(verbose_name=_('İlerleme Tarihi'))
    current_amount = models.DecimalField(max_digits=15, decimal_places=2, verbose_name=_('Mevcut Tutar'))
    achievement_percentage = models.DecimalField(max_digits=5, decimal_places=2, verbose_name=_('Başarım %'))
...
class IndustryBenchmark(models.Model):
    ...
    average_value = models.DecimalField(max_digits=15, decimal_places=2, verbose_name=_('Ortalama Değer'))
    top_quartile_value = models.DecimalField(max_digits=15, decimal_places=2, verbose_name=_('En İyi %25'))
...
class CompetitorAnalysis(models.Model):
    ...
    revenue = models.DecimalField(max_digits=15, decimal_places=2, null=True, blank=True, verbose_name=_('Gelir'))
    market_share = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True, verbose_name=_('Pazar Payı %'))
...
class SWOTAnalysis(models.Model):
    ...
    strategic_recommendations = models.TextField(null=True, blank=True, verbose_name=_('Stratejik Öneriler'))
...
class RiskAssessment(models.Model):
    ...
    risk_category = models.CharField(max_length=20, choices=CATEGORY_CHOICES, verbose_name=_('Risk Kategorisi'))
    
    description = models.TextField(verbose_name=_('Açıklama'))
    
    likelihood = models.CharField(max_length=20, choices=LIKELIHOOD_CHOICES, verbose_name=_('Olasılık'))
    impact = models.CharField(max_length=20, choices=IMPACT_CHOICES, verbose_name=_('Etki'))
    risk_score = models.IntegerField(default=0, verbose_name=_('Risk Skoru'))
    
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='IDENTIFIED', verbose_name=_('Durum'))
    
    identified_date = models.DateField(verbose_name=_('Tespit Tarihi'))
    
    created_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='kobi_risk_assessments', verbose_name=_('Oluşturan'))
    created_at = models.DateTimeField(auto_now_add=True)
...
class RiskMitigation(models.Model):
    ...
    risk = models.ForeignKey(RiskAssessment, on_delete=models.CASCADE, related_name='mitigation_plans', verbose_name=_('Risk'))
    mitigation_action = models.CharField(max_length=200, verbose_name=_('Önlem Aksiyonu'))
    description = models.TextField(verbose_name=_('Açıklama'))
    
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='PLANNED', verbose_name=_('Durum'))
    effectiveness = models.CharField(max_length=10, choices=EFFECTIVENESS_CHOICES, null=True, blank=True, verbose_name=_('Etkinlik'))
    
    deadline = models.DateField(null=True, blank=True, verbose_name=_('Termin'))
    cost_estimate = models.DecimalField(max_digits=15, decimal_places=2, null=True, blank=True, verbose_name=_('Tahmini Maliyet'))
    
    assigned_to = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, verbose_name=_('Sorumlu'))
...
class PerformanceMetric(models.Model):
    ...
    company = models.ForeignKey(Company, on_delete=models.CASCADE, verbose_name=_('Şirket'))
    metric_name = models.CharField(max_length=100, verbose_name=_('Metrik Adı'))
    metric_category = models.CharField(max_length=20, choices=CATEGORY_CHOICES, verbose_name=_('Kategori'))
    
    current_value = models.DecimalField(max_digits=15, decimal_places=2, verbose_name=_('Mevcut Değer'))
    target_value = models.DecimalField(max_digits=15, decimal_places=2, null=True, blank=True, verbose_name=_('Hedef Değer'))
    
    trend = models.CharField(max_length=10, choices=TREND_CHOICES, null=True, blank=True, verbose_name=_('Trend'))
...
class MetricTarget(models.Model):
    ...
    metric = models.ForeignKey(PerformanceMetric, on_delete=models.CASCADE, related_name='targets', verbose_name=_('Metrik'))
    target_period = models.DateField(verbose_name=_('Hedef Dönemi'))
    target_value = models.DecimalField(max_digits=15, decimal_places=2, verbose_name=_('Hedef Değer'))
...
class FinancialAlert(models.Model):
    ...
    company = models.ForeignKey(Company, on_delete=models.CASCADE, verbose_name=_('Şirket'))
    alert_type = models.CharField(max_length=20, choices=ALERT_TYPE_CHOICES, verbose_name=_('Uyarı Türü'))
    severity = models.CharField(max_length=10, choices=SEVERITY_CHOICES, verbose_name=_('Ciddiyet'))
    ...
    triggered_at = models.DateTimeField(auto_now_add=True, verbose_name=_('Tetiklenme Zamanı'))
    resolved_at = models.DateTimeField(null=True, blank=True, verbose_name=_('Çözüm Zamanı'))
...
class AdvisoryReport(models.Model):
    ...
    report_type = models.CharField(max_length=20, choices=REPORT_TYPE_CHOICES, verbose_name=_('Rapor Türü'))
    ...
    generated_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, verbose_name=_('Oluşturan'))
    created_at = models.DateTimeField(auto_now_add=True)
...
class FinancialHealthSnapshot(models.Model):
    ...
    overall_score = models.DecimalField(max_digits=5, decimal_places=2, verbose_name=_('Genel Skor'))
    liquidity_score = models.DecimalField(max_digits=5, decimal_places=2, verbose_name=_('Likidite Skoru'))
    profitability_score = models.DecimalField(max_digits=5, decimal_places=2, verbose_name=_('Karlılık Skoru'))
    solvency_score = models.DecimalField(max_digits=5, decimal_places=2, verbose_name=_('Ödeme Gücü Skoru'))
    efficiency_score = models.DecimalField(max_digits=5, decimal_places=2, verbose_name=_('Verimlilik Skoru'))
...
class FinancialRating(models.Model):
    ...
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name='financial_ratings', verbose_name=_('Şirket'))
    rating_date = models.DateField(verbose_name=_('Derecelendirme Tarihi'))
    
    # Derecelendirme
    overall_rating = models.CharField(max_length=5, choices=RATING_SCALE, verbose_name=_('Genel Derece'))
    financial_strength = models.CharField(max_length=5, choices=RATING_SCALE, verbose_name=_('Finansal Güç'))
    creditworthiness = models.CharField(max_length=5, choices=RATING_SCALE, verbose_name=_('Kredi Değerliliği'))
    
    outlook = models.CharField(max_length=20, choices=OUTLOOK_CHOICES, default='STABLE', verbose_name=_('Görünüm'))
    
    # Puanlar (0-100)
    liquidity_score = models.DecimalField(max_digits=5, decimal_places=2, verbose_name=_('Likidite Puanı'))
    profitability_score = models.DecimalField(max_digits=5, decimal_places=2, verbose_name=_('Karlılık Puanı'))
    leverage_score = models.DecimalField(max_digits=5, decimal_places=2, verbose_name=_('Kaldıraç Puanı'))
    management_score = models.DecimalField(max_digits=5, decimal_places=2, verbose_name=_('Yönetim Puanı'))
    
    # Analiz detayları
    rating_rationale = models.TextField(verbose_name=_('Derecelendirme Gerekçesi'))
    key_strengths = models.JSONField(default=list, blank=True, verbose_name=_('Temel Güçlü Yönler'))
    key_weaknesses = models.JSONField(default=list, blank=True, verbose_name=_('Temel Zayıf Yönler'))
    
    # Önceki derecelendirme
    previous_rating = models.CharField(max_length=5, choices=RATING_SCALE, null=True, blank=True, verbose_name=_('Önceki Derece'))
    rating_change = models.CharField(max_length=20, blank=True, verbose_name=_('Değişim'))
    
    # Metadata
    rated_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, verbose_name=_('Derecelendiren'))
...
class BusinessValuation(models.Model):
    ...
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name='valuations', verbose_name=_('Şirket'))
    valuation_date = models.DateField(verbose_name=_('Değerleme Tarihi'))
    valuation_method = models.CharField(max_length=20, choices=VALUATION_METHODS, verbose_name=_('Değerleme Yöntemi'))
    
    # Değerlemeler
    enterprise_value = models.DecimalField(max_digits=18, decimal_places=2, verbose_name=_('İşletme Değeri'))
    equity_value = models.DecimalField(max_digits=18, decimal_places=2, verbose_name=_('Özkaynak Değeri'))
    
    # Çarpanlar
    ev_ebitda = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, verbose_name=_('EV/EBITDA'))
    pe_ratio = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, verbose_name=_('F/K Oranı'))
    pb_ratio = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, verbose_name=_('PD/DD'))
    
    # Varsayımlar
    discount_rate = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True, verbose_name=_('İskonto Oranı (%)'))
    growth_rate = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True, verbose_name=_('Büyüme Oranı (%)'))
    
    # Detaylar
    assumptions = models.JSONField(default=dict, blank=True, verbose_name=_('Varsayımlar'))
    calculation_details = models.JSONField(default=dict, blank=True, verbose_name=_('Hesaplama Detayları'))
    methodology_notes = models.TextField(blank=True, verbose_name=_('Metodoloji Notları'))
    
    # Önceki değerleme ile karşılaştırma
    previous_valuation = models.DecimalField(max_digits=18, decimal_places=2, null=True, blank=True, verbose_name=_('Önceki Değerleme'))
    value_change_percentage = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, verbose_name=_('Değer Değişimi (%)'))
    
    # Metadata
    valued_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, verbose_name=_('Değerleyen'))
    report_file = models.FileField(upload_to='valuations/', null=True, blank=True, verbose_name=_('Rapor Dosyası'))
    ...
    def __str__(self):
        return f"{self.company.name} - {self.enterprise_value} TL ({self.valuation_date})"


class WorkingCapitalAnalysis(models.Model):
    """İşletme Sermayesi Analizi"""
    
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name='working_capital_analyses', verbose_name=_('Şirket'))
    analysis_date = models.DateField(verbose_name=_('Analiz Tarihi'))
    
    # Dönen varlıklar
    current_assets = models.DecimalField(max_digits=15, decimal_places=2, verbose_name=_('Dönen Varlıklar'))
    cash_and_equivalents = models.DecimalField(max_digits=15, decimal_places=2, verbose_name=_('Nakit ve Benzerleri'))
    accounts_receivable = models.DecimalField(max_digits=15, decimal_places=2, verbose_name=_('Alacaklar'))
    inventory = models.DecimalField(max_digits=15, decimal_places=2, verbose_name=_('Stoklar'))
    
    # Kısa vadeli yükümlülükler
    current_liabilities = models.DecimalField(max_digits=15, decimal_places=2, verbose_name=_('Kısa Vadeli Yükümlülükler'))
    accounts_payable = models.DecimalField(max_digits=15, decimal_places=2, verbose_name=_('Borçlar'))
    short_term_debt = models.DecimalField(max_digits=15, decimal_places=2, verbose_name=_('Kısa Vadeli Borç'))
    
    # Hesaplanan değerler
    working_capital = models.DecimalField(max_digits=15, decimal_places=2, verbose_name=_('İşletme Sermayesi'))
    net_working_capital = models.DecimalField(max_digits=15, decimal_places=2, verbose_name=_('Net İşletme Sermayesi'))
    
    # Oranlar
    current_ratio = models.DecimalField(max_digits=10, decimal_places=4, verbose_name=_('Cari Oran'))
    quick_ratio = models.DecimalField(max_digits=10, decimal_places=4, verbose_name=_('Asit Test Oranı'))
    cash_ratio = models.DecimalField(max_digits=10, decimal_places=4, verbose_name=_('Nakit Oranı'))
    
    # Dönem süreleri (gün)
    days_sales_outstanding = models.IntegerField(null=True, blank=True, verbose_name=_('Alacak Tahsil Süresi (Gün)'))
    days_inventory_outstanding = models.IntegerField(null=True, blank=True, verbose_name=_('Stok Devir Süresi (Gün)'))
    days_payable_outstanding = models.IntegerField(null=True, blank=True, verbose_name=_('Borç Ödeme Süresi (Gün)'))
    cash_conversion_cycle = models.IntegerField(null=True, blank=True, verbose_name=_('Nakit Dönüş Döngüsü (Gün)'))
...
class BreakEvenAnalysis(models.Model):
    ...
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name='breakeven_analyses', verbose_name=_('Şirket'))
    analysis_date = models.DateField(verbose_name=_('Analiz Tarihi'))
    analysis_period = models.CharField(max_length=20, choices=[
        ('monthly', _('Aylık')),
        ('quarterly', _('Çeyreklik')),
        ('annual', _('Yıllık'))
    ], default='monthly', verbose_name=_('Analiz Dönemi'))
    
    # Maliyet yapısı
    fixed_costs = models.DecimalField(max_digits=15, decimal_places=2, default=Decimal('0.00'), verbose_name=_('Sabit Maliyetler'))
    variable_costs_per_unit = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'), verbose_name=_('Birim Değişken Maliyet'))
    selling_price_per_unit = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'), verbose_name=_('Birim Satış Fiyatı'))
    
    # Hesaplanan değerler
    contribution_margin_per_unit = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal('0.00'), verbose_name=_('Birim Katkı Marjı'))
    contribution_margin_ratio = models.DecimalField(max_digits=5, decimal_places=2, default=Decimal('0.00'), verbose_name=_('Katkı Marjı Oranı (%)'))
    
    breakeven_units = models.DecimalField(max_digits=15, decimal_places=2, default=Decimal('0.00'), verbose_name=_('Başabaş Satış Miktarı'))
    breakeven_sales_revenue = models.DecimalField(max_digits=15, decimal_places=2, default=Decimal('0.00'), verbose_name=_('Başabaş Satış Geliri'))
    
    # Mevcut durum
    current_sales_units = models.DecimalField(max_digits=15, decimal_places=2, null=True, blank=True, verbose_name=_('Mevcut Satış Miktarı'))
    current_sales_revenue = models.DecimalField(max_digits=15, decimal_places=2, null=True, blank=True, verbose_name=_('Mevcut Satış Geliri'))
    margin_of_safety_units = models.DecimalField(max_digits=15, decimal_places=2, null=True, blank=True, verbose_name=_('Güvenlik Marjı (Adet)'))
    margin_of_safety_percentage = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True, verbose_name=_('Güvenlik Marjı (%)'))
    ...
    def save(self, *args, **kwargs):
        # Katkı marjı hesapla
        self.contribution_margin_per_unit = self.selling_price_per_unit - self.variable_costs_per_unit
        
        if self.selling_price_per_unit > 0:
            self.contribution_margin_ratio = (self.contribution_margin_per_unit / self.selling_price_per_unit) * 100
        ...
        if self.contribution_margin_per_unit > 0:
            ...
            self.breakeven_sales_revenue = self.breakeven_units * self.selling_price_per_unit
        
        # Güvenlik marjı hesapla
        if self.current_sales_units and self.breakeven_units:
            self.margin_of_safety_units = self.current_sales_units - self.breakeven_units
            if self.current_sales_units > 0:
                self.margin_of_safety_percentage = (self.margin_of_safety_units / self.current_sales_units) * 100
...
class SensitivityAnalysis(models.Model):
    ...
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name='sensitivity_analyses', verbose_name=_('Şirket'))
    analysis_date = models.DateField(verbose_name=_('Analiz Tarihi'))
    
    base_scenario_name = models.CharField(max_length=200, verbose_name=_('Baz Senaryo Adı'))
    
    # Analiz edilen değişkenler
    variable_name = models.CharField(max_length=100, verbose_name=_('Değişken Adı'))
    base_value = models.DecimalField(max_digits=15, decimal_places=2, verbose_name=_('Baz Değer'))
    
    # Senaryo değişimleri (JSON)
    scenarios = models.JSONField(default=list, verbose_name=_('Senaryolar'), help_text=_('Her senaryo: {change_percentage, new_value, impact}'))
    
    # Etki metrikleri
    impact_on_revenue = models.JSONField(default=dict, blank=True, verbose_name=_('Gelir Etkisi'))
    impact_on_profit = models.JSONField(default=dict, blank=True, verbose_name=_('Kar Etkisi'))
    impact_on_cash_flow = models.JSONField(default=dict, blank=True, verbose_name=_('Nakit Akışı Etkisi'))
    
    # Kritik noktalar
    critical_threshold = models.DecimalField(max_digits=15, decimal_places=2, null=True, blank=True, verbose_name=_('Kritik Eşik'))
    sensitivity_coefficient = models.DecimalField(max_digits=10, decimal_places=4, null=True, blank=True, verbose_name=_('Duyarlılık Katsayısı'))
    
    # Analiz sonuçları
    analysis_summary = models.TextField(verbose_name=_('Analiz Özeti'))
    risk_assessment = models.TextField(blank=True, verbose_name=_('Risk Değerlendirmesi'))
    recommendations = models.TextField(blank=True, verbose_name=_('Öneriler'))
...
class ScenarioPlanning(models.Model):
    ...
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name='scenario_plans', verbose_name=_('Şirket'))
    scenario_name = models.CharField(max_length=200, verbose_name=_('Senaryo Adı'))
    scenario_type = models.CharField(max_length=20, choices=SCENARIO_TYPES, verbose_name=_('Senaryo Tipi'))
    ...
    # Tahminler
    revenue_forecast = models.JSONField(default=list, verbose_name=_('Gelir Tahmini'), help_text=_('Aylık/çeyreklik tahminler'))
    expense_forecast = models.JSONField(default=list, verbose_name=_('Gider Tahmini'))
    cash_flow_forecast = models.JSONField(default=list, verbose_name=_('Nakit Akışı Tahmini'))
    
    # Varsayımlar
    key_assumptions = models.JSONField(default=dict, verbose_name=_('Temel Varsayımlar'))
    growth_rate = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True, verbose_name=_('Büyüme Oranı (%)'))
    inflation_rate = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True, verbose_name=_('Enflasyon Oranı (%)'))
    
    # Olasılık ve güven
    probability = models.DecimalField(max_digits=5, decimal_places=2, default=Decimal('50.00'), verbose_name=_('Gerçekleşme Olasılığı (%)'))
    confidence_level = models.CharField(max_length=20, choices=[
        ('HIGH', _('Yüksek')),
        ('MEDIUM', _('Orta')),
        ('LOW', _('Düşük'))
    ], default='MEDIUM', verbose_name=_('Güven Seviyesi'))
