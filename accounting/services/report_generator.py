...
class ReportPermissionChecker:
    ...
    def can_access_report(user, company, report_type):
        ...
        report_permissions = {
            'bilanco': ['finance_manager', 'accountant', 'muhasebe_elemani', 'kobi_owner'],
            'gelir_tablosu': ['finance_manager', 'accountant', 'muhasebe_elemani', 'kobi_owner'],
            'nakit_akisi': ['finance_manager', 'accountant', 'muhasebe_elemani', 'kobi_owner'],
            'mizan': ['finance_manager', 'accountant', 'muhasebe_elemani', 'kobi_owner'],
            'yevmiye': ['finance_manager', 'accountant', 'muhasebe_elemani', 'kobi_owner'],
            'kebir': ['finance_manager', 'accountant', 'muhasebe_elemani', 'kobi_owner'],
            'kdv': ['finance_manager', 'accountant', 'muhasebe_elemani', 'kobi_owner'],
            'muhtasar': ['finance_manager', 'accountant', 'muhasebe_elemani', 'kobi_owner'],
    ...
    def can_export_report(user, company, report_type):
        ...
        user_groups = user.groups.values_list('name', flat=True)
        export_roles = ['finance_manager', 'accountant', 'muhasebe_elemani', 'kobi_owner']
...
class KVKKDataProtection:
    ...
    @staticmethod
    def anonymize_personal_data(data: Dict[str, Any], fields_to_mask: List[str] = None) -> Dict[str, Any]:
        """Kişisel verileri anonimleştir"""
        if fields_to_mask is None:
            fields_to_mask = [
                'tc_kimlik_no', 'tax_number', 'phone', 'email', 
                'address', 'iban', 'account_number', 'customer_name',
...
class TFRSReportGenerator:
    ...
    def generate_balance_sheet(self, as_of_date: date, comparative: bool = False) -> Dict[str, Any]:
        """
    ...
    def generate_income_statement(self, start_date: date, end_date: date) -> Dict[str, Any]:
        ...
        # Vergi öncesi kar
        profit_before_tax = operating_profit + financial_income - financial_expense + other_income - other_expense
        
        # Gelir vergisi (tahmini %20)
        income_tax_rate = Decimal('0.20')
        income_tax = profit_before_tax * income_tax_rate if profit_before_tax > 0 else Decimal('0')
    ...
    def generate_cash_flow_statement(self, start_date: date, end_date: date) -> Dict[str, Any]:
        """
        ...
        # Net nakit artışı
        net_cash_increase = operating_cash_flow + investment_cash_flow + financing_cash_flow
        ...
        for account in accounts:
            ...
            if debit_total != 0 or credit_total != 0:
                rows.append({
                    ...
                    'name': account.name,
                    'category': account.category,
                    'begin_balance': balance.begin_balance if balance else Decimal('0'),
    ...
    def generate_journal_ledger(self, start_date: date, end_date: date) -> Dict[str, Any]:
        """
        Yevmiye Defteri (VUK Madde 64)
        Tarih sırasına göre tüm muhasebe kayıtları
        """
        entries = GLJournalEntry.objects.filter(
            company=self.company,
            date__gte=start_date,
            date__lte=end_date,
            state='posted'
        ).order_by('date', 'number').select_related('company')
    ...
    def generate_general_ledger(self, start_date: date, end_date: date) -> Dict[str, Any]:
        """
        ...
        for account in accounts:
            ...
            if lines.exists():
                ...
                ledger_data.append({
                    ...
                    'begin_balance': Decimal('0'),  # TODO: Dönem başı bakiyesi hesapla
                    'lines': account_lines,
                    'debit_total': debit_total,
                    'credit_total': credit_total,
                    'end_balance': debit_total - credit_total,
    ...
    def export_to_pdf(report_data: Dict[str, Any], report_type: str, template_name: str) -> HttpResponse:
        ...
        try:
            ...
            if report_type == 'bilanco':
                elements.extend(ReportExportService._build_balance_sheet_table(report_data, styles))
            elif report_type == 'gelir_tablosu':
                elements.extend(ReportExportService._build_income_statement_table(report_data, styles))
            elif report_type == 'mizan':
                elements.extend(ReportExportService._build_trial_balance_table(report_data, styles))
            else:
                # Genel tablo
    ...
    def send_report_email(
        ...
        try:
            ...
            html_content = render_to_string(
                'accounting/emails/report_email.html',
                context
            )
            ...
            logger.error(
                f"Rapor e-posta gönderme hatası: {e}",
                exc_info=True
            )
            return False
