    ...
    def get_queryset(self):
        ...
        if search:
            queryset = queryset.filter(
                Q(voucher_number__icontains=search) | Q(description__icontains=search)
            )
...
def voucher_create(request):
    ...
    if request.method == "POST":
        ...
        if voucher_form.is_valid() and entry_formset.is_valid():
            try:
                with transaction.atomic():
                    ...
                    if not voucher.voucher_number:
                        voucher.voucher_number = generate_voucher_number(
                            company, voucher.voucher_type
                        )
                    ...
                    for entry_form in entry_formset:
                        if entry_form.cleaned_data and not entry_form.cleaned_data.get(
                            "DELETE", False
                        ):
                    ...
                    if request.POST.get("draft"):
                        ...
                        return JsonResponse(
                            {"success": True, "message": str(_("Taslak kaydedildi"))}
                        )
            ...
            messages.error(
                request, _("Form verilerinde hata var. Lütfen kontrol edin.")
            )
    else:
        voucher_form = JournalVoucherForm()
        entry_formset = JournalEntryFormSet(
            prefix="entries", queryset=JournalEntry.objects.none()
        )

    # Hesap listesi
    accounts = (
        ChartOfAccounts.objects.filter(company=company, is_active=True).order_by("code")
        if company
        else []
    )
...
def fixed_assets_view(request):
    ...
    total_depreciation = (
        DepreciationEntry.objects.filter(asset__company=company).aggregate(
            total=Sum("amount")
        )["total"]
        or 0
    )
...
def ajax_search_accounts(request):
    ...
    accounts = (
        ChartOfAccounts.objects.filter(company=company, is_active=True)
        ...
        account_list.append(
            {
                ...
                "account_type": getattr(
                    account, "get_account_type_display", lambda: ""
                )(),
            }
        )
    ...
    class Meta:
        ...
        widgets = {
            ...
            "voucher_number": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Otomatik oluşturulur"}
            ),
    ...
    def clean(self):
        ...
        if debit > 0 and credit > 0:
            raise forms.ValidationError(
                _("Aynı kayıtta hem borç hem alacak tutar girilemez.")
            )

        # En az birinde tutar olmalı
        if debit == 0 and credit == 0:
            raise forms.ValidationError(
                _("Borç veya alacak tutarlarından en az birini girmelisiniz.")
            )
...
def generate_voucher_number(company, voucher_type):
    ...
    last_voucher = (
        JournalVoucher.objects.filter(
            company=company, voucher_type=voucher_type, date__year=year
        )
...
def calculate_account_balance(account):
    ...
    credit_total = entries.aggregate(total=Sum("credit_amount"))["total"] or Decimal(
        "0"
    )
...
def trial_balance(request):
    ...
    accounts = ChartOfAccounts.objects.filter(company=company, is_active=True).order_by(
        "code"
    )
    ...
    for account in accounts:
        ...
        entries = JournalEntry.objects.filter(
            account=account, voucher__date__lte=date_to
        )

        debit_sum = entries.aggregate(total=Sum("debit_amount"))["total"] or Decimal(
            "0"
        )
        credit_sum = entries.aggregate(total=Sum("credit_amount"))["total"] or Decimal(
            "0"
        )
