...
def company_detail(request: HttpRequest, slug: str) -> HttpResponse:
    ...
    delete_logs = CompanyDeleteLog.objects.filter(company=company).order_by(
        "-deleted_at"
    )[:5]
    ai_summary = request.GET.get(
        "ai_summary"
    )  # Geçici olarak, AI özetini context'e ekle
    return render(
        request,
        "accounting/company_detail.html",
        {
            "company": company,
...
def company_update(request: HttpRequest, slug: str) -> HttpResponse:
    ...
    return render(
        request, "accounting/company_form.html", {"form": form, "company": company}
    )
...
def company_delete(request: HttpRequest, slug: str) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if cache.get(cache_key):
            messages.error(
                request, "Çok hızlı silme işlemi! Lütfen 1 dakika sonra tekrar deneyin."
            )
            return redirect("accounting:company_list")
        if not reason:
            messages.error(request, "Silme gerekçesi zorunludur.")
            return render(
                request, "accounting/company_confirm_delete.html", {"company": company}
            )
        company.is_active = False
        company.save()
        CompanyDeleteLog.objects.create(
            company=company, user=request.user, reason=reason
        )
        cache.set(cache_key, True, timeout=60)  # 1 dakika
        messages.success(
            request,
            "Şirket başarıyla pasif hale getirildi ve silme gerekçesi kaydedildi.",
        )
        return redirect("accounting:company_list")
    return render(
        request, "accounting/company_confirm_delete.html", {"company": company}
