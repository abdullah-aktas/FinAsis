...
def tfrs_balance_sheet_view(request: HttpRequest) -> HttpResponse:
    ...
    KVKKDataProtection.create_data_access_log(
        request.user, company, "bilanco", ["balance_sheet_data", "company_financials"]
    )
    ...
    if action == "pdf":
        if not ReportPermissionChecker.can_export_report(
            request.user, company, "bilanco"
        ):
            messages.error(
                request, _("Bu raporu dışa aktarma yetkiniz bulunmamaktadır.")
            )
            return redirect(request.path)
        return ReportExportService.export_to_pdf(
            report_data, "bilanco", "accounting/reports/balance_sheet_pdf.html"
        )
        ...
        return render(
            request,
            "accounting/reports/balance_sheet_print.html",
            {
        ...
        if not ReportPermissionChecker.can_send_report(
            request.user, company, "bilanco"
        ):
        ...
        pdf_response = ReportExportService.export_to_pdf(
            report_data, "bilanco", "accounting/reports/balance_sheet_pdf.html"
        )
        ...
        success = ReportExportService.send_report_email(
            request.user, company, report_data, "bilanco", recipients, pdf_buffer
        )
    ...
    context = {
        ...
        "companies": companies,
        "as_of_date": as_of_date,
        "can_export": ReportPermissionChecker.can_export_report(
            request.user, company, "bilanco"
        ),
        "can_send": ReportPermissionChecker.can_send_report(
            request.user, company, "bilanco"
        ),
    }
...
def tfrs_income_statement_view(request: HttpRequest) -> HttpResponse:
    ...
    if not ReportPermissionChecker.can_access_report(
        request.user, company, "gelir_tablosu"
    ):
        messages.error(request, _("Bu rapora erişim yetkiniz bulunmamaktadır."))
        return redirect("accounting:home")
    ...
    if KVKKDataProtection.should_anonymize_report(
        request.user, company, "gelir_tablosu"
    ):
        report_data = KVKKDataProtection.anonymize_personal_data(report_data)
    ...
    if action == "pdf":
        if not ReportPermissionChecker.can_export_report(
            ...
            return redirect(request.path)
        return ReportExportService.export_to_pdf(
            report_data, "gelir_tablosu", "accounting/reports/income_statement_pdf.html"
        )
        ...
        return render(
            request,
            "accounting/reports/income_statement_print.html",
            {
                "report_data": report_data,
                "company": company,
        ...
        success = ReportExportService.send_report_email(
            request.user, company, report_data, "gelir_tablosu", recipients, pdf_buffer
        )
...
def tfrs_trial_balance_view(request: HttpRequest) -> HttpResponse:
    ...
    KVKKDataProtection.create_data_access_log(
        request.user, company, "mizan", ["trial_balance_data", "account_balances"]
    )

    if action == "pdf":
        if not ReportPermissionChecker.can_export_report(
            request.user, company, "mizan"
        ):
            messages.error(
                request, _("Bu raporu dışa aktarma yetkiniz bulunmamaktadır.")
            )
            return redirect(request.path)
        return ReportExportService.export_to_pdf(
            report_data, "mizan", "accounting/reports/trial_balance_pdf.html"
        )
        ...
        return render(
            request,
            "accounting/reports/trial_balance_print.html",
            {
                "report_data": report_data,
                "company": company,
                "as_of_date": as_of_date,
        ...
        success = ReportExportService.send_report_email(
            request.user, company, report_data, "mizan", recipients, pdf_buffer
        )
