...
from ..services.efatura_service import send_invoice_to_gib, check_invoice_status, cancel_invoice_on_gib, generate_invoice_xml
import json
...
def invoice_list(request: HttpRequest) -> HttpResponse:
    ...
    user_companies = request.user.created_companies.all() if hasattr(request.user, 'created_companies') else Company.objects.none()
    
    # Base queryset
    invoices = Invoice.objects.filter(is_active=True, company__in=user_companies).select_related('company', 'customer')
...
def api_invoice_bulk_action(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        user_companies = request.user.created_companies.all() if hasattr(request.user, 'created_companies') else Company.objects.none()
        invoices = Invoice.objects.filter(id__in=invoice_ids, company__in=user_companies)
