...
class CompanyAdmin(AuditFieldsMixin, admin.ModelAdmin):
    ...
    def is_active_colored(self, obj):
        if not obj.is_active:
            return format_html(
                '<span style="color: #888; background: #eee; padding:2px 8px; border-radius:4px;">Pasif</span>'
            )
    ...
    def archive_companies(self, request, queryset):
        ...
        for obj in queryset:
            ...
            if obj.is_active:
                CompanyDeleteLog.objects.create(
                    company=obj, user=request.user, reason="Admin bulk soft delete"
                )
                obj.is_active = False
    ...
    def delete_model(self, request, obj):
        ...
        try:
            CompanyDeleteLog.objects.create(
                company=obj, user=request.user, reason="Admin soft delete"
            )
        except Exception:
            pass
        obj.is_active = False
...
class PaymentAdmin(AuditFieldsMixin, admin.ModelAdmin):
    ...
    search_fields = (
        ...
        "customer__last_name",
        "related_invoice__invoice_number",
    )
    ...
    def action_generate_attach(self, request, queryset):
        ...
        for ed in queryset:
            try:
                ...
                self.message_user(
                    request, _(f"{ed.pk} üretim hatası: {e}"), level="ERROR"
                )
        if ok:
            self.message_user(
                request,
                _(f"{ok} kayıt için Yevmiye ve Berat üretildi ve iliştirildi."),
                level="SUCCESS",
            )
        ...
        for ed in queryset:
            try:
                ...
                zip_bytes = package_edefter_zip(
                    ...
                    ed.year,
                    ed.month,
                    include_signed=bool(getattr(request, "include_signed", False)),
                )
                ...
                self.message_user(
                    request, _(f"{ed.pk} ZIP üretim hatası: {e}"), level="ERROR"
                )
        if ok:
            self.message_user(
                request, _(f"{ok} kayıt için ZIP üretildi."), level="SUCCESS"
            )
    ...
    def action_send_gib(self, request, queryset):
        ...
        for ed in queryset:
            try:
                ...
                self.message_user(
                    request, _(f"{ed.pk} gönderim hatası: {e}"), level="ERROR"
                )
        if ok:
            self.message_user(
                request, _(f"{ok} kayıt GİB'e gönderildi."), level="SUCCESS"
            )
    ...
    def action_get_berat(self, request, queryset):
        ...
        for ed in queryset:
            try:
                ...
                self.message_user(
                    request, _(f"{ed.pk} berat alma hatası: {e}"), level="ERROR"
                )
        if ok:
            self.message_user(
                request, _(f"{ok} kayıt için berat alındı."), level="SUCCESS"
            )
