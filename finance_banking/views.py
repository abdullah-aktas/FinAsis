...
from .models import BankAccount, BankTransaction, BankReconciliation, PaymentOrder
...
def banking_dashboard(request):
    ...
    # Bekleyen ödemeler
    pending_payments = PaymentOrder.objects.filter(company=company, status="PENDING")
...
def transaction_list(request):
    ...
    transactions = (
        BankTransaction.objects.filter(account__company=company)
...
def reconciliation_list(request):
    ...
    reconciliations = (
        BankReconciliation.objects.filter(account__company=company)
        .select_related("account")
...
def payment_order_list(request):
    ...
    payment_orders = PaymentOrder.objects.filter(company=company).order_by(
        "-created_at"
    )
...
def ajax_account_balance(request, account_id):
    ...
    try:
        company = getattr(request.user, "company", None)
        account = BankAccount.objects.get(id=account_id, company=company)
        ...
        return JsonResponse(
            {"success": False, "error": _("Hesap bulunamadı")}, status=404
        )
...
def ajax_sync_transactions(request, account_id):
    ...
    if request.method != "POST":
        return JsonResponse(
            {"success": False, "error": _("Sadece POST istekleri kabul edilir")},
            status=405,
        )
...
def ajax_dashboard_stats(request):
    ...
    if not company:
        return JsonResponse(
            {"success": False, "error": _("Şirket bilgisi bulunamadı")}, status=400
        )
    ...
    stats = {
        ...
        "recent_transaction_count": BankTransaction.objects.filter(
            account__company=company, transaction_date__gte=timezone.now().date()
        ).count(),
    }
