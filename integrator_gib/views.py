...
from .models import IntegratorConfig, GIBCertificate, GIBSubmissionLog
...
def gib_dashboard(request):
    ...
    pending_submissions = GIBSubmissionLog.objects.filter(status="PENDING")
...
def ajax_test_connection(request):
    ...
    try:
        ...
        if config.is_active:
            ...
            return JsonResponse(
                {"success": False, "error": _("GIB entegrasyonu aktif değil.")}
            )

    except IntegratorConfig.DoesNotExist:
        return JsonResponse(
            {"success": False, "error": _("GIB yapılandırması bulunamadı.")}, status=404
        )
...
def ajax_retry_submission(request, submission_id):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "error": _("Gönderim bulunamadı.")}, status=404
        )
