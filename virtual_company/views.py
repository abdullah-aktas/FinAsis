...
from .serializers import (
    VirtualCompanySerializer,
    ProductSerializer,
    ARAccountingEntrySerializer,
    ARCompanySerializer,
    ARProductSerializer,
)
...
class VirtualCompanyViewSet(viewsets.ModelViewSet):
    ...
    def add_product(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            print(
                f"AUDIT: {request.user} {company} şirketine ürün ekledi: {product.name}"
            )
        ...
        for item in items:
            ...
            if serializer.is_valid():
                ...
                if data["price"] < 0 or data["stock"] < 0:
                    errors.append(
                        {"item": item, "error": _("Fiyat veya stok negatif olamaz.")}
                    )
                    continue
                product = serializer.save(company=company)
...
class VirtualCompanyListView(OwnerQuerysetMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if companies and hasattr(companies[0], "total_inventory_annotated"):
            total_inventory = sum(
                (c.total_inventory_annotated or 0 for c in companies), start=0
            )
        else:
            total_inventory = sum(
