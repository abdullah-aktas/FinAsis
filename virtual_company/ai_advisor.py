...
class VirtualCompanyAIAdvisor:
    ...
    def analyze_company_health(cls, company) -> Dict[str, Any]:
        ...
        # 4. İşlem geçmişi
        transaction_count = company.transactions.count() if hasattr(company, 'transactions') else 0
    ...
    @classmethod
    def suggest_accounting_entry(cls, company, transaction_type: str, amount: Decimal, description: str) -> Dict[str, Any]:
        """
        ...
        suggestion['effect_on_balance'] = 'POSITIVE' if transaction_type == 'INCOME' else 'NEGATIVE'
        
        # Alternatif öneriler
        suggestion['alternatives'] = cls._get_alternative_entries(transaction_type, description)
        
        return suggestion
    
    @classmethod
    def _get_alternative_entries(cls, transaction_type: str, description: str) -> List[Dict]:
        """Alternatif muhasebe kayıtları öner"""
        alternatives = []
        
        if transaction_type == 'INCOME':
            alternatives = [
                {'debit': '100 - Kasa', 'credit': '600 - Satış Gelirleri', 'note': 'Nakit satış'},
                {'debit': '120 - Alıcılar', 'credit': '600 - Satış Gelirleri', 'note': 'Vadeli satış'},
                {'debit': '102 - Bankalar', 'credit': '600 - Satış Gelirleri', 'note': 'Banka transferi'},
            ]
        else:
            alternatives = [
                {'debit': '770 - Genel Gider', 'credit': '100 - Kasa', 'note': 'Nakit ödeme'},
                {'debit': '770 - Genel Gider', 'credit': '320 - Satıcılar', 'note': 'Vadeli ödeme'},
                {'debit': '770 - Genel Gider', 'credit': '102 - Bankalar', 'note': 'Banka ödemesi'},
            ]
    ...
    def generate_learning_path(cls, student_profile) -> List[Dict[str, Any]]:
        ...
        """
        completed_modules = student_profile.completed_modules.all() if hasattr(student_profile, 'completed_modules') else []
