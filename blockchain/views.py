...
from .models import (
    ChainRecord, Block, Transaction, SmartContract,
    DigitalAsset, AssetBalance, AuditLog
)
from .services import (
    BlockchainManager, TransactionManager, SmartContractManager,
    AssetManager, compute_sha256_hex
)
import hashlib
import re
import json
...
def home(request):
    ...
    # Son bloklar
    latest_blocks = Block.objects.select_related('mined_by').order_by('-block_number')[:5]
    
    # Son transaction'lar
    latest_transactions = Transaction.objects.select_related('block', 'created_by').order_by('-timestamp')[:10]
...
def transaction_list(request):
    """Transaction listesi"""
    transactions = Transaction.objects.select_related('block', 'created_by').order_by('-timestamp')
...
def transaction_create(request):
    ...
    if request.method == 'POST':
        try:
            ...
            return redirect('blockchain:transaction_detail', transaction_id=tx.transaction_id)
        except Exception as e:
            return render(request, 'blockchain/transaction_create.html', {
                'error': str(e),
                'tx_types': Transaction.TRANSACTION_TYPES
            })
...
def contract_list(request):
    ...
    contracts = SmartContract.objects.select_related('deployed_by').order_by('-deployed_at')
    ...
    return render(request, 'blockchain/contract_list.html', {
        'contracts': contracts,
...
def contract_detail(request, contract_address):
    ...
    return render(request, 'blockchain/contract_detail.html', {
        'contract': contract
    })
...
def contract_deploy(request):
    ...
    if request.method == 'POST':
        try:
            ...
            return redirect('blockchain:contract_detail', contract_address=contract.contract_address)
        except Exception as e:
            return render(request, 'blockchain/contract_deploy.html', {
                'error': str(e),
                'contract_types': SmartContract.CONTRACT_TYPES
            })
...
def asset_list(request):
    ...
    return render(request, 'blockchain/asset_list.html', {
        'assets': assets,
...
def asset_detail(request, asset_id):
    ...
    # Top holders
    top_holders = AssetBalance.objects.filter(asset=asset).select_related('user').order_by('-balance')[:10]
    
    return render(request, 'blockchain/asset_detail.html', {
        'asset': asset,
...
def asset_create(request):
    ...
    if request.method == 'POST':
        try:
            ...
            return render(request, 'blockchain/asset_create.html', {
                'error': str(e),
                'asset_types': DigitalAsset.ASSET_TYPES
            })
...
def my_assets(request):
    ...
    balances = AssetBalance.objects.filter(user=request.user).select_related('asset').order_by('-balance')
    
    return render(request, 'blockchain/my_assets.html', {
        'balances': balances
    })
