...
from .models import (
    Block, Transaction, SmartContract, DigitalAsset,
    AssetBalance, AuditLog, ChainRecord
)


@admin.register(Block)
class BlockAdmin(admin.ModelAdmin):
    list_display = ['block_number', 'block_hash_short', 'transactions_count', 'mined_by', 'timestamp', 'is_valid']
...
class TransactionAdmin(admin.ModelAdmin):
    list_display = ['transaction_id_short', 'transaction_type', 'amount', 'status', 'block', 'timestamp']
...
class SmartContractAdmin(admin.ModelAdmin):
    list_display = ['contract_name', 'contract_type', 'deployed_by', 'execution_count', 'is_active', 'deployed_at']
...
class DigitalAssetAdmin(admin.ModelAdmin):
    list_display = ['asset_name', 'asset_symbol', 'asset_type', 'total_supply', 'owner', 'created_at']
...
class ChainRecordAdmin(admin.ModelAdmin):
    list_display = ['reference', 'hash_hex_short', 'status', 'transaction', 'created_at']
    list_filter = ['status']
