...
def hash_invoice(sender, instance: Invoice, created, **kwargs):
    ...
    ensure_record(
        ...
        payload=payload,
        status="anchored" if getattr(instance, "gib_uuid", None) else "pending",
    )
...
def hash_voucher(sender, instance: Voucher, created, **kwargs):
    ...
    ensure_record(
        reference=f"voucher:{instance.id}",
        payload=payload,
        status="anchored" if instance.state == "posted" else "pending",
    )
...
def hash_payment(sender, instance: Payment, created, **kwargs):
    ...
    ensure_record(
        reference=f"payment:{instance.id}", payload=payload, status="anchored"
    )
...
def hash_expense(sender, instance: Expense, created, **kwargs):
    ...
    ensure_record(
        reference=f"expense:{instance.id}", payload=payload, status="anchored"
    )
...
def hash_bank_transaction(sender, instance: BankTransaction, created, **kwargs):
    ...
    ensure_record(
        reference=f"banktxn:{instance.id}", payload=payload, status="anchored"
    )
