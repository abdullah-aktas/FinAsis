...
class BlockchainManager:
    ...
    def create_new_block(transactions: list = None, mined_by=None, difficulty=4) -> Block:
        ...
        if transactions is None:
            transactions = list(Transaction.objects.filter(
                status='pending',
                block__isnull=True
            )[:100])  # Blok başına max 100 transaction
...
class TransactionManager:
    ...
    def create_transaction(
        ...
        tx_id = compute_sha256_hex(f"{timezone.now().isoformat()}{from_address}{to_address}{amount}")
...
class SmartContractManager:
    ...
    @staticmethod
    def execute_contract(contract_address: str, execution_params: dict) -> Dict[str, Any]:
