...
from blockchain.services import ensure_record
import logging
...
def anchor_submission_log(sender, instance: SubmissionLog, created, **kwargs):
    ...
    sub_id = getattr(instance, "submission_id", None) or (
        instance.submission.pk if getattr(instance, "submission", None) else ""
    )
    ...
    ts = (
        instance.created_at.isoformat() if getattr(instance, "created_at", None) else ""
    )
    ...
    try:
        ...
        logger.warning(
            "SubmissionLog anchoring failed: %s",
            e,
            extra={
                "ref": ref,
