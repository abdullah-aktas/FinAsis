...
class Declaration(models.Model):
    ...
    payload = models.JSONField(
        default=dict
    )  # normalized data to render XML/JSON for GIB
    ...
    class Meta:
        indexes = [models.Index(fields=["code", "period", "taxpayer_vkn_tckn"])]


class Submission(models.Model):
    TARGETS = (("gib", "GIB"),)
    declaration = models.ForeignKey(
        Declaration, on_delete=models.CASCADE, related_name="submissions"
    )
...
class SubmissionTemplate(models.Model):
    ...
    code = models.CharField(
        max_length=50, verbose_name="Beyan Kodu", help_text="KDV1, BA-BS, Muhtasar, vs."
    )
    ...
    default_values = models.JSONField(
        default=dict, blank=True, verbose_name="Varsayılan Değerler"
    )
    validation_rules = models.JSONField(
        default=dict, blank=True, verbose_name="Doğrulama Kuralları"
    )
...
class SubmissionAttachment(models.Model):
    ...
    file_type = models.CharField(
        max_length=20, choices=ATTACHMENT_TYPES, verbose_name="Dosya Tipi"
    )
    ...
    uploaded_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, verbose_name="Yükleyen"
    )
...
class SubmissionApproval(models.Model):
    ...
    status = models.CharField(
        max_length=30, choices=STATUS_CHOICES, default="PENDING", verbose_name="Durum"
    )
    ...
    responded_at = models.DateTimeField(
        null=True, blank=True, verbose_name="Yanıt Tarihi"
    )
