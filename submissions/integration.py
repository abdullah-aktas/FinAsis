...
try:
    ...
    class GibClient:  # fallback minimal client
        def send_invoice(self, xml: bytes | str, idempotency_key: str | None = None):
            ...
            return _DummyResult(
                tracking_id=f"TEST-{uuid.uuid4().hex[:8]}", status="PENDING"
            )
...
def submit_via_gib(submission: Submission) -> Tuple[str, str]:
    ...
    if getattr(settings, "EDOC_CUSTOMIZATION_ID", None):
        os.environ.setdefault(
            "EDOC_CUSTOMIZATION_ID", str(settings.EDOC_CUSTOMIZATION_ID)
        )
    ...
    if getattr(settings, "EDOC_RETRY_MAX_BACKOFF", None):
        os.environ.setdefault(
            "EDOC_RETRY_MAX_BACKOFF", str(settings.EDOC_RETRY_MAX_BACKOFF)
        )

    # Şema doğrulaması (varsa) – başarısızlıkları log'la ama akışı engelleme
    schema_dir = (
        os.environ.get("EDOC_SCHEMAS_DIR")
        or getattr(settings, "EDOC_SCHEMAS_DIR", "")
        or ""
    )
    try:
    ...
    if status == "PENDING":
        ...
        try:
            max_tries = int(
                os.environ.get("EDOC_RETRY_MAX")
                or getattr(dj_settings, "EDOC_RETRY_MAX", 3)
            )
        except Exception:
            max_tries = 3
        try:
            backoff = float(
                os.environ.get("EDOC_RETRY_BACKOFF")
                or getattr(dj_settings, "EDOC_RETRY_BACKOFF", 0.7)
            )
        except Exception:
            backoff = 0.7
        try:
            max_backoff = float(
                os.environ.get("EDOC_RETRY_MAX_BACKOFF")
                or getattr(dj_settings, "EDOC_RETRY_MAX_BACKOFF", 8.0)
            )
        except Exception:
            max_backoff = 8.0
