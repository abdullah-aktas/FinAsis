...
class ARTradeTrail(BaseGame):
    ...
    def render_main_menu(self, surface):
        ...
        # Butonlar
        self.ui_elements["start_button"] = pygame.Rect(
            self.frame_w // 2 - 100, self.frame_h // 2 - 50, 200, 50
        )
        ...
        self.ui_elements["education_button"] = pygame.Rect(
            self.frame_w // 2 - 100, self.frame_h // 2 + 20, 200, 50
        )
        ...
        self.ui_elements["exit_button"] = pygame.Rect(
            self.frame_w // 2 - 100, self.frame_h // 2 + 90, 200, 50
        )
    ...
    def render_game_ui(self, surface):
        ...
        money_text = self.font.render(
            f"Para: {self.game_state['player']['money']} ₺", True, (255, 255, 255)
        )
        surface.blit(money_text, (20, 15))

        level_text = self.font.render(
            f"Seviye: {self.game_state['player']['business_level']}",
            True,
            (255, 255, 255),
        )
        ...
        day_text = self.font.render(
            f"Gün: {self.game_state['current_day']}", True, (255, 255, 255)
        )
        ...
        for marker_name, marker_data in self.markers.items():
            if marker_data["detected"]:
                ...
                pygame.draw.rect(
                    ...
                    (0, 255, 0),
                    (
                        marker_data["position"][0] - 40,
                        marker_data["position"][1] - 40,
                        80,
                        80,
                    ),
                    3,
                ...
                # İşaretçi adı
                marker_text = self.font_small.render(
                    marker_name.upper(), True, (255, 255, 0)
                )
                surface.blit(
                    marker_text,
                    (marker_data["position"][0] - 30, marker_data["position"][1] - 60),
                )

                # İşaretçi etkileşim butonu
                interact_button = pygame.Rect(
                    marker_data["position"][0] - 45,
                    marker_data["position"][1] + 50,
                    90,
                    30,
                )
                pygame.draw.rect(surface, (0, 100, 200), interact_button)
                interact_text = self.font_small.render(
                    "ETKİLEŞİM", True, (255, 255, 255)
                )
                surface.blit(
                    interact_text,
                    (marker_data["position"][0] - 40, marker_data["position"][1] + 55),
                )
    ...
    def render_education_ui(self, surface):
        ...
        topic_title = self.font.render(
            current_topic.replace("_", " ").upper(), True, (255, 220, 0)
        )
        ...
        if not self.game_state["education_complete"]:
            quiz_button = pygame.Rect(
                self.frame_w // 2 - 80, self.frame_h - 150, 160, 40
            )
    ...
    def render_market_ui(self, surface):
        ...
        for product, data in self.game_state["market"]["products"].items():
            # Ürün adı ve fiyatı
            product_text = self.font.render(
                f"{product}: {data['price']} ₺", True, (255, 255, 255)
            )
            ...
            count_text = self.font_small.render(
                f"Envanter: {count}", True, (255, 255, 0)
            )
            ...
            pygame.draw.rect(
                surface, (150, 150, 150), (700, y_pos + 5, 100, 15)
            )  # Arka plan
            pygame.draw.rect(
                surface, (0, 0, 200), (700, y_pos + 5, demand_width, 15)
            )  # Talep çubuğu
    ...
    def handle_events(self):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                # Ana menü ekranı
                if self.current_screen == "main_menu":
                    if self.ui_elements.get("start_button") and self.ui_elements[
                        "start_button"
                    ].collidepoint(event.pos):
                        self.current_screen = "game"
                    elif self.ui_elements.get("education_button") and self.ui_elements[
                        "education_button"
                    ].collidepoint(event.pos):
                        self.current_screen = "education"
                    elif self.ui_elements.get("exit_button") and self.ui_elements[
                        "exit_button"
                    ].collidepoint(event.pos):
                        self.running = False
                ...
                # Oyun ekranı
                    ...
                    if self.ui_elements.get("menu_button") and self.ui_elements[
                        "menu_button"
                    ].collidepoint(event.pos):
                    ...
                    for marker_name in self.markers:
                        if self.ui_elements.get(
                            f"interact_{marker_name}"
                        ) and self.ui_elements[f"interact_{marker_name}"].collidepoint(
                            event.pos
                        ):
                ...
                # Eğitim ekranı
                    ...
                    if self.ui_elements.get(
                        "education_exit_button"
                    ) and self.ui_elements["education_exit_button"].collidepoint(
                        event.pos
                    ):
                        self.current_screen = "main_menu"
                    elif self.ui_elements.get("quiz_button") and self.ui_elements[
                        "quiz_button"
                    ].collidepoint(event.pos):
                ...
                # Market ekranı
                    ...
                    if self.ui_elements.get("market_exit_button") and self.ui_elements[
                        "market_exit_button"
                    ].collidepoint(event.pos):
                        self.current_screen = "game"
                    ...
                    for product in self.game_state["market"]["products"]:
                        # Alış
                        if self.ui_elements.get(f"buy_{product}") and self.ui_elements[
                            f"buy_{product}"
                        ].collidepoint(event.pos):
                            self.buy_product(product)
                        # Satış
                        elif self.ui_elements.get(
                            f"sell_{product}"
                        ) and self.ui_elements[f"sell_{product}"].collidepoint(
                            event.pos
                        ):
                ...
                # Banka ekranı
                    ...
                    if self.ui_elements.get("bank_exit_button") and self.ui_elements[
                        "bank_exit_button"
                    ].collidepoint(event.pos):
                        self.current_screen = "game"
                    ...
                    for i in range(3):  # 3 banka işlemi için
                        if self.ui_elements.get(
                            f"bank_action_{i}"
                        ) and self.ui_elements[f"bank_action_{i}"].collidepoint(
                            event.pos
                        ):
    ...
    def buy_product(self, product):
        ...
        if self.game_state["player"]["money"] >= price:
            ...
            self.show_education_tip(
                "Alış İşlemi",
                [
                    f"{product} satın aldınız ve bu bir BORÇ kaydıdır.",
                    "Muhasebede: Mal Hesabı (+) | Kasa Hesabı (-)",
                    "Bu işlem envanterinizi arttırır ve nakit paranızı azaltır.",
                ],
            )
        else:
            self.show_education_tip(
    ...
    def sell_product(self, product):
        ...
        if (
            product in self.game_state["player"]["inventory"]
            and self.game_state["player"]["inventory"][product] > 0
        ):
            ...
            self.show_education_tip(
                ...
                [
                    ...
                    "Muhasebede: Kasa Hesabı (+) | Satılan Mal Maliyeti (-)",
                    "Bu işlem nakit paranızı arttırır ve envanterinizi azaltır.",
    ...
    def show_education_tip(self, title, messages):
        ...
        self.screen.blit(
            tip_surface, (self.frame_w // 2 - 300, self.frame_h // 2 - 100)
        )
