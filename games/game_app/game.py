...
class Game(BaseGame):
    def __init__(self):
        ...
        self.game_state = {
            "player": {
                ...
                "skills": {"trading": 1, "accounting": 1, "management": 1},
            },
            "market": {"products": self.load_products(), "prices": {}, "demand": {}},
            "time": {"day": 1, "month": 1, "year": 2024},
        }
    ...
    def handle_click(self, pos):
        ...
        if self.current_screen == "main_menu":
            ...
            if self.ui_elements["company_setup"]["name_input"].collidepoint(pos):
                ...
                self.starting_capital = int(
                    5000 + (rel_x / 400) * 45000
                )  # 5000-50000 arası
            ...
            for product in self.game_state["market"]["prices"].keys():
                ...
                if f"buy_{product}" in self.ui_elements and self.ui_elements[
                    f"buy_{product}"
                ].collidepoint(pos):
                    self.buy_product(product)
                # Satım butonu kontrolü
                elif f"sell_{product}" in self.ui_elements and self.ui_elements[
                    f"sell_{product}"
                ].collidepoint(pos):
    ...
    def draw_company_setup(self):
        ...
        pygame.draw.rect(
            self.screen,
            LIGHT_BLUE if self.active_input_field == "name" else GRAY,
        ...
        pygame.draw.rect(
            self.screen,
            LIGHT_BLUE if self.active_input_field == "location" else GRAY,
        ...
        pygame.draw.rect(
            self.screen, GRAY, self.ui_elements["company_setup"]["capital_slider"]
        )
        slider_pos = ((self.starting_capital - 5000) / 45000) * 400
        pygame.draw.rect(
            self.screen, BLUE, (SCREEN_WIDTH // 2 - 200 + slider_pos - 5, 485, 10, 30)
        )
        ...
        pygame.draw.rect(
            self.screen, GREEN, self.ui_elements["company_setup"]["start_button"]
        )
        start_text = font.render("Oyunu Başlat", True, BLACK)
        start_rect = start_text.get_rect(
            center=self.ui_elements["company_setup"]["start_button"].center
        )
        self.screen.blit(start_text, start_rect)

        pygame.draw.rect(
            self.screen, RED, self.ui_elements["company_setup"]["back_button"]
        )
        back_text = font.render("Geri", True, BLACK)
        back_rect = back_text.get_rect(
            center=self.ui_elements["company_setup"]["back_button"].center
        )
    ...
    def draw_game(self):
        ...
        for category, products in self.game_state["market"]["products"].items():
            ...
            for product, data in products.items():
                current_price = self.game_state["market"]["prices"].get(
                    product, data["base_price"]
                )
                product_text = font.render(
                    f"{product}: {current_price:,.0f} TL", True, BLACK
                )
    ...
    def sell_product(self, product):
        """Ürün sat"""
        if (
            product not in self.game_state["player"]["inventory"]
            or self.game_state["player"]["inventory"][product] <= 0
        ):
