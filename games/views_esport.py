...
from .models import (
    Tournament, TournamentParticipant, Match, PlayerProfile,
    DailyQuest, PlayerQuest, Item, PlayerInventory, 
    Leaderboard, Friend, Team, GameSession, Season, Badge
)
...
def tournaments_list(request):
    """Turnuva listesi"""
    active = Tournament.objects.filter(status__in=['registration', 'ongoing']).order_by('start_date')
...
def tournament_detail(request, tournament_id):
    ...
    can_register = (
        ...
        tournament.participants.count() < tournament.max_participants and
        profile.coins >= tournament.entry_fee_coins and
        profile.gems >= tournament.entry_fee_gems
    )
    
    participants = tournament.participants.select_related('player').order_by('seed')
    matches = tournament.matches.select_related('player1', 'player2', 'winner').order_by('-scheduled_time')
...
def tournament_register(request, tournament_id):
    ...
    if request.method == 'POST':
        ...
        # Ücret kontrolü
        if profile.coins < tournament.entry_fee_coins or profile.gems < tournament.entry_fee_gems:
            return redirect('games:tournament_detail', tournament_id=tournament_id)
...
def quests_page(request):
    ...
    completed_quests = PlayerQuest.objects.filter(
        player=request.user,
        is_completed=True
    ).select_related('quest').order_by('-completed_at')[:10]
...
def store_page(request):
    ...
    # Oyuncunun envanteri
    my_inventory = PlayerInventory.objects.filter(player=request.user).select_related('item')
    ...
    if request.method == 'POST':
        ...
        if profile.coins >= item.price_coins and profile.gems >= item.price_gems:
            ...
            if item.duration_hours > 0:
                inventory.expires_at = timezone.now() + timedelta(hours=item.duration_hours)
                inventory.save()
...
def rankings_page(request):
    """Sıralama tabloları"""
    # Global sıralama
    global_rankings = Leaderboard.objects.filter(
        leaderboard_type='global'
    ).select_related('player').order_by('rank')[:100]
    
    # Haftalık
    weekly_rankings = Leaderboard.objects.filter(
        leaderboard_type='weekly'
    ).select_related('player').order_by('rank')[:50]
...
def match_history(request):
    ...
    matches = Match.objects.filter(
        Q(player1=request.user) | Q(player2=request.user)
    ).select_related('game', 'player1', 'player2', 'winner').order_by('-ended_at')[:50]
