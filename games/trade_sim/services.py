...
def process_city_trade(from_city: City, to_city: City, product: Product, amount: int):
    ...
    with transaction.atomic():
        ...
        try:
            from_market = CityMarket.objects.select_for_update().get(
                city=from_city, product=product
            )
            to_market = CityMarket.objects.select_for_update().get(
                city=to_city, product=product
            )
