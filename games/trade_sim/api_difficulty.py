...
def start_game_with_difficulty(request):
    ...
    character_name = request.data.get(
        "character_name", f"{request.user.username} Trader"
    )
        ...
        return Response(
            {
                ...
                "requirements": {
                    "min_level": DIFFICULTY_UNLOCK_REQUIREMENTS[difficulty_level][
                        "min_level"
                    ],
                    "min_elo": DIFFICULTY_UNLOCK_REQUIREMENTS[difficulty_level][
                        "min_elo"
                    ],
...
def get_difficulty_info(request):
    ...
    # Win rate hesapla
    win_rate = (
        profile.games_won / profile.games_played if profile.games_played > 0 else 0.5
    )
...
def finish_game_with_rewards(request):
    ...
    if is_victory:
        profile.skill_trade = min(
            100, profile.skill_trade + (2 * difficulty_level.value)
        )
    else:
