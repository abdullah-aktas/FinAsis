...
def production_error_handler(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if hasattr(args[0], 'META') and 'application/json' in args[0].META.get('HTTP_ACCEPT', ''):
                return JsonResponse({
                    'success': False,
                ...
                return render(args[0], 'trade_sim/error.html', {
...
def log_game_action(action_type, user, details=None):
    ...
    logger.info(f"TradeSim Action: {action_type} by user {user.id if user.is_authenticated else 'anonymous'}", extra={
...
class TradeSim404Handler:
    ...
    def handle_404(request, exception=None):
        return render(request, 'trade_sim/404.html', {
...
class TradeSim500Handler:
    ...
    def handle_500(request):
        return render(request, 'trade_sim/500.html', {
