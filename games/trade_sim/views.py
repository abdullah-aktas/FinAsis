...
from .services import update_city_market, process_city_trade, random_market_event, market_tick
...
def start_game(request):
    ...
    if request.user.is_authenticated:
        ...
        available_difficulties = get_available_difficulties(profile.level, profile.elo_rating)
...
def play(request):
    ...
    if not request.user.is_authenticated:
        return redirect(f"/accounts/login/?next=/games/trade-sim/start/")
    ...
    context = {
        'game_session': session,
        'difficulty_param': int(diff_param) if diff_param and diff_param.isdigit() else None,
...
def leaderboard(request):
    ...
    if request.user.is_authenticated:
        try:
            ...
            if sort_by == 'elo_rating':
                user_rank = all_players.filter(
                    Q(elo_rating__gt=user_profile.elo_rating) |
                    Q(elo_rating=user_profile.elo_rating, total_score__gt=user_profile.total_score)
                ).count() + 1
            elif sort_by == 'level':
                user_rank = all_players.filter(
                    Q(level__gt=user_profile.level) |
                    Q(level=user_profile.level, xp__gt=user_profile.xp) |
                    Q(level=user_profile.level, xp=user_profile.xp, total_score__gt=user_profile.total_score)
                ).count() + 1
            else:  # total_score veya diğerleri
                user_rank = all_players.filter(
                    Q(total_score__gt=user_profile.total_score) |
                    Q(total_score=user_profile.total_score, elo_rating__gt=user_profile.elo_rating)
                ).count() + 1
    ...
    for index, player in enumerate(players_list, start=1):
        ...
        win_rate = (games_won / player.games_played * 100) if player.games_played > 0 else 0
...
def guest_onboarding(request: Request):
    ...
    try:
        ...
        login(cast(HttpRequest, base_req), user, backend='django.contrib.auth.backends.ModelBackend')
    except TypeError:
...
def ai_story_suggestion(request, character_id):
    ...
    durum = {
        ...
        'aktif_görevler': [
            cq.quest.name for cq in CharacterQuest.objects.filter(character=character, is_completed=False)
        ],
    }
...
def market_trade(request: Request):
    ...
    city_name = req_data.get('city', 'istanbul')
    
    if not all([action, item_name, amount > 0, price > 0]):
        return Response({'success': False, 'error': 'Eksik veya geçersiz parametreler'}, status=400)
    ...
    try:
        ...
        character = Character.objects.filter(user=request.user).first()
        if not character:
            return Response({'success': False, 'error': 'Karakter bulunamadı'}, status=404)
        ...
        if action == 'buy':
            ...
            if character.score < total_cost:
                return Response({'success': False, 'error': 'Yeterli paranız yok!'}, status=400)
            ...
            if has_amount < amount:
                return Response({'success': False, 'error': 'Envanterde yeterli ürün yok!'}, status=400)
...
def change_city(request: Request):
    ...
    try:
        ...
        if character.score < travel_cost:
            return Response({'success': False, 'error': 'Seyahat için yeterli paranız yok!'}, status=400)
        
        # Aynı şehir mi kontrolü
        if character.city and character.city.pk == new_city.pk:
            return Response({'success': False, 'error': 'Zaten bu şehirdesiniz!'}, status=400)
