...
class DifficultyManager:
    ...
    def calculate_market_price(self, base_price: float, product: str, city: str) -> float:
        """Zorluk bazlı pazar fiyatı hesapla"""
        price = base_price
        
        # Volatilite ekle
        volatility = random.uniform(-self.config.price_volatility, self.config.price_volatility)
        ...
        for event in self.active_events:
            if event['type'] == 'inflation_spike':
                ...
                price *= event['effect'].get('supply_multiplier', 1.0) * 2.0  # Kıtlıkta fiyat artar
...
def get_available_difficulties(player_level: int, player_elo: int) -> List[DifficultyLevel]:
    """Oyuncunun erişebileceği zorluk seviyelerini döndür"""
    available = []
    for level, requirements in DIFFICULTY_UNLOCK_REQUIREMENTS.items():
        if player_level >= requirements['min_level'] and player_elo >= requirements['min_elo']:
            available.append(level)
    return available


def get_recommended_difficulty(player_level: int, player_elo: int, win_rate: float) -> DifficultyLevel:
