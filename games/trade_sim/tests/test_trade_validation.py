...
def test_city_trade_insufficient_supply_returns_400():
    client = APIClient()
    a = City.objects.create(name='A', description='A', sectors=[], market_size=1000, coordinates={})
    b = City.objects.create(name='B', description='B', sectors=[], market_size=1000, coordinates={})
    ...
    payload = {'from_city': getattr(a, 'id'), 'to_city': getattr(b, 'id'), 'product_id': getattr(p, 'id'), 'amount': 5}
...
def test_city_trade_negative_amount_returns_400():
    ...
    url = reverse('trade_sim:city_trade')
    payload = {'from_city': getattr(a, 'id'), 'to_city': getattr(b, 'id'), 'product_id': getattr(p, 'id'), 'amount': -1}
    res = cast(DRFResponse, client.post(url, payload, format='json'))
    assert res.status_code == 400
