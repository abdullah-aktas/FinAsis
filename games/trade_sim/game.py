...
from games.notifications import (
    NotificationSystem,
    NotificationType,
...
class TradeSim:
    ...
    def _initialize_tradesim_content(self):
        ...
        special_item = Item(
            ...
            value={
                CurrencyType.COINS: 10000,
                CurrencyType.GEMS: 100,
                CurrencyType.TOKENS: 10,
            },
            effects={"trade_bonus": 0.10},
        )
        self.economy_system.items[special_item.id] = special_item
        # Sosyal: Özel lonca
        self.social_system.create_clan(
            "admin", "TradeSim Elitleri", "TSE", "En iyi traderların loncası."
        )
    ...
    def setup_world(self):
        ...
        for x in range(self.stand_origin[0], self.stand_origin[0] + self.stand_size[0]):
            for z in range(
                self.stand_origin[2], self.stand_origin[2] + self.stand_size[2]
            ):
    ...
    def on_trade(self, player_id, trade_data):
        ...
        self.quest_system.update_quest_progress(
            player_id, "daily_trade_1", {"trades": 1}
        )
        ...
        if trade_data.get("profit", 0) >= 10000:
            ...
            self.notification_system.send_achievement_notification(
                player_id, "Büyük Kar!"
            )
        self.economy_system.award_achievement_rewards(1)
    ...
    def on_friend_added(self, player_id, friend_id):
        ...
        self.notification_system.send_friend_notification(
            player_id, friend_id, "ekledi"
        )
    ...
    def on_quest_completed(self, player_id, quest_id):
        if quest_id == "tradesim_marathon":
            ...
            self.notification_system.send_quest_notification(
                player_id, new_quest.title, False
            )
    ...
    def create_team(self, team_name, creator_id):
        ...
        self.game_state["teams"][team_name] = {"members": [creator_id], "score": 0}
        self.notification_system.send_notification(
    ...
    def eliminate_player_battle_royale(self, player_id):
        ...
        if player_id in br["players"] and player_id not in br["eliminated"]:
            ...
            if len(br["players"]) - len(br["eliminated"]) == 1:
                winner = [pid for pid in br["players"] if pid not in br["eliminated"]][
                    0
                ]
    ...
    def join_guild(self, guild_name, player_id):
        if (
            "guilds" not in self.game_state
            or guild_name not in self.game_state["guilds"]
        ):
            return False
    ...
    def initialize_needs(self, player_id):
        player = self.game_state["players"].get(player_id)
        if player:
            player["needs"] = {"energy": 100, "happiness": 100, "hunger": 100}
    ...
    def spawn_random_collectible(self):
        ...
        self.collectibles.append(
            {"type": collectible, "position": pos, "collected": False}
        )
        self.notification_system.send_notification(
            player_id="all",
            notification_type=NotificationType.SYSTEM,
    ...
    def record_blockchain_transaction(
        self, player_id, title, tx_type, reference_id=None
    ):
