...
def leaderboard(request):
    ...
    top_players = PlayerProfile.objects.select_related("user").order_by(
    ...
    if request.user.is_authenticated:
        try:
            ...
            higher_count = PlayerProfile.objects.filter(
                ...
                | (
                    Q(total_score=user_profile.total_score)
                    & Q(games_played__gt=user_profile.games_played)
                )
            ).count()
            user_rank = {
                "rank": higher_count + 1,
...
def earn_badge(request):
    ...
    try:
        if badge_id:
            ...
            return Response(
                {"error": "badge_id veya badge_code gereklidir."},
        ...
        if badge in profile.badges.all():
            return Response(
                {"message": "Bu rozet zaten kazanılmış."}, status=status.HTTP_200_OK
            )
        ...
        return Response(
            {"error": "Rozet bulunamadı."}, status=status.HTTP_404_NOT_FOUND
        )
    except PlayerProfile.DoesNotExist:
        return Response(
            {"error": "Oyuncu profili bulunamadı."}, status=status.HTTP_404_NOT_FOUND
        )
...
def save_simulation_score(request):
    ...
    if score is None:
        return Response(
            {"error": "Skor zorunludur."}, status=status.HTTP_400_BAD_REQUEST
        )
...
def track(request):
    ...
    if outcome not in ("correct", "incorrect"):
        return Response(
            {"error": _("Geçersiz outcome değeri")}, status=status.HTTP_400_BAD_REQUEST
        )

    res = track_event(
        request.user, game=game, category=category, correct=(outcome == "correct")
    )
