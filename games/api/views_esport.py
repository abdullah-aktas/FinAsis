...
from ..models import PlayerProfile, DailyQuest, PlayerQuest, GameSession
    ...
    if active.count() < 3:
        ...
        for quest in available_quests:
            ...
            if not PlayerQuest.objects.filter(
                player=request.user, quest=quest, assigned_at__gte=today_start
            ).exists():
                PlayerQuest.objects.create(
                    player=request.user, quest=quest, expires_at=today_end
                )

        # Yeniden getir
        active = PlayerQuest.objects.filter(
            player=request.user, expires_at__gte=timezone.now()
        ).select_related("quest")
