...
class EconomySystem:
    ...
    def generate_daily_deals(self):
        ...
        for _ in range(5):  # Her gün 5 fırsat
            ...
            random.uniform(0.5, 0.8)  # %20-50 indirim
    ...
    def award_achievement_rewards(
        self, achievement_level: int
    ...
    def calculate_battle_royale_rewards(
        self, placement: int, kills: int
    ) -> Dict[CurrencyType, int]:
        """Battle Royale ödüllerini hesaplar"""
        placement_multiplier = (
            101 - placement
        ) / 100  # 1. sıra = 1.0, 100. sıra = 0.01
        kill_bonus = kills * 10
    ...
    def generate_loot_box(self, rarity: ItemRarity) -> List[Item]:
        ...
        for _ in range(num_items):
            ...
            available_items = [
                item for item in self.items.values() if item.rarity == rarity
            ]
            if available_items:
