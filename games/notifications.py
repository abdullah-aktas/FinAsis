...
class NotificationSystem:
    ...
    def get_player_notifications(
        self, player_id: str, unread_only: bool = False, limit: int = 50
    ) -> List[Notification]:
    ...
    def clear_old_notifications(self, player_id: str, days: int = 30) -> int:
        ...
        self.notifications[player_id] = [
            n for n in self.notifications[player_id] if n.timestamp > cutoff_date
        ]

        # Okunmamış sayısını güncelle
        self.unread_counts[player_id] = len(
            [n for n in self.notifications[player_id] if not n.read]
        )
    ...
    def send_tournament_notification(
        self, player_id: str, tournament_name: str, event_type: str
    ):
    ...
    def send_training_notification(
        self, player_id: str, module_name: str, progress: int
    ):
    ...
    def send_community_notification(
        self, player_id: str, content_name: str, status: str
    ):
