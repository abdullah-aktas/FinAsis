...
class Game(models.Model):
    ...
    duration_minutes = models.IntegerField(default=15, help_text="Ortalama oyun süresi (dakika)")
...
class Badge(models.Model):
    ...
    criteria = models.JSONField(default=dict, blank=True, verbose_name="Kazanma Kriteri")
...
class PlayerProfile(models.Model):
    ...
    difficulty = models.CharField(max_length=16, choices=DIFFICULTY_CHOICES, default="adaptive", verbose_name="Zorluk")
    ...
    xp_to_next_level = models.PositiveIntegerField(default=1000, verbose_name="Sonraki Seviyeye Kalan XP")
    ...
    # Rekabet metrikleri
    rank = models.CharField(max_length=20, choices=RANK_CHOICES, default='bronze', verbose_name="Rank")
    ...
    highest_score = models.PositiveIntegerField(default=0, verbose_name="En Yüksek Skor")
    ...
    # İlişkiler
    badges = models.ManyToManyField(Badge, blank=True, related_name="players", verbose_name="Rozetler")
    current_season = models.ForeignKey(Season, on_delete=models.SET_NULL, null=True, blank=True)
    ...
    def add_xp(self, amount):
        ...
        while self.xp >= self.xp_to_next_level:
            ...
            self.level += 1
            self.xp_to_next_level = int(1000 * (1.5 ** (self.level - 1)))  # Exponential growth
    ...
    def record_event(self, category: str, correct: bool):
        ...
        self.save(update_fields=[
            "stats", "skill_trade", "skill_invest", "skill_budget", 
            "skill_education", "skill_accounting", "games_played", "updated_at"
        ])
...
class Tournament(models.Model):
    ...
    tournament_type = models.CharField(max_length=20, choices=TOURNAMENT_TYPES, default='casual')
    ...
    # Katılım
    max_participants = models.IntegerField(default=128, validators=[MinValueValidator(2)])
    ...
    bracket_type = models.CharField(max_length=20, default='single_elimination', 
        choices=[('single_elimination', 'Eleme'), ('double_elimination', 'Çift Eleme'), ('round_robin', 'Herkes Herkesle')])
    
    created_at = models.DateTimeField(auto_now_add=True)
    created_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, related_name='created_tournaments')
...
class TournamentParticipant(models.Model):
    ...
    player = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='tournament_participations')
...
class Match(models.Model):
    ...
    player2 = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='matches_as_player2', null=True, blank=True)
...
class PlayerQuest(models.Model):
    ...
    player = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='quests')
...
class GameSession(models.Model):
    ...
    # Detaylar
    session_data = models.JSONField(default=dict, blank=True, help_text="Oyun içi events, actions vb.")
...
class Friend(models.Model):
    ...
    from_player = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='sent_friend_requests')
    to_player = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='received_friend_requests')
...
class Team(models.Model):
    ...
    tag = models.CharField(max_length=10, unique=True, help_text="Kısa takım etiketi (örn: FIN)")
    description = models.TextField(blank=True)
    
    owner = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='owned_teams')
    members = models.ManyToManyField(settings.AUTH_USER_MODEL, related_name='teams', through='TeamMembership')
