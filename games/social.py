...
class SocialSystem:
    ...
    def create_clan(
        self, leader_id: str, name: str, tag: str, description: str
    ) -> Optional[Clan]:
    ...
    def get_player_reputation(self, player_id: str) -> float:
        ...
        recent_reports = [
            r for r in reports if (datetime.now() - r["timestamp"]).days <= 30
        ]
    ...
    def get_clan_leaderboard(self, limit: int = 10) -> List[Clan]:
        ...
        return sorted(self.clans.values(), key=lambda c: c.level, reverse=True)[:limit]
