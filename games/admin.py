...
from .models import (
    Game, Season, Badge, PlayerProfile, Tournament, TournamentParticipant,
    Match, DailyQuest, PlayerQuest, Item, PlayerInventory, 
    Leaderboard, Friend, Team, TeamMembership, GameSession
)


@admin.register(Game)
class GameAdmin(admin.ModelAdmin):
    list_display = ['name', 'game_type', 'is_esport_enabled', 'min_players', 'max_players', 'duration_minutes']
...
class SeasonAdmin(admin.ModelAdmin):
    list_display = ['name', 'start_date', 'end_date', 'is_active', 'is_ongoing', 'reward_pool']
...
class PlayerProfileAdmin(admin.ModelAdmin):
    list_display = [
        'user', 'level', 'rank', 'elo_rating', 'total_score', 
        'games_won', 'games_played', 'daily_streak', 'coins', 'gems'
    ]
    ...
    fieldsets = (
        ...
        ('Progression', {
            'fields': ('level', 'xp', 'xp_to_next_level', 'rank', 'elo_rating', 'mmr')
        }),
        ('Beceriler', {
            'fields': ('skill_trade', 'skill_invest', 'skill_budget', 'skill_education', 'skill_accounting')
        }),
        ('İstatistikler', {
            'fields': ('games_played', 'games_won', 'games_lost', 'total_score', 'highest_score')
        }),
        ('Ekonomi', {
            'fields': ('coins', 'gems')
        }),
        ('Streak', {
            'fields': ('daily_streak', 'last_played_date')
        }),
        ('İlişkiler', {
            'fields': ('badges', 'current_season')
        }),
    )
...
class TournamentAdmin(admin.ModelAdmin):
    list_display = [
        'name', 'game', 'tournament_type', 'status', 'start_date', 
        'participant_count', 'prize_pool_coins'
    ]
...
class TournamentParticipantAdmin(admin.ModelAdmin):
    list_display = ['player', 'tournament', 'seed', 'final_rank', 'total_score', 'registered_at']
...
class MatchAdmin(admin.ModelAdmin):
    list_display = [
        'id', 'tournament', 'game', 'player1', 'player2', 
        'status', 'get_score', 'winner', 'scheduled_time'
    ]
...
class DailyQuestAdmin(admin.ModelAdmin):
    list_display = ['title', 'quest_type', 'target_value', 'difficulty_level', 'reward_xp', 'reward_coins', 'is_daily']
...
class PlayerQuestAdmin(admin.ModelAdmin):
    list_display = ['player', 'quest', 'current_value', 'target_value', 'is_completed', 'expires_at']
...
class ItemAdmin(admin.ModelAdmin):
    list_display = ['name', 'item_type', 'rarity', 'price_coins', 'price_gems', 'is_tradeable', 'is_purchasable']
...
class PlayerInventoryAdmin(admin.ModelAdmin):
    list_display = ['player', 'item', 'quantity', 'is_equipped', 'acquired_at', 'expires_at']
...
class LeaderboardAdmin(admin.ModelAdmin):
    list_display = ['rank', 'player', 'leaderboard_type', 'game', 'season', 'score', 'updated_at']
...
class TeamAdmin(admin.ModelAdmin):
    list_display = ['tag', 'name', 'owner', 'team_rank', 'team_elo', 'member_count', 'created_at']
...
class GameSessionAdmin(admin.ModelAdmin):
    list_display = ['id', 'player', 'game', 'score', 'is_victory', 'duration_seconds', 'xp_earned', 'coins_earned', 'started_at']
