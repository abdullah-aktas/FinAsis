...
class FinanceEducator(BaseGame):
    def __init__(self):
        super().__init__()
        self.app = Ursina()
        ...
        # Oyun durumu
        self.game_state = (
            "main_menu"  # main_menu, character_select, game, quest, learning, tutorial
        )
    ...
    def create_main_menu(self):
        ...
        self.background = Entity(
            model="quad", scale=(2, 1), color=color.rgb(50, 50, 100), parent=camera.ui
        )
    ...
    def select_age_group(self, age_group):
        ...
        # Butonları güncelle
        for i, (button, key) in enumerate(
            zip(self.age_buttons, ["child", "teen", "adult", "senior"])
        ):
            if key == age_group:
    ...
    def create_character_selection(self):
        ...
        # Arka plan
        self.background = Entity(
            model="quad", scale=(2, 1), color=color.rgb(50, 100, 50), parent=camera.ui
        )

        # Başlık
        self.title = Text(
            text="Karakterinizi Seçin", scale=3, y=0.4, origin=(0, 0), parent=camera.ui
        )
    ...
    def create_tutorial_world(self):
        ...
        self.tutorial_panel = Panel(
            scale=(0.6, 0.2), position=(0, 0.4), parent=camera.ui
        )
    ...
    def create_game_world(self):
        ...
        self.quest_panel = Panel(
            scale=(0.3, 0.5), position=(0.6, 0), parent=camera.ui, visible=False
        )
        ...
        self.info_panel = Panel(
            scale=(0.3, 0.2), position=(-0.6, 0.3), parent=camera.ui
        )
        ...
        # Gösterge UI'ı
        self.hud_panel = Panel(scale=(0.3, 0.1), position=(0, 0.45), parent=camera.ui)
...
class Panel(Entity):
    def __init__(self, **kwargs):
        super().__init__(model="quad", color=color.rgba(0, 0, 0, 0.7), **kwargs)
