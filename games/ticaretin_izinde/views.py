...
from .serializers import (
    ...
    UrsinaPlayerSerializer, PlayerWalletSerializer, UrsinaGameSessionSerializer,
    TournamentSerializer, AchievementSerializer, PlayerAchievementSerializer
)
    ...
    def perform_create(self, serializer):
        ...
        for achievement in achievements:
            ...
            try:
                ...
                elif isinstance(achievement.criteria, str) and achievement.criteria.strip():
...
def start_game(request):
    ...
    result = game.start()
    return JsonResponse({'status': result.get('status', 'success'), 'message': 'Oyun başlatıldı', 'pid': result.get('pid')})
...
def pause_game(request):
    ...
    game = FinancialTradingGame()
    result = game.pause()
    return JsonResponse({'status': result.get('status', 'success'), 'message': 'Oyun duraklatıldı'})
...
def resume_game(request):
    ...
    game = FinancialTradingGame()
    result = game.resume()
    return JsonResponse({'status': result.get('status', 'success'), 'message': 'Oyun devam ediyor'})
...
def end_game(request):
    ...
    game = FinancialTradingGame()
    result = game.end()
    return JsonResponse({'status': result.get('status', 'success'), 'message': 'Oyun sonlandırıldı'})
