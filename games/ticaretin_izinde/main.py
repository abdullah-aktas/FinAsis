        ...
        def _watch_control():
            ...
            while True:
                try:
                    if ctrl_path.exists():
                        data = _json.loads(
                            ctrl_path.read_text(encoding="utf-8") or "{}"
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--fullscreen", action="store_true", help="Tam ekran modunda başlat"
    )
    parser.add_argument(
        "--age",
    ...
    parser.add_argument(
        "--character",
        choices=["businessman", "student", "teacher", "retiree"],
