...
from helpers import get_age_group_colors
...
class TicaretinIzinde3D(BaseGame):
    def __init__(self):
        ...
        self.game_state = {
            "player": {
                ...
                "knowledge": {"accounting": 1, "trading": 1, "management": 1},
            },
            "market": {"products": {}},
    ...
    def create_main_menu(self):
        ...
        for i, (age_text, age_key) in enumerate(age_groups):
            ...
            btn.color = (
                ...
                if age_key == self.game_state["player"]["age_group"]
                else color.gray
            )
        ...
        self.ui_elements["main_menu"] = [
            ...
            self.subtitle,
            self.age_group_text,
            *[btn for btn, _ in self.age_buttons],
            self.start_button,
            self.exit_button,
        ]
    ...
    def select_age_group(self, age_group):
        ...
        for button, key in self.age_buttons:
            button.color = (
                self.color_scheme["primary"] if key == age_group else color.gray
            )
    ...
    def create_character_selection(self):
        ...
        characters = [
            (
                "İş İnsanı",
                "businessman",
                "İş dünyasında deneyimli bir girişimci. Ticaret ve yatırım konusunda avantajlı başlar.",
            ),
            (
                "Öğrenci",
                "student",
                "Yeni mezun olmuş bir öğrenci. Düşük sermaye ama yüksek öğrenme kapasitesi.",
            ),
            (
                "Öğretmen",
                "teacher",
                "Bilgi ve eğitim odaklı. Finansal okuryazarlık ve planlama avantajı.",
            ),
            (
                "Emekli",
                "retiree",
                "Deneyimli bir emekli. Başlangıçta daha yüksek sermaye ama daha düşük enerji.",
            ),
        ]
        ...
        self.ui_elements["character_select"] = [
            ...
            *[btn for btn, _ in self.character_buttons],
            ...
            self.company_type_text,
            *[btn for btn, _ in self.company_type_buttons],
    ...
    def select_character(self, character, description):
        ...
        for button, key in self.character_buttons:
            button.color = (
                self.color_scheme["primary"] if key == character else color.gray
            )
    ...
    def select_company_type(self, company_type):
        ...
        for button, key in self.company_type_buttons:
            button.color = (
                self.color_scheme["primary"] if key == company_type else color.gray
            )
    ...
    def create_tutorial_world(self):
        ...
        try:
            from games.finquest.world import (
                create_tutorial_world as _create_tutorial_world,
            )
        except Exception:
    ...
    def assign_default_quests(self):
        ...
        for quest in self.quests["tutorial"]:
            if (
                quest["id"] not in self.game_state["active_quests"]
                and quest["id"] not in self.game_state["completed_quests"]
            ):
    ...
    def check_quests(self):
        ...
        for quest_id in self.game_state["active_quests"]:
            ...
            if quest_data and quest_data["completion_criteria"]():
                ...
                # Ödülü ver
                self.game_state["player"]["money"] += quest_data["reward"].get(
                    "money", 0
                )
