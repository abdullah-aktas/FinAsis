...
class GameManager:
    ...
    def save_game_state(self, game_state):
        ...
        save_path = os.path.join(
            settings.MEDIA_ROOT, "game_saves", f"player_{self.player.id}.json"
        )
    ...
    def check_challenges(self, game_state):
        ...
        for challenge in Challenge.objects.filter(game=self.player.game):
            if not PlayerChallenge.objects.filter(
                player=self.player, challenge=challenge
            ).exists():
                pc = PlayerChallenge.objects.create(
                    player=self.player, challenge=challenge
                )
    ...
    def get_player_data(self):
        ...
        return {
            ...
            "active_challenges": list(
                PlayerChallenge.objects.filter(
                    player=self.player, is_completed=False
                ).values(
                    "challenge__title", "challenge__description", "challenge__points"
                )
            ),
        }
