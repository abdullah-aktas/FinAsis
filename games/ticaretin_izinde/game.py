...
player = {
    ...
    "experience": {"level": 1, "current_xp": 0, "next_level_xp": 1000, "total_xp": 0},
    "achievements": [],
    "active_quests": [],
    "completed_quests": [],
    "quest_progress": {},
...
quest_definitions = {
    ...
    "tutorial_buy": {
        ...
        "type": "tutorial",
        "check_completion": lambda: len(
            [t for t in player["trading_history"] if t["type"] == "buy"]
        )
        > 0,
    },
    "tutorial_sell": {
        ...
        "type": "tutorial",
        "check_completion": lambda: len(
            [t for t in player["trading_history"] if t["type"] == "sell"]
        )
        > 0,
    },
    "tutorial_diversity": {
        ...
        "type": "tutorial",
        "check_completion": lambda: len(
            [s for s in player["portfolio"].values() if s["shares"] > 0]
        )
        >= 3,
    },
}
...
class FinAsisGame(Ursina):  # noqa: F811
    def __init__(self, online_mode: bool = False):  # noqa: F811
        ...
        # Dünya oluşturma
        self.dunya = Entity(
            model="plane", texture="white_cube", scale=(100, 1, 100), color=color.gray
        )
    ...
    def toggle_language(self):
        ...
        current_index = available_locales.index(
            self.locale_manager.get_current_locale()
        )
    ...
    def alis_yap(self):
        if self.oyun.oyuncu_bakiyesi >= 1000:
            ...
            if basari:
                self.ui_elements["bildirim"].text = self.locale_manager.get_text(
                    "game.transactions.success"
                )
                self.ui_elements["bildirim"].color = color.green
            else:
                self.ui_elements["bildirim"].text = self.locale_manager.get_text(
                    "game.transactions.fail"
                )
                self.ui_elements["bildirim"].color = color.red
        else:
            self.ui_elements["bildirim"].text = self.locale_manager.get_text(
                "game.transactions.insufficient_balance"
            )
            self.ui_elements["bildirim"].color = color.red
    ...
    def ui_guncelle(self):
        ...
        self.ui_elements["satis_buton"].text = self.locale_manager.get_text(
            "game.transactions.sell"
        )
            ...
            if simdiki_zaman - olay["baslangic"] > olay["sure"]:
                ...
                olay["tip"] = random.choice(
                    [
                        ...
                        "Borsa DÃ¼ÅŸÃ¼ÅŸÃ¼",
                        "Enflasyon ArtÄ±ÅŸÄ±",
                        "Enflasyon DÃ¼ÅŸÃ¼ÅŸÃ¼",
                        "Faiz ArtÄ±ÅŸÄ±",
                        "Faiz DÃ¼ÅŸÃ¼ÅŸÃ¼",
                        "DÃ¶viz DalgalanmasÄ±",
                        "AltÄ±n FiyatÄ± DeÄŸiÅŸimi",
                    ]
                )
                olay["etki"] = random.uniform(-0.2, 0.2)
                olay["baslangic"] = simdiki_zaman
...
def start_tutorial():
    ...
    tutorial_panel = Entity(
        parent=camera.ui, model="quad", scale=(0.4, 0.2), color=color.rgba(0, 0, 0, 0.9)
    )
...
def save_game():
    ...
    try:
        ...
        with open("save_game.json", "w", encoding="utf-8") as f:
            json.dump(
                save_data, f, default=convert_datetime, ensure_ascii=False, indent=4
            )
...
def show_notification(message, color=color.white):
    """Bildirim gÃ¶ster"""
    notification = Text(text=message, position=(0, 0.4), scale=2, color=color)
...
def assign_daily_quests():
    ...
    player["active_quests"] = [
        ...
        if q not in [d["id"] for d in quest_system["daily_quests"]]
    ]

    # Rastgele 2 gÃ¼nlÃ¼k gÃ¶rev seÃ§
    selected_quests = random.sample(
        quest_system["daily_quests"], min(2, len(quest_system["daily_quests"]))
    )
...
def assign_weekly_quests():
    ...
    player["active_quests"] = [
        q
        for q in player["active_quests"]
        if q not in [w["id"] for w in quest_system["weekly_quests"]]
    ]

    # Rastgele 2 haftalÄ±k gÃ¶rev seÃ§
    selected_quests = random.sample(
        quest_system["weekly_quests"], min(2, len(quest_system["weekly_quests"]))
    )
...
def give_quest_rewards(quest):
    ...
    if "experience" in reward:
        ...
        while (
            ...
            player["experience"]["next_level_xp"] = calculate_next_level_xp(
...
def calculate_daily_profit():
    ...
    today_trades = [
        t for t in player["trading_history"] if t["timestamp"].date() == today
    ]
...
def update_market_state():
    ...
    current_time = datetime.now()
    if (
        current_time - market_state["last_update"]
    ).seconds >= 30:  # Her 30 saniyede bir gÃ¼ncelle
...
def update_market():
    ...
    # Piyasa gÃ¼ncellemesi
    if (current_time - market_state["last_update"]).seconds >= game_settings[
...
def update_stock_prices():
    ...
    for symbol, data in player["portfolio"].items():
        ...
        volatility_effect = random.uniform(
            -market_state["volatility"], market_state["volatility"]
        )
...
def apply_market_event(event):
    ...
    if "volatility" in effect:
        market_state["volatility"] = max(
            0.1, min(1.0, market_state["volatility"] + effect["volatility"])
        )
...
def calculate_debt_to_income_ratio():
    ...
    total_income = sum(
        t["amount"]
        for t in player.get("trading_history", [])
...
def update_notifications():
    ...
    ui["notifications"] = [
        n for n in ui["notifications"] if (current_time - n["timestamp"]).seconds < 5
    ]
...
def check_quests():
    ...
    for quest in quest_system["daily_quests"]:
        if (
            not quest.get("completed", False)
            and quest_definitions[quest["id"]]["check_completion"]()
        ):
...
def check_level_up():
    ...
    while player["experience"]["current_xp"] >= player["experience"]["next_level_xp"]:
        ...
        rewards = {"money": 1000 * player["experience"]["level"], "experience": 0}
...
class CharacterSelection:
    ...
    def create_ui(self):  # noqa: F811
        ...
        self.ui_elements["panel"] = Entity(
            model="quad", scale=(1, 1), color=color.black90, position=(0, 0, -1)
        )
...
class MissionSystem:
    ...
    def create_mission_ui(self):
        # GÃ¶rev paneli
        self.ui_elements["panel"] = Entity(
            model="quad", scale=(0.4, 0.3), color=color.black90, position=(0.7, 0.6, -1)
        )
    ...
    def level_up(self):
        ...
        if self.current_level <= len(self.levels):
            ...
            show_notification(
                f"ðŸŽ‰ Seviye {self.current_level} AÃ§Ä±ldÄ±!", color.yellow
            )
...
class BattleRoyaleMode:
    ...
    def load_events(self):
        ...
        try:
            with open(
                "games/ursina_game/arena_events.json", "r", encoding="utf-8"
            ) as f:
    ...
    def olay_guncelle(self):  # noqa: F811
        ...
        for event in self.events:
            if "start_time" in event and current_time - event["start_time"] > event.get(
                "duration", 0
            ):
...
class FastFinanceTournament:
    ...
    def submit_answer(self, option_id):
        ...
        if (
            isinstance(self.current_scenario, dict)
            and "correct_answer" in self.current_scenario
        ):
    ...
    events = [
        {
            ...
            "effect": lambda p: setattr(
                p, "market_multiplier", p.market_multiplier * 1.2
            ),
        },
        {
            ...
            "effect": lambda p: setattr(
                p, "resource_efficiency", p.resource_efficiency * 1.15
            ),
        },
    ]
...
class FinAsisGame(Ursina):  # noqa: F811
    def __init__(self, player_id):  # noqa: F811
        ...
        self.player_data = {
            ...
            "experience": 0,
            "level": 1,
            "skills": {"management": 1, "marketing": 1, "finance": 1, "production": 1},
            "achievements": [],
            "inventory": {},
            "buildings": [],
            "employees": [],
        ...
        self.quality_system = {
            "iso_standards": {"iso9001": False, "iso14001": False, "iso45001": False},
            "quality_control": {
                "processes": [],
                "checkpoints": [],
                "defect_rate": 0.05,
        ...
        self.exercise_system = {
            ...
            "exercise_types": {
                ...
                "strength": ["ÅžÄ±nav", "Mekik", "Squat", "Plank", "Lunge"],
                "cardio": [
                    "Yerinde koÅŸu",
                    "ZÄ±plama",
                    "Merdiven Ã§Ä±kma",
    ...
    def _panel_olustur(self, isim, pozisyon):
        """Tek bir panel oluşturur ve döndürür."""
        return Entity(
            model="quad",
            texture="white_cube",
            scale=(0.3, 0.4),
    ...
    def create_ui(self):  # noqa: F811
        ...
        # Panel isimleri ve pozisyonları
        paneller = {
            "market_research_panel": (-0.7, 0.2),
            "competition_panel": (-0.7, -0.3),
            "financial_panel": (0, 0.2),
            "strategic_panel": (0, -0.3),
            "production_panel": (0.7, 0.2),
            "hr_panel": (0.7, -0.3),
            "marketing_panel": (0.35, 0.2),
            "rnd_panel": (0.35, -0.3),
        }
        for isim, pozisyon in paneller.items():
            setattr(self, isim, self._panel_olustur(isim, pozisyon))
    ...
    def toggle_pause(self):
        ...
        self.show_notification(
            "Oyun "
            + ("duraklatÄ±ldÄ±" if self.game_state["is_paused"] else "devam ediyor")
        )
    ...
    def show_notification(self, message, duration=3):  # noqa: F811
        ...
        self.notification_system["messages"].append(
            {"text": message, "time": time.time(), "duration": duration}
        )
    ...
    def update_training_system(self):
        ...
        current_time = time.time()
        if (
            current_time - self.training_system["last_training_update"]
            >= self.training_system["training_interval"]
        ):
    ...
    def complete_training(self, training):
        ...
        employee["skills"][course["skill"]] = min(
            10, employee["skills"][course["skill"]] + skill_increase
        )
        ...
        # Bildirim gÃ¶ster
        self.show_notification(
            f"{employee['name']} {course['name']} eÄŸitimini tamamladÄ±!"
        )
    ...
    def show_training_panel(self):
        ...
        y_pos = -0.05
        for training in self.training_system["employee_trainings"]:
            progress = (time.time() - training["start_time"]) / (
    ...
    def update_quality_system(self):
        ...
        # Denetim kontrolÃ¼
        if (
            current_time - self.quality_system["quality_control"]["last_audit"]
            >= self.quality_system["quality_control"]["audit_interval"]
        ):
        ...
        if (
            current_time - self.quality_system["customer_satisfaction"]["last_survey"]
            >= self.quality_system["customer_satisfaction"]["survey_interval"]
        ):
        ...
        if (
            current_time - self.quality_system["continuous_improvement"]["last_review"]
            >= self.quality_system["continuous_improvement"]["review_interval"]
        ):
    ...
    def perform_customer_survey(self):
        ...
        self.quality_system["customer_satisfaction"]["score"] = total_score / (
            feedback_count * 4
        )
    ...
    def review_improvement_projects(self):
        ...
        for project in self.quality_system["continuous_improvement"]["projects"][:]:
            if (
                project["status"] == "in_progress" and random.random() < 0.2
            ):  # %20 ÅŸans
    ...
    def apply_improvement_results(self, project):
        ...
        if project["type"] == "process":
            ...
            self.quality_system["customer_satisfaction"]["score"] = min(
                1.0, self.quality_system["customer_satisfaction"]["score"] * 1.05
            )
    ...
    def update_sustainability_system(self):
        ...
        if (
            current_time
            - self.sustainability_system["environmental_impact"]["last_measurement"]
            >= self.sustainability_system["environmental_impact"][
        ...
        if (
            current_time
            - self.sustainability_system["social_responsibility"]["last_assessment"]
            >= self.sustainability_system["social_responsibility"][
                "assessment_interval"
            ]
        ):
        ...
        if (
            current_time - self.sustainability_system["energy_efficiency"]["last_audit"]
            >= self.sustainability_system["energy_efficiency"]["audit_interval"]
        ):
        ...
        if (
            current_time - self.sustainability_system["waste_management"]["last_review"]
            >= self.sustainability_system["waste_management"]["review_interval"]
        ):
    ...
    def assess_social_responsibility(self):
        ...
        # Ã‡alÄ±ÅŸan refahÄ±
        self.sustainability_system["social_responsibility"]["employee_wellbeing"] = min(
            1.0,
            self.sustainability_system["social_responsibility"]["employee_wellbeing"]
            * random.uniform(1.0, 1.05),
        )

        # Ã‡eÅŸitlilik skoru
        self.sustainability_system["social_responsibility"]["diversity_score"] = min(
            1.0,
            self.sustainability_system["social_responsibility"]["diversity_score"]
            * random.uniform(1.0, 1.05),
        )
    ...
    def audit_energy_efficiency(self):
        ...
        if random.random() < 0.3:  # %30 ÅŸans
            self.sustainability_system["energy_efficiency"][
    ...
    def review_waste_management(self):
        ...
        if random.random() < 0.3:  # %30 ÅŸans
            self.sustainability_system["waste_management"]["recycling_rate"] = min(
                1.0,
                self.sustainability_system["waste_management"]["recycling_rate"] * 1.1,
            )

        # AtÄ±k azaltma
        self.sustainability_system["waste_management"][
            "waste_reduction"
        ] += random.uniform(0.1, 0.5)
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            ...
            self.sustainability_system["waste_management"]["waste_projects"].append(
                new_project
            )
    ...
    def update_crisis_system(self):
        ...
        if (
            current_time - self.crisis_system["risk_assessment"]["last_assessment"]
            >= self.crisis_system["risk_assessment"]["assessment_interval"]
        ):
            self.assess_risks()
        ...
        for plan_type in self.crisis_system["emergency_plans"]:
            if (
                current_time
                - self.crisis_system["emergency_plans"][plan_type]["last_update"]
                >= self.crisis_system["emergency_plans"][plan_type]["update_interval"]
            ):
                self.update_emergency_plan(plan_type)
                self.crisis_system["emergency_plans"][plan_type][
                    "last_update"
                ] = current_time

        # Ä°ÅŸ sÃ¼rekliliÄŸi testi
        if (
            current_time - self.crisis_system["business_continuity"]["last_test"]
            >= self.crisis_system["business_continuity"]["test_interval"]
        ):
        ...
        if (
            current_time - self.crisis_system["crisis_communication"]["last_review"]
            >= self.crisis_system["crisis_communication"]["review_interval"]
        ):
    ...
    def update_emergency_plan(self, plan_type):
        ...
        if random.random() < 0.3:  # %30 ÅŸans
            ...
            self.show_notification(
                f"{plan_type.replace('_', ' ').title()} acil durum planÄ± gÃ¼ncellendi!"
            )
    ...
    def test_business_continuity(self):
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            ...
            self.crisis_system["business_continuity"]["backup_systems"].append(
                new_system
            )
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            new_plan = {
                ...
                "scenario": random.choice(
                    ["financial_crisis", "market_crisis", "operational_crisis"]
                ),
    ...
    def review_crisis_communication(self):
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            ...
            self.crisis_system["crisis_communication"]["stakeholders"].append(
                new_stakeholder
            )
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            ...
            self.crisis_system["crisis_communication"]["communication_plans"].append(
                new_plan
            )
    ...
    def update_international_trade(self):
        ...
        if (
            current_time - self.international_trade["exchange_rates"]["last_update"]
            >= self.international_trade["exchange_rates"]["update_interval"]
        ):
        ...
        if (
            current_time - self.international_trade["customs_processes"]["last_check"]
            >= self.international_trade["customs_processes"]["check_interval"]
        ):
        ...
        if (
            current_time
            - self.international_trade["cultural_adaptation"]["last_review"]
            >= self.international_trade["cultural_adaptation"]["review_interval"]
        ):
            self.review_cultural_adaptation()
            self.international_trade["cultural_adaptation"][
                "last_review"
            ] = current_time

        # Tedarik zinciri denetimi
        if (
            current_time - self.international_trade["supply_chain"]["last_audit"]
            >= self.international_trade["supply_chain"]["audit_interval"]
        ):
    ...
    def check_customs_processes(self):
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            ...
            self.international_trade["customs_processes"]["documents"].append(
                new_document
            )
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            new_regulation = {
                ...
                "country": random.choice(
                    ["ABD", "AB", "Ã‡in", "Japonya", "Ä°ngiltere"]
                ),
                "impact": random.uniform(0.1, 0.3),
            }
            self.international_trade["customs_processes"]["regulations"].append(
                new_regulation
            )
    ...
    def review_cultural_adaptation(self):
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            ...
            self.international_trade["cultural_adaptation"]["markets"].append(
                new_market
            )
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            ...
            self.international_trade["cultural_adaptation"]["localization"].append(
                new_localization
            )
    ...
    def update_digital_transformation(self):
        ...
        if (
            current_time
            - self.digital_transformation["artificial_intelligence"]["last_update"]
            >= self.digital_transformation["artificial_intelligence"]["update_interval"]
        ):
            self.update_ai_systems()
            self.digital_transformation["artificial_intelligence"][
                "last_update"
            ] = current_time

        # Otomasyon denetimi
        if (
            current_time - self.digital_transformation["automation"]["last_audit"]
            >= self.digital_transformation["automation"]["audit_interval"]
        ):
        ...
        if (
            current_time
            - self.digital_transformation["data_analytics"]["last_analysis"]
            >= self.digital_transformation["data_analytics"]["analysis_interval"]
        ):
            self.analyze_data()
            self.digital_transformation["data_analytics"][
                "last_analysis"
            ] = current_time

        # Siber gÃ¼venlik kontrolÃ¼
        if (
            current_time - self.digital_transformation["cyber_security"]["last_check"]
            >= self.digital_transformation["cyber_security"]["check_interval"]
        ):
            self.check_security()
    ...
    def update_ai_systems(self):
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            ...
            self.digital_transformation["artificial_intelligence"]["models"].append(
                new_model
            )
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            new_application = {
                ...
                "department": random.choice(
                    ["finans", "pazarlama", "Ã¼retim", "insan_kaynaklari"]
                ),
                "efficiency": random.uniform(0.1, 0.3),
            }
            self.digital_transformation["artificial_intelligence"][
                "applications"
            ].append(new_application)
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            new_dashboard = {
                ...
                "update_frequency": random.choice(
                    ["gerÃ§ek zamanlÄ±", "saatlik", "gÃ¼nlÃ¼k"]
                ),
            }
            self.digital_transformation["data_analytics"]["dashboards"].append(
                new_dashboard
            )
    ...
    def check_security(self):
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            ...
            self.digital_transformation["cyber_security"]["defenses"].append(
                new_defense
            )
    ...
    def show_digital_panel(self):
        ...
        # Tehdit seviyesi
        threat_level = len(
            self.digital_transformation["cyber_security"]["threats"]
        ) / max(1, len(self.digital_transformation["cyber_security"]["defenses"]))
        Text(
            parent=self.digital_panel,
    ...
    def update_innovation_system(self):
        ...
        if (
            current_time - self.innovation_system["idea_management"]["last_review"]
            >= self.innovation_system["idea_management"]["review_interval"]
        ):
            self.review_ideas()
        ...
        if (
            current_time - self.innovation_system["patent_management"]["last_check"]
            >= self.innovation_system["patent_management"]["check_interval"]
        ):
            self.check_patents()
        ...
        if (
            current_time - self.innovation_system["rnd_projects"]["last_update"]
            >= self.innovation_system["rnd_projects"]["update_interval"]
        ):
        ...
        if (
            current_time - self.innovation_system["technology_transfer"]["last_audit"]
            >= self.innovation_system["technology_transfer"]["audit_interval"]
        ):
    ...
    def review_ideas(self):
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            new_idea = {
                ...
                "category": random.choice(
                    ["Ã¼rÃ¼n", "sÃ¼reÃ§", "hizmet", "iÅŸ modeli"]
                ),
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            new_evaluation = {
                "idea_id": random.randint(
                    0, len(self.innovation_system["idea_management"]["ideas"])
                ),
                "score": random.uniform(0.1, 1.0),
                "feedback": random.choice(["geliÅŸtir", "reddet", "beklet"]),
            }
            self.innovation_system["idea_management"]["evaluations"].append(
                new_evaluation
            )
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            new_application = {
                ...
                "status": random.choice(
                    ["beklemede", "deÄŸerlendirmede", "onaylandÄ±"]
                ),
                "priority": random.uniform(0.1, 1.0),
            }
            self.innovation_system["patent_management"]["applications"].append(
                new_application
            )
    ...
    def update_rnd_projects(self):
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            ...
            self.innovation_system["rnd_projects"]["active_projects"].append(
                new_project
            )
        ...
        for project in self.innovation_system["rnd_projects"]["active_projects"][:]:
            if project["progress"] >= 1.0:
                self.innovation_system["rnd_projects"]["completed_projects"].append(
                    project
                )
                self.innovation_system["rnd_projects"]["active_projects"].remove(
                    project
                )
    ...
    def audit_technology_transfer(self):
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            new_partnership = {
                ...
                "type": random.choice(
                    ["Ã¼niversite", "araÅŸtÄ±rma merkezi", "ÅŸirket"]
                ),
                "value": random.uniform(0.1, 0.5),
            }
            self.innovation_system["technology_transfer"]["partnerships"].append(
                new_partnership
            )
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            ...
            self.innovation_system["technology_transfer"]["licenses"].append(
                new_license
            )
    ...
    def update_crm_system(self):
        ...
        if (
            current_time - self.crm_system["customer_data"]["last_update"]
            >= self.crm_system["customer_data"]["update_interval"]
        ):
        ...
        if (
            current_time - self.crm_system["sales_history"]["last_check"]
            >= self.crm_system["sales_history"]["check_interval"]
        ):
        ...
        # MÃ¼ÅŸteri memnuniyeti anketi
        if (
            current_time - self.crm_system["customer_satisfaction"]["last_survey"]
            >= self.crm_system["customer_satisfaction"]["survey_interval"]
        ):
        ...
        if (
            current_time - self.crm_system["marketing_campaigns"]["last_review"]
            >= self.crm_system["marketing_campaigns"]["review_interval"]
        ):
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            {
                "customer_id": random.randint(
                    1, len(self.crm_system["customer_data"]["customers"])
                ),
    ...
    def update_scm_system(self):
        ...
        # TedarikÃ§i kontrolÃ¼
        if (
            current_time - self.scm_system["suppliers"]["last_check"]
            >= self.scm_system["suppliers"]["check_interval"]
        ):
        ...
        # Envanter denetimi
        if (
            current_time - self.scm_system["inventory"]["last_audit"]
            >= self.scm_system["inventory"]["audit_interval"]
        ):
        ...
        if (
            current_time - self.scm_system["logistics"]["last_update"]
            >= self.scm_system["logistics"]["update_interval"]
        ):
        ...
        # Risk deÄŸerlendirmesi
        if (
            current_time - self.scm_system["risk_management"]["last_assessment"]
            >= self.scm_system["risk_management"]["assessment_interval"]
        ):
            self.assess_risks()
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            new_evaluation = {
                "supplier_id": random.randint(
                    1, len(self.scm_system["suppliers"]["active_suppliers"])
                ),
                "score": random.uniform(0.1, 1.0),
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            new_shipment = {
                ...
                "to_location": random.choice(
                    ["mÃ¼ÅŸteri1", "mÃ¼ÅŸteri2", "mÃ¼ÅŸteri3"]
                ),
    ...
    def assess_risks(self):  # noqa: F811
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            new_mitigation = {
                "risk_id": random.randint(
                    1, len(self.scm_system["risk_management"]["risks"])
                ),
                "action": random.choice(
                    ["yedek tedarikÃ§i", "stok artÄ±rÄ±mÄ±", "alternatif rota"]
                ),
    ...
    def show_scm_panel(self):
        ...
        performance_score = (
            sum(
                s["reliability"]
                for s in self.scm_system["suppliers"]["active_suppliers"]
            )
            / max(1, len(self.scm_system["suppliers"]["active_suppliers"]))
            * 0.4
    ...
    def update_pm_system(self):
        ...
        if (
            current_time - self.pm_system["projects"]["last_review"]
            >= self.pm_system["projects"]["review_interval"]
        ):
        ...
        # Kaynak kontrolÃ¼
        if (
            current_time - self.pm_system["resources"]["last_check"]
            >= self.pm_system["resources"]["check_interval"]
        ):
        ...
        if (
            current_time - self.pm_system["scheduling"]["last_update"]
            >= self.pm_system["scheduling"]["update_interval"]
        ):
        ...
        if (
            current_time - self.pm_system["performance"]["last_assessment"]
            >= self.pm_system["performance"]["assessment_interval"]
        ):
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            new_team = {
                ...
                "skills": random.sample(
                    ["planlama", "geliÅŸtirme", "test", "dokÃ¼mantasyon"], 2
                ),
            }
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            new_task = {
                ...
                "project_id": random.randint(
                    1, len(self.pm_system["projects"]["active_projects"])
                ),
    ...
    def show_pm_panel(self):
        ...
        performance_score = (
            sum(p["progress"] for p in self.pm_system["projects"]["active_projects"])
            / max(1, len(self.pm_system["projects"]["active_projects"]))
            * 0.4
            + len(self.pm_system["resources"]["teams"]) / 10 * 0.2
            + sum(
                1
                for t in self.pm_system["scheduling"]["tasks"]
                if t["status"] == "tamamlandÄ±"
            )
            / max(1, len(self.pm_system["scheduling"]["tasks"]))
            * 0.2
            + sum(m["value"] for m in self.pm_system["performance"]["metrics"])
            / max(1, len(self.pm_system["performance"]["metrics"]))
            * 0.2
        ) * 10
        Text(
            parent=self.pm_panel,
    ...
    def update_hr_system(self):
        ...
        if (
            current_time - self.hr_system["personnel"]["last_update"]
            >= self.hr_system["personnel"]["update_interval"]
        ):
        ...
        if (
            current_time - self.hr_system["training"]["last_review"]
            >= self.hr_system["training"]["review_interval"]
        ):
        ...
        # Performans deÄŸerlendirmesi
        if (
            current_time - self.hr_system["performance"]["last_assessment"]
            >= self.hr_system["performance"]["assessment_interval"]
        ):
            self.assess_performance()
        ...
        # Kariyer kontrolÃ¼
        if (
            current_time - self.hr_system["career"]["last_check"]
            >= self.hr_system["career"]["check_interval"]
        ):
            self.check_career()
    ...
    def update_personnel(self):
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            new_employee = {
                ...
                "department": random.choice(
                    ["Ã¼retim", "pazarlama", "satÄ±ÅŸ", "ar-ge"]
                ),
                "position": random.choice(
                    ["uzman", "yÃ¶netici", "mÃ¼dÃ¼r", "direktÃ¶r"]
                ),
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            new_certification = {
                ...
                "employee_id": random.randint(
                    1, len(self.hr_system["personnel"]["employees"])
                ),
    ...
    def assess_performance(self):  # noqa: F811
        ...
        if random.random() < 0.2:  # %20 ÅŸans
            new_evaluation = {
                ...
                "criteria": random.choice(
                    ["iÅŸ kalitesi", "verimlilik", "takÄ±m Ã§alÄ±ÅŸmasÄ±", "liderlik"]
                ),
            }
    ...
    def show_hr_panel(self):
        ...
        hr_score = (
            ...
            + sum(p["score"] for p in self.hr_system["performance"]["evaluations"])
            / max(1, len(self.hr_system["performance"]["evaluations"]))
            * 0.3
            + sum(p["progress"] for p in self.hr_system["career"]["paths"])
            / max(1, len(self.hr_system["career"]["paths"]))
            * 0.2
    ...
    def update_health_system(self):
        ...
        # Mola kontrolÃ¼
        if (
            current_time - self.health_system["last_break"]
            >= self.health_system["recommended_break_interval"]
        ):
    ...
    def update_exercise_system(self):
        ...
        if (
            current_time - self.exercise_system["last_exercise"]
            >= self.exercise_system["exercise_interval"]
        ):
    ...
    def show_exercise_panel(self):
        ...
        # Son egzersiz zamanÄ±
        y_pos = 0.05
        last_exercise_minutes = (
            time.time() - self.exercise_system["last_exercise"]
        ) / 60
        Text(
            parent=self.exercise_panel,
