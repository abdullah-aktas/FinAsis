...
a = Analysis(
    ...
    datas=[
        ...
        ('{ursina_game_dir}/translations', 'translations'),
        ('{ursina_game_dir}/locales', 'locales'),
{extra_datas}
    ],
...
source.include_patterns = assets/*,models/*,textures/*,sounds/*,src/**
version = 1.0.0
...
GAMES = {
    "ticaretin_izinde_3d": {
        "display_name": "FinQuest 3D",
        "main_script": "main.py",
        "icon": "assets/icons/game_icon.ico",
        "package_name": "finquest3d",
        "console": False,
    }
}
...
def build_desktop(game_key):
    ...
    # Ek veri dosyaları (JSON) için liste oluştur
    extra_datas_list = []
    for json_file in URSINA_GAME_DIR.glob("*.json"):
        extra_datas_list.append((str(json_file).replace("\\", "/"), "."))
    extra_datas = ",\n        ".join(
        [f"('{src}', '{dst}')" for src, dst in extra_datas_list]
    )
    ...
    if result.returncode == 0:
        print(
            f"[SUCCESS] '{game['display_name']}' başarıyla derlendi! Dosya konumu: {DIST_DIR / 'desktop' / game['display_name']}.exe"
        )
        return True
    else:
        print(
            f"[ERROR] Derleme hatası: {result.stderr.decode('utf-8', errors='ignore')}"
        )
        return False
...
def build_mobile(game_key):
    ...
    # Gerekli dosyaları kopyala (assets kökte dursun)
    shutil.copytree(
        URSINA_GAME_DIR / "assets", mobile_build_dir / "assets", dirs_exist_ok=True
    )
    shutil.copytree(
        URSINA_GAME_DIR / "models", mobile_build_dir / "models", dirs_exist_ok=True
    )
    shutil.copytree(
        URSINA_GAME_DIR / "textures", mobile_build_dir / "textures", dirs_exist_ok=True
    )
    shutil.copytree(
        URSINA_GAME_DIR / "sounds", mobile_build_dir / "sounds", dirs_exist_ok=True
    )

    # Tüm src ağacını kopyala (finquest ve ursina_game modülleri dahil)
    shutil.copytree(PROJECT_ROOT, mobile_build_dir / "src", dirs_exist_ok=True)

    # Giriş noktası olarak main.py'yi köke kopyala
    shutil.copy2(URSINA_GAME_DIR / "main.py", mobile_build_dir / "main.py")

    # Kök JSON dosyalarını kopyala (oyun verileri)
    for json_file in URSINA_GAME_DIR.glob("*.json"):
        shutil.copy2(json_file, mobile_build_dir)
    ...
    if result.returncode == 0:
        print(
            f"[SUCCESS] '{game['display_name']}' başarıyla derlendi! APK konumu: {mobile_build_dir}/bin/"
        )
...
def main():
    ...
    parser.add_argument(
        ...
        default="ticaretin_izinde_3d",
        help="Derlenecek oyun (varsayılan: ticaretin_izinde_3d)",
    )
    parser.add_argument(
        "--desktop-only",
        action="store_true",
        help="Sadece masaüstü (.exe) sürümünü derle",
    )
    parser.add_argument(
        "--mobile-only", action="store_true", help="Sadece mobil (.apk) sürümünü derle"
    )
    ...
    if args.desktop_only:
        ...
        if desktop_success and mobile_success:
            print(
                f"[SUCCESS] '{GAMES[game_key]['display_name']}' tüm platformlar için başarıyla derlendi!"
            )
        else:
            print("[WARN] Bazı derleme işlemleri başarısız oldu.")
