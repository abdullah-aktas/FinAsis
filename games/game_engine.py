...
class GameEngine:
    ...
    def start_tournament(self, tournament_config: Dict):
        ...
        max_participants = rules.get("max_participants", tournament_config.get("max_participants", 64))
    ...
    def is_tournament_join_open(self, tournament_id: str, player_id: Optional[str] = None) -> Tuple[bool, str]:
        """
        ...
        if t.get("lock_on_start", True) and now_ts > t["start_time"] and len(t["participants"]) > 0:
            return False, "locked_after_start"
        if t.get("max_participants") and len(t["participants"]) >= t["max_participants"]:
    ...
    def end_tournament(self, tournament_id: str, force: bool = False) -> bool:
        ...
        if rankings:
            ...
            if winner_id in self.players:
                self.players[winner_id].stats["tournament_wins"] = self.players[winner_id].stats.get("tournament_wins", 0) + 1
        t["status"] = "finished"
        return True
    ...
    def _award_tournament_prizes(self, tournament: Dict, rankings: List[str]):
        ...
        if isinstance(prizes, dict):
            ...
            for i, pid in enumerate(rankings[:3]):
                self._grant_reward(pid, {"currency": 1000 * (3 - i), "experience": 500 * (3 - i)})
    ...
    def record_tournament_score(self, tournament_id: str, player_id: str, score_delta: int) -> bool:
    ...
    def get_tournament_leaderboard(self, tournament_id: str, top_n: Optional[int] = None) -> List[Dict]:
    ...
    def register_player(self, username: str, level: int = 1, experience: int = 0, currency: int = 0) -> str:
