...
from .models import (
    FinQuestCharacter, FinQuestItem, FinQuestInventory,
    FinQuestQuest, FinQuestPlayerQuest, FinQuestNPC,
    FinQuestWorld, FinQuestEvent, FinQuestSession
)


@admin.register(FinQuestCharacter)
class FinQuestCharacterAdmin(admin.ModelAdmin):
    list_display = ['name', 'user', 'level', 'current_city', 'money', 'difficulty', 'last_played']
    ...
    fieldsets = (
        ...
        ('İstatistikler', {
            'fields': ('level', 'xp', 'health', 'energy', 'money')
        }),
        ('Beceriler', {
            'fields': ('skill_trading', 'skill_negotiation', 'skill_accounting', 'skill_investing', 'skill_management')
        }),
        ('Progression', {
            'fields': ('quests_completed', 'cities_explored', 'achievements_unlocked')
        }),
    )
...
class FinQuestItemAdmin(admin.ModelAdmin):
    list_display = ['icon', 'name', 'item_type', 'buy_price', 'sell_price', 'is_tradeable']
...
class FinQuestQuestAdmin(admin.ModelAdmin):
    list_display = ['title', 'quest_type', 'required_level', 'difficulty_level', 'reward_xp', 'reward_money', 'is_active']
...
class FinQuestSessionAdmin(admin.ModelAdmin):
    list_display = ['character', 'started_at', 'duration_seconds', 'xp_earned', 'money_earned', 'quests_completed']
