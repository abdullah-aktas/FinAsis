...
def create_tutorial_world(game):
    ...
    # Walls
    game.walls = [
        Entity(model='cube', scale=Vec3(1, 5, 100), position=Vec3(-50, 2.5, 0), color=color.gray, texture='brick', collider='box'),
        Entity(model='cube', scale=Vec3(1, 5, 100), position=Vec3(50, 2.5, 0), color=color.gray, texture='brick', collider='box'),
        Entity(model='cube', scale=Vec3(100, 5, 1), position=Vec3(0, 2.5, 50), color=color.gray, texture='brick', collider='box'),
        Entity(model='cube', scale=Vec3(100, 5, 1), position=Vec3(0, 2.5, -50), color=color.gray, texture='brick', collider='box')
    ]

    # Tutorial buildings
    game.tutorial_buildings = [
        Entity(model='cube', scale=Vec3(10, 5, 10), position=Vec3(20, 2.5, 20), color=color.azure, texture='brick', collider='box'),
        Entity(model='cube', scale=Vec3(10, 7, 10), position=Vec3(-20, 3.5, -20), color=color.orange, texture='brick', collider='box'),
        Entity(model='cube', scale=Vec3(10, 4, 10), position=Vec3(20, 2, -20), color=color.green, texture='brick', collider='box'),
        Entity(model='cube', scale=Vec3(10, 6, 10), position=Vec3(-20, 3, 20), color=color.yellow, texture='brick', collider='box')
    ]

    # Building signs
    game.building_signs = [
        Entity(text='MARKET', scale=Vec3(2, 2, 2), position=Vec3(20, 6, 20), color=color.blue, billboard=True),
        Entity(text='BANKA', scale=Vec3(2, 2, 2), position=Vec3(-20, 8, -20), color=color.gold, billboard=True),
        Entity(text='OFİS', scale=Vec3(2, 2, 2), position=Vec3(20, 5, -20), color=color.lime, billboard=True),
        Entity(text='EĞİTİM MERKEZİ', scale=Vec3(2, 2, 2), position=Vec3(-20, 6.5, 20), color=color.red, billboard=True)
    ]

    # Player controller
    game.player_controller = FirstPersonController(
        position=Vec3(0, 1, 0),
        speed=10
    )
...
def create_city_center(game):
    ...
    # Roads
    roads = [
        Entity(model='cube', scale=Vec3(100, 0.1, 10), position=Vec3(0, 0.05, 0), color=color.gray, texture='asphalt'),
        Entity(model='cube', scale=Vec3(10, 0.1, 100), position=Vec3(0, 0.05, 0), color=color.gray, texture='asphalt')
    ]
    ...
    for building_data in building_positions:
        building = Entity(
            model='cube',
            position=Vec3(building_data['position'][0], building_data['scale'][1] / 2, building_data['position'][2]),
        ...
        door = Entity(
            model='cube',
            parent=building,
            position=Vec3(0, -building_data['scale'][1] / 2 + 1, building_data['scale'][2] / 2 + 0.1),
            scale=Vec3(3, 5, 0.2),
            color=color.brown,
            collider='box'
        )
        ...
        entry_point = Entity(
            model='sphere',
            parent=building,
            position=Vec3(0, -building_data['scale'][1] / 2 + 1, building_data['scale'][2] / 2 + 2),
        ...
        trigger = Entity(
            parent=building,
            position=Vec3(0, -building_data['scale'][1] / 2 + 1, building_data['scale'][2] / 2 + 3),
            scale=Vec3(5, 5, 5),
            collider='box',
            visible=False
        )
        ...
        def on_trigger_enter(trigger=trigger):
            ...
            game.show_interaction_prompt(f"'{trigger.building_type.capitalize()}' binasına girmek için E tuşuna basın")
    ...
    for _ in range(50):
        ...
        if not too_close:
            tree = Entity(model='cube', position=Vec3(x, 2, z), scale=Vec3(1, 4, 1), color=color.brown)
            leaves = Entity(model='sphere', parent=tree, position=Vec3(0, 2, 0), scale=Vec3(3, 3, 3), color=color.green)
    ...
    for _ in range(10):
        ...
        bench = Entity(model='cube', position=Vec3(x, 0.5, z), scale=Vec3(2, 0.5, 0.5), color=color.brown)
