...
class MarketEngine:
    ...
    def calculate_new_price(self, asset):
        ...
        price_change_percent = (
            trend_effect + momentum_effect + market_noise + event_impact
        )
    ...
    def _get_market_event_impact(self, asset):
        ...
        active_events = MarketEvent.objects.filter(
            is_active=True, event_date__lte=now, expires_at__gte=now
        ).filter(affected_assets=asset)
        ...
        for event in active_events:
            ...
            total_impact += (
                float(event.price_impact_percentage) * impact_multiplier / 100
            )

        return total_impact

    def create_market_event(
        self, event_type, impact_level, affected_assets, price_impact
    ):
    ...
    def simulate_random_event(self):
        ...
        return self.create_market_event(
            event_type, impact_level, affected_assets, price_impact
        )
...
class PortfolioAnalyzer:
    ...
    def analyze_portfolio(profile):
        ...
        for holding in holdings:
            if holding.asset and total_value > 0:
                ...
                risk_value = {"low": 1, "medium": 2, "high": 3, "very_high": 4}.get(
                    holding.asset.risk_level, 2
                )
        ...
        if diversification_score < 40:
            recommendations.append(
                "Portföyünüzü daha fazla çeşitlendirin. Farklı yatırım araçlarına yatırım yapın."
            )

        if risk_score > 2.5:
            recommendations.append(
                "Portföyünüz yüksek riskli. Daha güvenli araçlara yatırım yapmayı düşünün."
            )
        elif risk_score < 1.5:
            recommendations.append(
                "Portföyünüz çok güvenli. Daha yüksek getiri için orta riskli araçlar ekleyebilirsiniz."
            )
        ...
        if bullish_assets:
            recommendations.append(
                f"{len(bullish_assets)} yatırımınız yükseliş trendinde. Bu fırsatı değerlendirin!"
            )

        bearish_assets = [h for h in holdings if h.asset.trend == "bearish"]
        if bearish_assets:
            recommendations.append(
                f"{len(bearish_assets)} yatırımınız düşüş trendinde. Risk yönetimi yapın!"
            )
    ...
    def get_suggested_assets(profile, limit=5):
        ...
        owned_asset_ids = Portfolio.objects.filter(profile=profile).values_list(
            "asset_id", flat=True
        )
        ...
        suggested = (
            ...
            .exclude(id__in=owned_asset_ids)
            .order_by("-momentum", "-expected_return")[:limit]
        )
