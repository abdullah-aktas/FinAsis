...
def request_approval(request, app_label, model, pk):
    ...
    AuditLog.log_action(obj=obj, action='approval_request', user=request.user, ip=request.META.get('REMOTE_ADDR'))
...
def approval_action(request, pk, action):
    ...
    if action == 'approve':
        ...
        AuditLog.log_action(obj=ar.content_object, action='approved', user=request.user, ip=request.META.get('REMOTE_ADDR'))
    elif action == 'reject':
        ar.reject(user=request.user)
        AuditLog.log_action(obj=ar.content_object, action='rejected', user=request.user, ip=request.META.get('REMOTE_ADDR'))
    else:
