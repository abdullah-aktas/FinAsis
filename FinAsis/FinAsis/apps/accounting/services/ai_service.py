from FinAsis.apps.finance.accounting.models import Voucher, VoucherLine, Account, VoucherType, FiscalYear, Currency
from FinAsis.apps.accounting.models import Company
from decimal import Decimal
from typing import Dict
from FinAsis.apps.ai_assistant.services.ml_service import SimpleVoucherClassifier
...
def map_ocr_to_voucher_lines(company: Company, ocr_data: Dict) -> Dict:
    ...
    try:
        ...
        tax_rate = Decimal('0')
    kdv = (total * tax_rate / Decimal('100')).quantize(Decimal('0.01')) if tax_rate else Decimal('0.00')
    matrah = (total - kdv).quantize(Decimal('0.01')) if total else Decimal('0.00')

    def get_account(code: str, name_fallback: str) -> Account:
        acc, _ = Account.objects.get_or_create(company=company, code=code, defaults={
            'name': name_fallback,
            'type_id': 1
        })
        return acc
    ...
    rule_qs = AutoBookingRule.objects.filter(company=company, is_active=True).order_by('priority')
    nature = None
    selected_rule = None
    import re
    for r in rule_qs:
    ...
    account_kdv = get_account(getattr(selected_rule, 'kdv_account_code', None) or '191', 'İndirilecek KDV')
    account_saticilar = get_account(getattr(selected_rule, 'credit_account_code', None) or '320', 'Satıcılar')
    account_alicilar = get_account(getattr(selected_rule, 'debit_account_code', None) or '120', 'Alıcılar')
...
def create_voucher_from_lines(company: Company, mapped: Dict) -> Voucher:
    fiscal_year, _ = FiscalYear.objects.get_or_create(company=company, year=timezone.now().year, defaults={
        'start_date': timezone.now().date().replace(month=1, day=1),
        'end_date': timezone.now().date().replace(month=12, day=31),
    })
    vt, _ = VoucherType.objects.get_or_create(code='MM', defaults={'name': 'Muhasebe Fişi', 'prefix': 'MM'})
    currency, _ = Currency.objects.get_or_create(code=getattr(company, 'base_currency', 'TRY'), defaults={'name': 'Para Birimi', 'symbol': getattr(company, 'base_currency', 'TRY')})
...
def map_text_to_voucher_lines(company: Company, text: str) -> Dict:
    ...
    ref_match = re.search(r'(fatura|fis|belge)\s*no[:\s]*([a-z0-9-]+)', (text or '').lower())
    ...
    ocr_like = {'total': total_str, 'tax_rate': tax_rate_str, 'invoice_number': reference, 'company_name': ''}
