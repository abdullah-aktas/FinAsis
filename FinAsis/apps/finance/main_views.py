...
from .models import CashFlow, IncomeStatement, Account, Transaction, Budget, FinancialReport, Tax, BaseModel, BankAccount, Invoice, EInvoice, EInvoiceItem, Employee, Voucher, AIConfig
from .serializers import TransactionSerializer, BudgetSerializer, AccountSerializer, FinancialReportSerializer, TaxSerializer
...
from .forms import TransactionForm, AccountForm, BudgetForm, TaxForm, CashFlowForm, IncomeStatementForm, BankAccountForm, InvoiceForm, FinancialReportForm, EInvoiceForm, EInvoiceItemForm, EmployeeForm, VoucherForm
...
def finance_dashboard(request):
    ...
    today = date.today()
    months = [(today.replace(day=1) - timedelta(days=30*i)).replace(day=1) for i in reversed(range(6))]
    ...
    for m in months:
        ...
        income = Transaction.objects.filter(
            ...
            type='DEBIT', status='POSTED', account__user=request.user
        ).aggregate(total=Sum('amount'))['total'] or 0
        expense = Transaction.objects.filter(
            date__gte=month_start, date__lt=month_end,
            type='CREDIT', status='POSTED', account__user=request.user
        ).aggregate(total=Sum('amount'))['total'] or 0
    ...
    credit_cards = [
        {'name': 'Akbank Axess', 'debt': 3200, 'limit': 8000, 'usage_percent': int(3200/8000*100)},
        {'name': 'Yapı Kredi World', 'debt': 1500, 'limit': 6000, 'usage_percent': int(1500/6000*100)},
    ]
    ...
    if total_expense > total_income:
        ai_advice = "Giderleriniz gelirlerinizden fazla. Harcamaları kontrol etmenizi öneririz."
    else:
...
def download_financial_report(request):
    ...
    ai_advice = get_ai_financial_advice(request.user, total_income, total_expense, total_investment)
