...
class UserRoleDeleteView(LoginRequiredMixin, PermissionRequiredMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        self.object.is_active = False
        self.object.save()
        messages.success(self.request, _('Kullanıcı rolü başarıyla silindi (pasif yapıldı).'))
        return super(DeleteView, self).delete(request, *args, **kwargs)
...
class UserRoleViewSet(viewsets.ModelViewSet):
    ...
    def perform_destroy(self, instance: UserRole) -> None:
        ...
        if not self.request.user.is_superuser:
            raise PermissionDenied(_("Sadece süperuser kullanıcılar kullanıcı rolü silebilir."))
...
class IsRoleManager(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        return request.user.is_superuser or request.user.groups.filter(name='RoleManager').exists()
