...
def home(request):
    ...
    # Toplam gelir
    total_income = Invoice.objects.filter(company__in=companies, is_active=True).aggregate(models.Sum('total_amount'))['total_amount__sum'] or 0
    # Toplam gider
    total_expense = Expense.objects.filter(company__in=companies, is_active=True).aggregate(models.Sum('amount'))['amount__sum'] or 0
    # Son 5 fatura
    ...
    # Son 5 banka işlemi
    last_banktransactions = BankTransaction.objects.filter(company__in=companies).order_by('-created_at')[:5]
    context = {
