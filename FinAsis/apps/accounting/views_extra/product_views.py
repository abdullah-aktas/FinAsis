...
def product_update(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    return render(request, 'accounting/product_form.html', {'form': form, 'product': product})
...
def product_delete(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    if request.method == 'POST':
        product.delete()
        return redirect('accounting:product_list')
    return render(request, 'accounting/product_confirm_delete.html', {'product': product}) 