...
class PayTRClient:
    @staticmethod
    def _hash_token(merchant_id, user_ip, merchant_oid, email, payment_amount, user_basket_b64, no_installment, max_installment, currency, test_mode, merchant_salt, merchant_key):
    ...
    def init_payment(payload: dict) -> dict:
        ...
        return {
            'status': 'success',
            'iframe_token': 'sandbox_token_example'
        }

    @staticmethod
    def verify_callback(merchant_oid: str, status: str, total_amount: str, hash_str: str, request_ip: str | None = None) -> bool:
        ...
        if not (merchant_salt and merchant_key and merchant_oid and status and total_amount and hash_str):
            return False
        ...
        expected = base64.b64encode(hmac.new(key, message, hashlib.sha256).digest()).decode('utf-8')
        ...
        if expected == hash_str:
            return True
        fallback = base64.b64encode(hashlib.sha256((f"{merchant_oid}{merchant_salt}{status}{total_amount}" + merchant_key).encode('utf-8')).digest()).decode('utf-8')
