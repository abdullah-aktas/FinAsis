...
from .views import DeclarationViewSet, SubmissionViewSet, SubmissionLogViewSet
from django.contrib.auth.decorators import login_required
from django.shortcuts import render

def submissions_list_view(request):
    from .models import Submission
    qs = Submission.objects.select_related('declaration').filter(
        submitted_by=request.user
    ).order_by('-submitted_at', '-id') if request.user.is_authenticated else []
