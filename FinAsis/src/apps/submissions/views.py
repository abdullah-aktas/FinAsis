...
from .serializers import DeclarationSerializer, SubmissionSerializer, SubmissionLogSerializer
from .permissions import AdvisorOnlySubmission
from src.apps.advisors.models import Engagement
...
class SubmissionViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        has_engagement = Engagement.objects.filter(advisor=advisor, taxpayer__vkn_tckn=vkn, status='active').exists()
    ...
    def send(self, request, pk=None):
        ...
        if submission.submitted_by_id != request.user.id and submission.declaration.created_by_id != request.user.id:
    ...
    def validate(self, request, pk=None):
        ...
        msg = 'AI ön kontrol tamamlandı.' + (f" Uyarılar: {', '.join(issues)}" if issues else '')
        SubmissionLog.objects.create(
            submission=submission,
            level=level,
            message=msg,
...
class SubmissionLogViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return qs.filter(Q(submission__submitted_by=user) | Q(submission__declaration__created_by=user))
