...
def build_ubl_invoice_xml(*, code: str, period: str, taxpayer_vkn_tckn: str, payload: dict | None) -> bytes:
    """
    ...
    if currency:
        cbc_doc_currency = ET.SubElement(root, ET.QName(nsmap['cbc'], 'DocumentCurrencyCode'))
    ...
    cac_supplier = ET.SubElement(root, ET.QName(nsmap['cac'], 'AccountingSupplierParty'))
    ...
    if isinstance(supplier, dict):
        ...
        endpoint = ET.SubElement(cac_supplier_party, ET.QName(nsmap['cbc'], 'EndpointID'))
        endpoint.text = str(supplier.get('vkn_tckn') or taxpayer_vkn_tckn)
        party_name = supplier.get('name')
        if party_name:
            cac_party_name = ET.SubElement(cac_supplier_party, ET.QName(nsmap['cac'], 'PartyName'))
    ...
    if isinstance(customer, dict):
        cac_customer = ET.SubElement(root, ET.QName(nsmap['cac'], 'AccountingCustomerParty'))
        cac_customer_party = ET.SubElement(cac_customer, ET.QName(nsmap['cac'], 'Party'))
        endpoint = ET.SubElement(cac_customer_party, ET.QName(nsmap['cbc'], 'EndpointID'))
        endpoint.text = str(customer.get('vkn_tckn') or '')
        party_name = customer.get('name')
        if party_name:
            cac_party_name = ET.SubElement(cac_customer_party, ET.QName(nsmap['cac'], 'PartyName'))
            cbc_name = ET.SubElement(cac_party_name, ET.QName(nsmap['cbc'], 'Name'))
            cbc_name.text = str(party_name)
    ...
    if isinstance(lines, list):
        for idx, line in enumerate(lines, start=1):
            ...
            ET.SubElement(il, ET.QName(nsmap['cbc'], 'InvoicedQuantity')).text = str(qty)
            ET.SubElement(il, ET.QName(nsmap['cbc'], 'LineExtensionAmount')).text = f"{ext:.2f}"
            # Item name (optional)
            item_name = line.get('name')
            if item_name:
                item = ET.SubElement(il, ET.QName(nsmap['cac'], 'Item'))
                ET.SubElement(item, ET.QName(nsmap['cbc'], 'Name')).text = str(item_name)
            # Price (optional structure)
            price_el = ET.SubElement(il, ET.QName(nsmap['cac'], 'Price'))
            ET.SubElement(price_el, ET.QName(nsmap['cbc'], 'PriceAmount')).text = f"{price:.2f}"

    # Tax total (optional)
    if total_tax_amt > 0:
        tax_total = ET.SubElement(root, ET.QName(nsmap['cac'], 'TaxTotal'))
        ET.SubElement(tax_total, ET.QName(nsmap['cbc'], 'TaxAmount')).text = f"{total_tax_amt:.2f}"

    # LegalMonetaryTotal (optional minimal)
    if line_total > 0:
        lmt = ET.SubElement(root, ET.QName(nsmap['cac'], 'LegalMonetaryTotal'))
        ET.SubElement(lmt, ET.QName(nsmap['cbc'], 'LineExtensionAmount')).text = f"{line_total:.2f}"
        ET.SubElement(lmt, ET.QName(nsmap['cbc'], 'TaxExclusiveAmount')).text = f"{line_total:.2f}"
        ET.SubElement(lmt, ET.QName(nsmap['cbc'], 'TaxInclusiveAmount')).text = f"{(line_total + total_tax_amt):.2f}"
...
def validate_xml_against_xsd(xml_bytes: bytes, schemas_dir: str | None = None) -> List[str]:
    """
    ...
    try:
        ...
        return []
    except ET.DocumentInvalid as e:  # schema validation errors
        # Return lines of error log
        return [str(err) for err in schema.error_log]  # type: ignore[name-defined]
    except Exception as e:  # Parsing or IO problems – surface as generic single error line
