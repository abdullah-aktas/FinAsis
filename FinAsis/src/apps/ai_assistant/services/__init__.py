...
# OCR ve ML servisleri opsiyonel bağımlılıklara sahip olduğundan korumalı içe aktarım kullanıyoruz.
try:
    from .ocr_service import OCRService
except Exception:
    OCRService = None  # isteğe bağlı; kullanırken doğrulanır

try:
    from .ml_service import RecommendationService
except Exception:
    class RecommendationService:  # type: ignore
        def __init__(self, *args, **kwargs):
            raise RuntimeError(
                "ML servisleri kullanılamıyor: 'prophet' veya bağlı paketler kurulu değil."
            )
try:
