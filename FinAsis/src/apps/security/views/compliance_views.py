...
from src.apps.finance.data_security_compliance import (
    PersonalDataRecord,
    DataSubjectRequest,
    SecurityIncident,
    DataBackup,
...
def compliance_dashboard(request):
    ...
    compliance_score = getattr(compliance_checker, 'calculate_overall_compliance_score', lambda: 0)()
    ...
    for category in PersonalDataCategory.objects.filter(company=company)[:6]:
        ...
        data_categories.append({
            ...
            'sensitivity_level': (getattr(category, 'sensitivity_level', '') or '').lower(),
            'sensitivity_text': getattr(category, 'get_sensitivity_level_display', lambda: '')(),
    ...
    for request_obj in data_requests.order_by('-created_at')[:5]:
        ...
        days_remaining = (due_date_date - today).days if (due_date_date and isinstance(due_date_date, type(today))) else None
        recent_requests.append({
            'id': getattr(request_obj, 'id', None),
            'request_type': getattr(request_obj, 'request_type', ''),
            'request_type_text': getattr(request_obj, 'get_request_type_display', lambda: '')(),
    ...
    for incident in security_incidents.order_by('-incident_date')[:5]:
        recent_incidents.append({
            ...
            'incident_type_text': getattr(incident, 'get_incident_type_display', lambda: '')(),
            'severity': (getattr(incident, 'severity', '') or '').lower(),
            'severity_text': getattr(incident, 'get_severity_display', lambda: '')(),
    ...
    for backup in data_backups.order_by('-backup_date')[:5]:
        backup_status.append({
            ...
            'backup_type_text': getattr(backup, 'get_backup_type_display', lambda: '')(),
    ...
    compliance_items = getattr(compliance_checker, 'get_compliance_checklist', lambda: [])()
    ...
    last_key_obj = encryption_keys.order_by('-created_at').first() if encryption_keys.exists() else None
    context['encryption_status'] = {
        'active_keys': encryption_keys.count(),
        'key_strength': _('AES-256') if encryption_keys.exists() else _('Şifreleme yapılandırılmamış'),
...
def data_subject_requests(request):
    ...
    # Mevcut talepler
    requests = DataSubjectRequest.objects.filter(company=company).order_by('-created_at')
...
def personal_data_inventory(request):
    ...
    if request.method == 'POST':
        try:
            ...
            category = PersonalDataCategory.objects.create(
                ...
                retention_period_months=int(request.POST.get('retention_period_months', 12)),
                created_by=request.user,
            )
    ...
    # Veri kategorileri
    categories = PersonalDataCategory.objects.filter(company=company).order_by('category_name')
    ...
    for category in categories:
        try:
            ...
            rc = 0
        category_usage.append({'category_id': getattr(category, 'id', None), 'records_count': rc})
    
    # Hassasiyet seviyeleri istatistiği
    try:
        sensitivity_stats = categories.values('sensitivity_level').annotate(
            count=Count('id')
    ...
    if request.method == 'POST':
        try:
            ...
            incident = SecurityIncident.objects.create(
                ...
                affected_individuals_count=int(request.POST.get('affected_individuals_count', 0)),
                reported_by=request.user,
                incident_date=request.POST.get('incident_date') or timezone.now().date(),
            )
            ...
            if incident.severity in ['HIGH', 'CRITICAL']:
                ...
                messages.warning(request, _('Kritik güvenlik olayı kaydedildi. İlgili otoriteler bilgilendirilmeli.'))
            
            messages.success(request, _('Güvenlik olayı başarıyla kaydedildi.'))
            return redirect('security:security_incidents')
            
        except Exception as e:
            messages.error(request, f'Güvenlik olayı kaydedilirken hata oluştu: {str(e)}')
    
    # Güvenlik olayları
    incidents = SecurityIncident.objects.filter(company=company).order_by('-incident_date')
        ...
        try:
            ...
            if action == 'create_backup':
                ...
                backup = DataBackup.objects.create(
                    ...
                    retention_date=timezone.now().date() + timedelta(
                        days=int(request.POST.get('retention_days', 90))
                    ),
                    created_by=request.user,
                    status='IN_PROGRESS'
                )
                ...
                messages.success(request, f'"{backup.backup_type}" yedeklemesi geri yüklendi.')
...
def encryption_management(request):
    ...
    if request.method == 'POST':
        try:
            ...
            if action == 'generate_key':
                ...
                messages.success(request, _('Şifreleme anahtarı başarıyla oluşturuldu.'))
    ...
    key_type_stats = keys.values('key_type').annotate(
        count=Count('id')
    ).order_by('key_type')
...
def ajax_compliance_check(request):
    ...
    compliance_checker = GDPRComplianceChecker(company)
    score = getattr(compliance_checker, 'calculate_overall_compliance_score', lambda: 0)()
    
    return JsonResponse({
        'compliance_score': score,
        'status': get_gdpr_compliance_status(score),
    'recommendations': getattr(compliance_checker, 'get_improvement_recommendations', lambda: [])(),
    })
...
def ajax_security_metrics(request):
    ...
    metrics = {
        'security_incidents': SecurityIncident.objects.filter(
            company=company,
            incident_date__gte=last_30_days.date()
        ).count(),
        ...
        'successful_backups': DataBackup.objects.filter(
            company=company,
            backup_date__gte=last_30_days.date(),
            status='SUCCESSFUL'
        ).count(),
...
def get_gdpr_compliance_description(score):
    ...
    """
    if score >= 90:
        return _('GDPR/KVKK gerekliliklerine tam uyum sağlanmış. Mevcut süreçleri koruyun.')
    elif score >= 80:
...
def calculate_data_security_risks(company):
    ...
    recent_incidents = SecurityIncident.objects.filter(
        company=company,
        incident_date__gte=timezone.now() - timedelta(days=90)
    ).count()
