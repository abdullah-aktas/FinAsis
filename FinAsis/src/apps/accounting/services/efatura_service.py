...
def generate_invoice_xml(invoice: Invoice) -> bytes:
    ...
    SubElement(supplier, 'Name').text = getattr(invoice.company, 'name', '') if getattr(invoice, 'company', None) else ''
    SubElement(supplier, 'TaxNumber').text = getattr(invoice.company, 'tax_number', '') if getattr(invoice, 'company', None) else ''
    ...
    if items:
        for it in items:
            ...
            SubElement(line, 'Description').text = it.description or getattr(it.product, 'name', '') or ''
    ...
    if kdv_rate is not None:
        SubElement(tax_total, 'TaxAmount').text = str(float(invoice.total_amount) * float(kdv_rate))
...
def send_invoice_to_gib(invoice: Invoice):
    ...
    endpoint = 'sendEArchive' if hasattr(invoice, 'e_archive') and getattr(invoice, 'e_archive', False) else 'sendInvoice'
    url = f"{base_url}/{endpoint}"
    headers = {"Content-Type": "application/xml"}
    try:
        response = requests.post(url, data=xml_bytes, headers=headers, auth=(settings.GIB_USERNAME, settings.GIB_PASSWORD))
...
def check_invoice_status(invoice: Invoice):
    base_url = settings.GIB_EFATURA_BASE_URL
    endpoint = 'earchiveStatus' if hasattr(invoice, 'e_archive') and getattr(invoice, 'e_archive', False) else 'invoiceStatus'
