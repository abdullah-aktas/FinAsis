...
def expense_update(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    return render(request, 'accounting/expense_form.html', {'form': form, 'expense': expense})
...
def expense_delete(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    if request.method == 'POST':
        expense.delete()
        return redirect('accounting:expense_list')
    return render(request, 'accounting/expense_confirm_delete.html', {'expense': expense}) 