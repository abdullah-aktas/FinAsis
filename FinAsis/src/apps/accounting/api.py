...
def ocr_confirm_voucher(request):
    ...
    from src.apps.finance.accounting.models import Account
    resolved_lines = []
    ...
    return Response({'status': 'created', 'voucher_id': voucher_id}, status=status.HTTP_201_CREATED)
...
def derive_rule_from_preview(request):
    ...
    return Response({'status': 'created', 'rule_id': getattr(rule, 'id', None)}, status=status.HTTP_201_CREATED)
