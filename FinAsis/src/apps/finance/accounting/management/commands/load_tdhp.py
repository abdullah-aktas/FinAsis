...
from src.apps.finance.accounting.models import Account, AccountType
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--csv', type=str, default='', help='Path to CSV (code,name,type_code). If omitted, uses bundled data/tdhp.csv')
        parser.add_argument('--strict', action='store_true', help='Enable strict TDHP code validation during load')
    ...
    def handle(self, *args, **options):
        ...
        if not csv_path:
            csv_path = Path(__file__).resolve().parent.parent.parent / 'data' / 'tdhp.csv'
        else:
        ...
        def _load():
            ...
            with csv_path.open('r', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    atype, _ = AccountType.objects.get_or_create(code=type_code, defaults={'name': type_code})
                    parent_code = None
                    if '.' in code:
                    ...
                    parent = None
                    if parent_code:
                        parent, _ = Account.objects.get_or_create(company=company, code=parent_code, defaults={
                            'name': parent_code,
                            'type': atype,
                        })
        ...
        self.stdout.write(self.style.SUCCESS(_('Done. Created: %d, Updated: %d') % (created, updated)))
