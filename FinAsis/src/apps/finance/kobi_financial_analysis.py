...
from django.db.models import Sum, Avg, F, Q, Case, When, DecimalField
...
class KOBIFinancialAnalysis(models.Model):
    ...
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name='financial_analyses')
    analysis_date = models.DateField(_('Analiz Tarihi'))
    period_type = models.CharField(_('Dönem Tipi'), max_length=20, choices=ANALYSIS_PERIODS)
    start_date = models.DateField(_('Başlangıç Tarihi'))
    end_date = models.DateField(_('Bitiş Tarihi'))
    
    # Likidite Oranları
    current_ratio = models.DecimalField(_('Cari Oran'), max_digits=8, decimal_places=4, null=True, blank=True)
    quick_ratio = models.DecimalField(_('Asit Test Oranı'), max_digits=8, decimal_places=4, null=True, blank=True)
    cash_ratio = models.DecimalField(_('Nakit Oranı'), max_digits=8, decimal_places=4, null=True, blank=True)
    
    # Karlılık Oranları
    gross_profit_margin = models.DecimalField(_('Brüt Kar Marjı %'), max_digits=8, decimal_places=4, null=True, blank=True)
    net_profit_margin = models.DecimalField(_('Net Kar Marjı %'), max_digits=8, decimal_places=4, null=True, blank=True)
    return_on_assets = models.DecimalField(_('Aktif Karlılığı %'), max_digits=8, decimal_places=4, null=True, blank=True)
    return_on_equity = models.DecimalField(_('Özkaynak Karlılığı %'), max_digits=8, decimal_places=4, null=True, blank=True)
    
    # Faaliyet Oranları
    asset_turnover = models.DecimalField(_('Aktif Devir Hızı'), max_digits=8, decimal_places=4, null=True, blank=True)
    inventory_turnover = models.DecimalField(_('Stok Devir Hızı'), max_digits=8, decimal_places=4, null=True, blank=True)
    receivables_turnover = models.DecimalField(_('Alacak Devir Hızı'), max_digits=8, decimal_places=4, null=True, blank=True)
    
    # Kaldıraç Oranları
    debt_to_equity = models.DecimalField(_('Borç/Özkaynak Oranı'), max_digits=8, decimal_places=4, null=True, blank=True)
    debt_to_assets = models.DecimalField(_('Borç/Aktif Oranı'), max_digits=8, decimal_places=4, null=True, blank=True)
    equity_multiplier = models.DecimalField(_('Özkaynak Çarpanı'), max_digits=8, decimal_places=4, null=True, blank=True)
    
    # Toplam değerler (hesaplamalar için)
    total_assets = models.DecimalField(_('Toplam Aktif'), max_digits=15, decimal_places=2, default=0)
    total_liabilities = models.DecimalField(_('Toplam Borç'), max_digits=15, decimal_places=2, default=0)
    total_equity = models.DecimalField(_('Toplam Özkaynak'), max_digits=15, decimal_places=2, default=0)
    total_revenue = models.DecimalField(_('Toplam Gelir'), max_digits=15, decimal_places=2, default=0)
    net_income = models.DecimalField(_('Net Kar'), max_digits=15, decimal_places=2, default=0)
    
    # Analiz sonuçları
    financial_health_score = models.DecimalField(_('Mali Sağlık Skoru'), max_digits=5, decimal_places=2, 
    ...
    def calculate_all_ratios(self):
        ...
        generator = FinancialStatementGenerator(self.company, self.start_date, self.end_date)
        ...
        self.total_liabilities = (balance_sheet['pasif']['kisa_vadeli_borclar'].get('total', 0) + 
    ...
    def _calculate_profitability_ratios(self):
        ...
        # Net kar marjı
        if satislar > 0 and self.net_income:
            self.net_profit_margin = (self.net_income / satislar) * 100
        
        # Aktif karlılığı (ROA)
        if self.total_assets > 0 and self.net_income:
            self.return_on_assets = (self.net_income / self.total_assets) * 100
        
        # Özkaynak karlılığı (ROE)
        if self.total_equity > 0 and self.net_income:
    ...
    def _calculate_financial_health_score(self):
        ...
        if self.net_profit_margin:
            if self.net_profit_margin >= 15:
                score += Decimal('25')
            elif self.net_profit_margin >= 10:
                score += Decimal('20')
            elif self.net_profit_margin >= 5:
                score += Decimal('15')
        ...
        if self.asset_turnover:
            if self.asset_turnover >= 2:
                score += Decimal('25')
            elif self.asset_turnover >= 1.5:
                score += Decimal('20')
            elif self.asset_turnover >= 1:
                score += Decimal('15')
    ...
    def _generate_recommendations(self):
        ...
        # Likidite önerileri
        if self.current_ratio and self.current_ratio < 1:
        ...
        elif self.current_ratio and self.current_ratio > 3:
            recommendations.append({
                'type': 'liquidity',
                'priority': 'medium',
        ...
        # Karlılık önerileri
        if self.net_profit_margin and self.net_profit_margin < 5:
            recommendations.append({
                'type': 'profitability',
                'priority': 'high',
        ...
        # Borç önerileri
        if self.debt_to_equity and self.debt_to_equity > 2:
            recommendations.append({
                'type': 'leverage',
                'priority': 'high',
        ...
        # Faaliyet önerileri
        if self.inventory_turnover and self.inventory_turnover < 4:
            recommendations.append({
                'type': 'operations',
                'priority': 'medium',
...
class BudgetPlan(models.Model):
    ...
    total_budgeted_amount = models.DecimalField(_('Toplam Bütçe'), max_digits=15, decimal_places=2, default=0)
    total_actual_amount = models.DecimalField(_('Toplam Gerçekleşen'), max_digits=15, decimal_places=2, default=0)
    variance_amount = models.DecimalField(_('Sapma Tutarı'), max_digits=15, decimal_places=2, default=0)
    variance_percentage = models.DecimalField(_('Sapma Yüzdesi'), max_digits=8, decimal_places=4, default=0)
    ...
    def calculate_variance(self):
        ...
        if self.total_budgeted_amount != 0:
            self.variance_percentage = (self.variance_amount / self.total_budgeted_amount) * 100
        self.save(update_fields=['variance_amount', 'variance_percentage'])
    
    def update_actuals(self):
        """Gerçekleşen tutarları güncelle"""
        total_actual = self.budget_lines.aggregate(
...
class BudgetLine(models.Model):
    ...
    budget_plan = models.ForeignKey(BudgetPlan, on_delete=models.CASCADE, 
                                   related_name='budget_lines', verbose_name=_('Bütçe Planı'))
    account = models.ForeignKey('finance.ChartOfAccounts', on_delete=models.PROTECT,
                               related_name='budget_lines', verbose_name=_('Hesap'))
    cost_center = models.ForeignKey('finance.CostCenter', on_delete=models.SET_NULL, 
                                   null=True, blank=True, verbose_name=_('Masraf Merkezi'))
    
    line_number = models.PositiveIntegerField(_('Sıra No'))
    description = models.CharField(_('Açıklama'), max_length=255)
    
    budgeted_amount = models.DecimalField(_('Bütçe Tutarı'), max_digits=15, decimal_places=2)
    actual_amount = models.DecimalField(_('Gerçekleşen Tutar'), max_digits=15, decimal_places=2, default=0)
    variance_amount = models.DecimalField(_('Sapma Tutarı'), max_digits=15, decimal_places=2, default=0)
    variance_percentage = models.DecimalField(_('Sapma %'), max_digits=8, decimal_places=4, default=0)
    
    # Aylık detay (12 aylık dağılım için)
    january_budget = models.DecimalField(_('Ocak Bütçe'), max_digits=15, decimal_places=2, default=0)
    february_budget = models.DecimalField(_('Şubat Bütçe'), max_digits=15, decimal_places=2, default=0)
    march_budget = models.DecimalField(_('Mart Bütçe'), max_digits=15, decimal_places=2, default=0)
    april_budget = models.DecimalField(_('Nisan Bütçe'), max_digits=15, decimal_places=2, default=0)
    may_budget = models.DecimalField(_('Mayıs Bütçe'), max_digits=15, decimal_places=2, default=0)
    june_budget = models.DecimalField(_('Haziran Bütçe'), max_digits=15, decimal_places=2, default=0)
    july_budget = models.DecimalField(_('Temmuz Bütçe'), max_digits=15, decimal_places=2, default=0)
    august_budget = models.DecimalField(_('Ağustos Bütçe'), max_digits=15, decimal_places=2, default=0)
    september_budget = models.DecimalField(_('Eylül Bütçe'), max_digits=15, decimal_places=2, default=0)
    october_budget = models.DecimalField(_('Ekim Bütçe'), max_digits=15, decimal_places=2, default=0)
    november_budget = models.DecimalField(_('Kasım Bütçe'), max_digits=15, decimal_places=2, default=0)
    december_budget = models.DecimalField(_('Aralık Bütçe'), max_digits=15, decimal_places=2, default=0)
...
class CashFlowForecast(models.Model):
    ...
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name='cash_flow_forecasts')
    ...
    week_1_net_flow = models.DecimalField(_('1. Hafta Net Akış'), max_digits=15, decimal_places=2, default=0)
    week_2_net_flow = models.DecimalField(_('2. Hafta Net Akış'), max_digits=15, decimal_places=2, default=0)
    week_3_net_flow = models.DecimalField(_('3. Hafta Net Akış'), max_digits=15, decimal_places=2, default=0)
    week_4_net_flow = models.DecimalField(_('4. Hafta Net Akış'), max_digits=15, decimal_places=2, default=0)
    week_5_net_flow = models.DecimalField(_('5. Hafta Net Akış'), max_digits=15, decimal_places=2, default=0)
    week_6_net_flow = models.DecimalField(_('6. Hafta Net Akış'), max_digits=15, decimal_places=2, default=0)
    week_7_net_flow = models.DecimalField(_('7. Hafta Net Akış'), max_digits=15, decimal_places=2, default=0)
    week_8_net_flow = models.DecimalField(_('8. Hafta Net Akış'), max_digits=15, decimal_places=2, default=0)
    week_9_net_flow = models.DecimalField(_('9. Hafta Net Akış'), max_digits=15, decimal_places=2, default=0)
    week_10_net_flow = models.DecimalField(_('10. Hafta Net Akış'), max_digits=15, decimal_places=2, default=0)
    week_11_net_flow = models.DecimalField(_('11. Hafta Net Akış'), max_digits=15, decimal_places=2, default=0)
    week_12_net_flow = models.DecimalField(_('12. Hafta Net Akış'), max_digits=15, decimal_places=2, default=0)
    week_13_net_flow = models.DecimalField(_('13. Hafta Net Akış'), max_digits=15, decimal_places=2, default=0)
    
    opening_cash_balance = models.DecimalField(_('Açılış Nakit Bakiyesi'), max_digits=15, decimal_places=2)
    minimum_cash_required = models.DecimalField(_('Minimum Nakit Gereksinimi'), max_digits=15, decimal_places=2, default=0)
    
    # Analiz sonuçları
    lowest_cash_balance = models.DecimalField(_('En Düşük Nakit Bakiyesi'), max_digits=15, decimal_places=2, null=True)
    lowest_cash_week = models.PositiveIntegerField(_('En Düşük Bakiye Haftası'), null=True)
    cash_shortage_weeks = models.JSONField(_('Nakit Sıkıntısı Haftaları'), default=list, blank=True)
    
    # Öneriler ve uyarılar
    recommendations = models.JSONField(_('Öneriler'), default=list, blank=True)
    alerts = models.JSONField(_('Uyarılar'), default=list, blank=True)
    
    accuracy_score = models.DecimalField(_('Doğruluk Skoru %'), max_digits=5, decimal_places=2, 
                                       null=True, blank=True, help_text="Geçmiş tahminlerin doğruluk oranı")
    ...
    def calculate_weekly_balances(self):
        ...
        weekly_flows = [
            self.week_1_net_flow, self.week_2_net_flow, self.week_3_net_flow, self.week_4_net_flow,
            self.week_5_net_flow, self.week_6_net_flow, self.week_7_net_flow, self.week_8_net_flow,
            self.week_9_net_flow, self.week_10_net_flow, self.week_11_net_flow, self.week_12_net_flow,
    ...
    def _generate_cash_flow_recommendations(self):
        ...
        if self.lowest_cash_balance and self.lowest_cash_balance < (self.minimum_cash_required * Decimal('1.5')):
            recommendations.append({
                'type': 'low_cash',
                'priority': 'high',
    ...
    def _generate_cash_flow_alerts(self):
        ...
        average_flow = sum(weekly_flows) / len(weekly_flows)
        for week, flow in enumerate(weekly_flows, 1):
            if flow < (average_flow * Decimal('2')) and flow < 0:  # %200'den fazla negatif sapma
                alerts.append({
                    'type': 'large_outflow',
                    'severity': 'high',
...
class ProfitabilityAnalysis(models.Model):
    ...
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name='profitability_analyses')
    ...
    # Genel totaller
    total_revenue = models.DecimalField(_('Toplam Gelir'), max_digits=15, decimal_places=2, default=0)
    total_direct_costs = models.DecimalField(_('Toplam Direkt Maliyetler'), max_digits=15, decimal_places=2, default=0)
    total_indirect_costs = models.DecimalField(_('Toplam Endirekt Maliyetler'), max_digits=15, decimal_places=2, default=0)
    gross_profit = models.DecimalField(_('Brüt Kar'), max_digits=15, decimal_places=2, default=0)
    net_profit = models.DecimalField(_('Net Kar'), max_digits=15, decimal_places=2, default=0)
    
    # Karlılık oranları
    gross_margin_percentage = models.DecimalField(_('Brüt Kar Marjı %'), max_digits=8, decimal_places=4, default=0)
    net_margin_percentage = models.DecimalField(_('Net Kar Marjı %'), max_digits=8, decimal_places=4, default=0)
    
    # En karlı ve en az karlı kalemler
    most_profitable_item = models.JSONField(_('En Karlı Kalem'), default=dict, blank=True)
    least_profitable_item = models.JSONField(_('En Az Karlı Kalem'), default=dict, blank=True)
    
    # Öneriler
    optimization_suggestions = models.JSONField(_('Optimizasyon Önerileri'), default=list, blank=True)
    ...
    def calculate_totals(self):
        ...
        self.total_revenue = lines.aggregate(total=Sum('revenue'))['total'] or Decimal('0')
        self.total_direct_costs = lines.aggregate(total=Sum('direct_costs'))['total'] or Decimal('0')
        self.total_indirect_costs = lines.aggregate(total=Sum('indirect_costs'))['total'] or Decimal('0')
        ...
        if self.total_revenue > 0:
            self.gross_margin_percentage = (self.gross_profit / self.total_revenue) * 100
...
class ProfitabilityLine(models.Model):
    ...
    analysis = models.ForeignKey(ProfitabilityAnalysis, on_delete=models.CASCADE, 
                               related_name='profitability_lines', verbose_name=_('Analiz'))
    item_code = models.CharField(_('Kalem Kodu'), max_length=50)
    item_name = models.CharField(_('Kalem Adı'), max_length=200)
    
    quantity_sold = models.DecimalField(_('Satılan Miktar'), max_digits=15, decimal_places=3, default=0)
    revenue = models.DecimalField(_('Gelir'), max_digits=15, decimal_places=2)
    direct_costs = models.DecimalField(_('Direkt Maliyetler'), max_digits=15, decimal_places=2)
    indirect_costs = models.DecimalField(_('Endirekt Maliyetler'), max_digits=15, decimal_places=2, default=0)
    ...
    customer_segment = models.CharField(_('Müşteri Segmenti'), max_length=50, blank=True)
    ...
    def calculate_margins(self):
        ...
        self.save(update_fields=[
            'gross_profit', 'net_profit', 'gross_margin_percentage', 'net_margin_percentage'
        ])