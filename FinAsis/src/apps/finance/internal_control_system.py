...
class AuditTrail(models.Model):
    ...
    # Temel bilgiler
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name='audit_trails')
    user = models.ForeignKey(dj_settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True)
    ...
    risk_level = models.CharField(_('Risk Seviyesi'), max_length=20, choices=RISK_LEVELS, default='LOW')
    ...
    def __str__(self):
        return f"{self.user} - {self.action_type} - {self.table_name} ({self.timestamp})"
    
    @classmethod
    def log_action(cls, user, company, action_type, obj=None, old_values=None, new_values=None, 
                   description='', ip_address=None, user_agent='', **kwargs):
        """İşlem kaydı oluştur"""
        ...
        audit_entry.risk_level = cls._determine_risk_level(action_type, audit_entry.table_name)
    ...
    def _determine_risk_level(action_type, table_name):
        """Risk seviyesini belirle"""
        # Kritik işlemler
        if action_type in ['DELETE', 'UNPOST'] or table_name in ['journal_voucher', 'payment', 'invoice']:
            return 'CRITICAL'
...
class UserPermission(models.Model):
    ...
    user = models.ForeignKey(dj_settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='custom_permissions')
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name='user_permissions')
    
    # İzin detayları
    module_name = models.CharField(_('Modül Adı'), max_length=100,
                                  help_text="accounting, finance, inventory vb.")
    permission_type = models.CharField(_('İzin Tipi'), max_length=20, choices=PERMISSION_TYPES)
    
    # Kısıtlamalar
    amount_limit = models.DecimalField(_('Tutar Limiti'), max_digits=15, decimal_places=2, 
                                     null=True, blank=True, help_text="Bu tutarın üstündeki işlemlerde izin yok")
    date_restriction = models.CharField(_('Tarih Kısıtı'), max_length=50, blank=True,
    ...
    # Onay bilgileri
    granted_by = models.ForeignKey(dj_settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, 
                                  null=True, related_name='granted_permissions', verbose_name=_('İzin Veren'))
...
class ApprovalWorkflow(models.Model):
    ...
    workflow_type = models.CharField(_('İş Akışı Tipi'), max_length=30, choices=WORKFLOW_TYPES)
    
    # Tetikleme koşulları
    min_amount = models.DecimalField(_('Minimum Tutar'), max_digits=15, decimal_places=2, 
                                   null=True, blank=True)
    max_amount = models.DecimalField(_('Maksimum Tutar'), max_digits=15, decimal_places=2, 
                                   null=True, blank=True)
    applicable_modules = models.JSONField(_('Geçerli Modüller'), default=list, blank=True)
    
    # İş akışı ayarları
    is_sequential = models.BooleanField(_('Sıralı Onay'), default=True,
                                      help_text="True: Sırayla onay, False: Paralel onay")
    require_all_approvers = models.BooleanField(_('Tüm Onaylayanlar Gerekli'), default=True)
...
class ApprovalWorkflowStep(models.Model):
    ...
    workflow = models.ForeignKey(ApprovalWorkflow, on_delete=models.CASCADE, 
                               related_name='workflow_steps', verbose_name=_('İş Akışı'))
    ...
    approver_user = models.ForeignKey(dj_settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, 
                                     null=True, blank=True, verbose_name=_('Onaylayıcı Kullanıcı'))
    approver_role = models.CharField(_('Onaylayıcı Rolü'), max_length=100, blank=True,
    ...
    auto_approve_after_timeout = models.BooleanField(_('Zaman Aşımında Otomatik Onay'), default=False)
    
    # Koşullar
    condition_expression = models.TextField(_('Koşul İfadesi'), blank=True,
...
class ApprovalRequest(models.Model):
    ...
    workflow = models.ForeignKey(ApprovalWorkflow, on_delete=models.PROTECT, related_name='approval_requests')
    ...
    requested_by = models.ForeignKey(dj_settings.AUTH_USER_MODEL, on_delete=models.PROTECT, 
                                   related_name='approval_requests', verbose_name=_('Talep Eden'))
    ...
    amount = models.DecimalField(_('Tutar'), max_digits=15, decimal_places=2, null=True, blank=True)
    ...
    def process_next_step(self):
        ...
        for approver in next_step.get_approver_users():
            ApprovalRecord.objects.create(
                ...
                approver=approver,
                due_date=timezone.now() + timezone.timedelta(hours=next_step.timeout_hours)
            )
    ...
    def approve(self, user, comments=''):
        ...
        approval_record = self.approval_records.filter(
            workflow_step__step_order=self.current_step,
            approver=user,
            status='PENDING'
        ).first()
    ...
    def reject(self, user, comments=''):
        ...
        if approval_record:
            ...
            self.completed_at = timezone.now()
            self.final_decision = f"Reddeden: {user.get_full_name()}\nGerekçe: {comments}"
            self.save()
...
class ApprovalRecord(models.Model):
    ...
    request = models.ForeignKey(ApprovalRequest, on_delete=models.CASCADE, 
                               related_name='approval_records', verbose_name=_('Onay Talebi'))
    workflow_step = models.ForeignKey(ApprovalWorkflowStep, on_delete=models.PROTECT,
                                     verbose_name=_('İş Akışı Adımı'))
    approver = models.ForeignKey(dj_settings.AUTH_USER_MODEL, on_delete=models.PROTECT,
...
class RiskAssessment(models.Model):
    ...
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name='risk_assessments')
    assessment_date = models.DateField(_('Değerlendirme Tarihi'))
    assessor = models.ForeignKey(dj_settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, 
                                null=True, verbose_name=_('Değerlendiren'))
    
    # Risk tanımı
    risk_title = models.CharField(_('Risk Başlığı'), max_length=200)
    risk_description = models.TextField(_('Risk Açıklaması'))
    risk_category = models.CharField(_('Risk Kategorisi'), max_length=20, choices=RISK_CATEGORIES)
    
    # Risk seviyesi değerlendirmesi
    likelihood = models.CharField(_('Olasılık'), max_length=20, choices=RISK_LEVELS,
                                 help_text="Riskin gerçekleşme olasılığı")
    impact = models.CharField(_('Etki'), max_length=20, choices=RISK_LEVELS,
    ...
    overall_risk_level = models.CharField(_('Genel Risk Seviyesi'), max_length=20, 
                                        choices=RISK_LEVELS, null=True, blank=True)
    
    # Mali etki tahmini
    potential_financial_impact = models.DecimalField(_('Potansiyel Mali Etki'), max_digits=15, 
                                                   decimal_places=2, null=True, blank=True)
    
    # Mevcut kontroller
    existing_controls = models.TextField(_('Mevcut Kontroller'), blank=True)
    control_effectiveness = models.CharField(_('Kontrol Etkinliği'), max_length=20, 
                                          choices=RISK_LEVELS, default='MEDIUM')
    
    # Önerilen aksiyonlar
    recommended_actions = models.TextField(_('Önerilen Aksiyonlar'), blank=True)
    action_priority = models.CharField(_('Aksiyon Önceliği'), max_length=20, 
                                     choices=RISK_LEVELS, default='MEDIUM')
    target_completion_date = models.DateField(_('Hedef Tamamlanma Tarihi'), null=True, blank=True)
    ...
    # Takip bilgileri
    next_review_date = models.DateField(_('Sonraki İnceleme Tarihi'), null=True, blank=True)
    responsible_person = models.ForeignKey(dj_settings.AUTH_USER_MODEL, on_delete=models.SET_NULL,
...
class ControlActivity(models.Model):
    ...
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name='control_activities')
    risk_assessment = models.ForeignKey(RiskAssessment, on_delete=models.SET_NULL, null=True, blank=True,
                                       related_name='control_activities', verbose_name=_('İlgili Risk'))
    ...
    # Kontrol özellikleri
    control_type = models.CharField(_('Kontrol Tipi'), max_length=20, choices=CONTROL_TYPES)
    control_nature = models.CharField(_('Kontrol Doğası'), max_length=20, choices=CONTROL_NATURE)
    frequency = models.CharField(_('Sıklık'), max_length=20, choices=FREQUENCY_CHOICES)
    
    # Sorumluluk
    control_owner = models.ForeignKey(dj_settings.AUTH_USER_MODEL, on_delete=models.SET_NULL,
                                     null=True, related_name='owned_controls', verbose_name=_('Kontrol Sahibi'))
    control_performer = models.ForeignKey(dj_settings.AUTH_USER_MODEL, on_delete=models.SET_NULL,
                                        null=True, blank=True, related_name='performed_controls',
    ...
    design_effectiveness = models.CharField(_('Tasarım Etkinliği'), max_length=20, choices=[
        ('EFFECTIVE', _('Etkili')),
        ('INEFFECTIVE', _('Etkisiz')),
        ('NOT_EVALUATED', _('Değerlendirilmedi')),
    ], default='NOT_EVALUATED')
    
    operating_effectiveness = models.CharField(_('İşleyiş Etkinliği'), max_length=20, choices=[
        ('EFFECTIVE', _('Etkili')),
        ('INEFFECTIVE', _('Etkisiz')),
        ('NOT_EVALUATED', _('Değerlendirilmedi')),
    ], default='NOT_EVALUATED')
    ...
    # Durum ve takip
    is_active = models.BooleanField(_('Aktif'), default=True)
    last_performed_date = models.DateField(_('Son Uygulama Tarihi'), null=True, blank=True)
    next_due_date = models.DateField(_('Sonraki Vade Tarihi'), null=True, blank=True)
    
    # İyileştirme önerileri
    improvement_recommendations = models.TextField(_('İyileştirme Önerileri'), blank=True)
    ...
    def calculate_next_due_date(self):
        ...
        self.next_due_date = self.last_performed_date + timezone.timedelta(days=days_to_add)
...
class ControlExecution(models.Model):
    ...
    control_activity = models.ForeignKey(ControlActivity, on_delete=models.CASCADE,
                                       related_name='executions', verbose_name=_('Kontrol Faaliyeti'))
    execution_date = models.DateField(_('Uygulama Tarihi'))
    performed_by = models.ForeignKey(dj_settings.AUTH_USER_MODEL, on_delete=models.PROTECT,
...
class ComplianceReportGenerator:
    ...
    def generate_audit_summary(self):
        ...
        summary = {
            ...
            'high_risk_actions': audit_trails.filter(risk_level__in=['HIGH', 'CRITICAL']).count()
        }
    ...
    def generate_approval_summary(self):
        ...
        summary = {
            ...
            'pending': approval_requests.filter(status__in=['PENDING', 'IN_PROGRESS']).count(),
            'average_approval_time': self._calculate_average_approval_time(approval_requests),
    ...
    def _calculate_average_approval_time(self, approval_requests):
        ...
        completed_requests = approval_requests.filter(
