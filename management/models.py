...
class SystemHealth(models.Model):
    ...
    # Sistem metrikleri
    cpu_usage = models.DecimalField(max_digits=5, decimal_places=2, verbose_name="CPU Kullanımı (%)")
    memory_usage = models.DecimalField(max_digits=5, decimal_places=2, verbose_name="RAM Kullanımı (%)")
    disk_usage = models.DecimalField(max_digits=5, decimal_places=2, verbose_name="Disk Kullanımı (%)")
    ...
    # Durum
    overall_status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='HEALTHY', verbose_name="Genel Durum")
...
class PerformanceMetric(models.Model):
    ...
    metric_type = models.CharField(max_length=30, choices=METRIC_TYPES, verbose_name="Metrik Tipi")
    ...
    unit = models.CharField(max_length=20, verbose_name="Birim", help_text="ms, %, requests/sec, vs.")
    
    # Hedef ve durum
    target_value = models.DecimalField(max_digits=15, decimal_places=2, null=True, blank=True, verbose_name="Hedef Değer")
...
class ErrorLog(models.Model):
    ...
    severity = models.CharField(max_length=20, choices=SEVERITY_LEVELS, default='ERROR', verbose_name="Ciddiyet")
    ...
    line_number = models.IntegerField(null=True, blank=True, verbose_name="Satır Numarası")
    
    # Request bilgisi
    request_path = models.CharField(max_length=500, blank=True, verbose_name="İstek Yolu")
    request_method = models.CharField(max_length=10, blank=True, verbose_name="HTTP Metodu")
    ...
    resolved_by = models.ForeignKey(settings.AUTH_USER_MODEL, null=True, blank=True, on_delete=models.SET_NULL, related_name='resolved_errors', verbose_name="Çözen")
...
class BackupLog(models.Model):
    ...
    backup_type = models.CharField(max_length=20, choices=BACKUP_TYPES, verbose_name="Yedekleme Tipi")
    
    # Dosya bilgisi
    file_path = models.CharField(max_length=500, verbose_name="Dosya Yolu")
    file_size_mb = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, verbose_name="Dosya Boyutu (MB)")
    ...
    duration_seconds = models.IntegerField(null=True, blank=True, verbose_name="Süre (saniye)")
    ...
    # İstatistikler
    items_backed_up = models.IntegerField(null=True, blank=True, verbose_name="Yedeklenen Öğe Sayısı")
    
    # Metadata
    initiated_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, verbose_name="Başlatan")
...
class SystemAudit(models.Model):
    ...
    audit_criteria = models.JSONField(default=dict, blank=True, verbose_name="Denetim Kriterleri")
    ...
    # Skorlama
    compliance_score = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True, verbose_name="Uyumluluk Skoru (%)")
    
    # Öneriler
    recommendations = models.TextField(blank=True, verbose_name="Öneriler")
    action_items = models.JSONField(default=list, blank=True, verbose_name="Aksiyon Maddeleri")
    
    # Zamanlama
    audit_date = models.DateField(verbose_name="Denetim Tarihi")
    next_audit_date = models.DateField(null=True, blank=True, verbose_name="Sonraki Denetim")
    
    # Metadata
    audited_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, verbose_name="Denetleyen")
    created_at = models.DateTimeField(auto_now_add=True)
...
class MaintenanceWindow(models.Model):
    ...
    # Etkilenen servisler
    affected_modules = models.JSONField(default=list, blank=True, verbose_name="Etkilenen Modüller")
    ...
    # Metadata
    scheduled_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='scheduled_maintenances', verbose_name="Planlayan")
    performed_by = models.ForeignKey(settings.AUTH_USER_MODEL, null=True, blank=True, on_delete=models.SET_NULL, related_name='performed_maintenances', verbose_name="Gerçekleştiren")
...
class UsageStatistics(models.Model):
    ...
    period_type = models.CharField(max_length=20, choices=[
        ...
        ('WEEKLY', 'Haftalık'),
        ('MONTHLY', 'Aylık'),
    ], default='DAILY', verbose_name="Dönem Tipi")
    ...
    average_session_duration = models.IntegerField(default=0, verbose_name="Ortalama Oturum Süresi (dk)")
    ...
    # Modül kullanımı
    module_usage = models.JSONField(default=dict, blank=True, verbose_name="Modül Kullanımı", help_text="{'accounting': 150, 'finance': 200, ...}")
    
    # Feature kullanımı
    feature_usage = models.JSONField(default=dict, blank=True, verbose_name="Özellik Kullanımı")
...
class DatabaseSnapshot(models.Model):
    ...
    # Database boyutları
    total_size_mb = models.DecimalField(max_digits=15, decimal_places=2, verbose_name="Toplam Boyut (MB)")
    data_size_mb = models.DecimalField(max_digits=15, decimal_places=2, verbose_name="Veri Boyutu (MB)")
    index_size_mb = models.DecimalField(max_digits=15, decimal_places=2, verbose_name="İndeks Boyutu (MB)")
    ...
    model_statistics = models.JSONField(default=dict, verbose_name="Model İstatistikleri", help_text="{'User': 1500, 'Invoice': 35000, ...}")
    
    # Performans
    avg_query_time_ms = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, verbose_name="Ortalama Sorgu Süresi (ms)")
    slow_queries_count = models.IntegerField(default=0, verbose_name="Yavaş Sorgu Sayısı")
    
    # Fragmentation
    fragmentation_percentage = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True, verbose_name="Fragmentation (%)")
...
class FeatureFlag(models.Model):
    ...
    enabled_for_users = models.ManyToManyField(settings.AUTH_USER_MODEL, blank=True, related_name='enabled_features', verbose_name="Aktif Kullanıcılar")
    enabled_for_roles = models.JSONField(default=list, blank=True, verbose_name="Aktif Roller")
    
    # Percentage rollout
    rollout_percentage = models.IntegerField(default=0, verbose_name="Kullanıma Sunma Yüzdesi", help_text="0-100")
    ...
    def is_enabled_for_user(self, user):
        ...
        if self.rollout_percentage > 0:
            ...
            import hashlib
            hash_value = int(hashlib.md5(f"{user.id}{self.key}".encode()).hexdigest(), 16)
