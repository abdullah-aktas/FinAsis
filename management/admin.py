...
from .models import (
    SystemHealth, PerformanceMetric, ErrorLog, BackupLog,
    SystemAudit, MaintenanceWindow, UsageStatistics, DatabaseSnapshot,
    FeatureFlag
)

@admin.register(SystemHealth)
class SystemHealthAdmin(admin.ModelAdmin):
    list_display = ('checked_at', 'overall_status', 'cpu_usage', 'memory_usage', 'disk_usage', 'active_users', 'error_count')
...
class PerformanceMetricAdmin(admin.ModelAdmin):
    list_display = ('metric_name', 'metric_type', 'value', 'unit', 'module', 'is_healthy', 'recorded_at')
...
class ErrorLogAdmin(admin.ModelAdmin):
    list_display = ('severity', 'error_type', 'module', 'user', 'is_resolved', 'occurred_at')
...
class BackupLogAdmin(admin.ModelAdmin):
    list_display = ('backup_name', 'backup_type', 'status', 'file_size_mb', 'duration_seconds', 'started_at', 'completed_at')
...
class SystemAuditAdmin(admin.ModelAdmin):
    list_display = ('audit_name', 'audit_type', 'status', 'compliance_score', 'issues_found', 'audit_date', 'audited_by')
...
class MaintenanceWindowAdmin(admin.ModelAdmin):
    list_display = ('title', 'status', 'start_time', 'end_time', 'estimated_duration_minutes', 'notification_sent')
...
class UsageStatisticsAdmin(admin.ModelAdmin):
    list_display = ('period_date', 'period_type', 'total_users', 'active_users', 'new_users', 'total_sessions', 'api_calls')
...
class DatabaseSnapshotAdmin(admin.ModelAdmin):
    list_display = ('snapshot_date', 'total_size_mb', 'table_count', 'total_rows', 'avg_query_time_ms', 'optimization_needed')
...
class FeatureFlagAdmin(admin.ModelAdmin):
    list_display = ('name', 'key', 'module', 'is_enabled', 'enabled_for_all', 'rollout_percentage', 'created_by')
