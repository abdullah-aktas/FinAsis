...
def admin_dashboard(request):
    ...
    # Son kayıtlar
    latest_users = CustomUser.objects.select_related("company").order_by(
        "-date_joined"
    )[:5]
    latest_companies = (
        Company.objects.order_by("-created_at")[:5]
        if hasattr(Company, "created_at")
        else []
    )
    latest_invoices = (
        Invoice.objects.select_related("company", "customer").order_by("-issue_date")[
            :8
        ]
    ...
    for i in range(6, -1, -1):
        ...
        user_count_on_date = CustomUser.objects.filter(
            date_joined__date__lte=date
        ).count()
        user_growth.append(user_count_on_date)

        # O gün oluşturulan fatura sayısı
        invoice_count_on_date = (
            Invoice.objects.filter(issue_date=date).count()
            if hasattr(Invoice, "issue_date")
            else 0
        )
    ...
    accounting_shortcuts = [
        ...
        {
            "name": "Giderler",
            "url": reverse("accounting:expense_list"),
            "icon": "bi-receipt",
        },
        {
            "name": "Müşteriler",
            "url": reverse("accounting:customer_list"),
...
def user_list(request):
    ...
    if request.method == "POST":
        ...
        if ids:
            deleted_users = list(
                CustomUser.objects.filter(id__in=ids).values_list("username", flat=True)
            )
    ...
    return render(
        request, "management/user_list.html", {"page_obj": page_obj, "filter": f}
    )
...
def user_list_export_csv(request):
    ...
    for user in f.qs:
        yetki = (
            ...
            else ("Yönetici" if user.is_staff else "Kullanıcı")
        )
        writer.writerow(
            [
                user.id,
...
def company_list(request):
    ...
    if query:
        companies = companies.filter(name__icontains=query) | companies.filter(
            tax_number__icontains=query
        )
    if sector:
...
def company_edit(request, company_id):
    ...
    return render(
        request,
        "management/company_form.html",
        {"form": form, "edit": True, "company": company},
    )
...
def company_delete(request, company_id):
    ...
    if request.method == "POST":
        ...
        company.delete()
        ActionLog.objects.create(
            user=request.user, action="Şirket Silme", detail=f"Silinen şirket: {name}"
        )
        messages.success(request, "Şirket silindi.")
        return redirect("company_list")
    return render(
        request, "management/company_confirm_delete.html", {"company": company}
    )
...
def invoice_list(request):
    ...
    if query:
        invoices = invoices.filter(description__icontains=query) | invoices.filter(
            total_amount__icontains=query
        )
    if company_name:
    ...
    return render(
        request,
        "management/invoice_list.html",
        {"page_obj": page_obj, "query": query, "company_name": company_name},
    )
...
def user_detail(request, user_id):
    ...
    user_invoices = (
        Invoice.objects.filter(company__owner=user)
        if hasattr(Invoice, "company") and hasattr(Company, "owner")
        else []
    )
    return render(
        request,
        "management/user_detail.html",
        {"user": user, "user_invoices": user_invoices},
    )
