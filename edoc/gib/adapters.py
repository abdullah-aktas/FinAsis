...
class StubGibAdapter:
    ...
    def _send_kind(
        self, kind: str, idempotency_key: Optional[str] = None
    ) -> SendResult:
        ...
        if p.exists():
            ...
            return SendResult(
                tracking_id=tracking_id, status=data["status"]
            )  # idempotent
...
class HttpGibAdapter:
    ...
    def _send(
        ...
        try:
            r = self.session.post(
                ep.send_url(), data=xml_bytes, headers=headers, timeout=15
            )
            if r.status_code in (200, 202):
                ...
                tracking = (
                    payload.get("tracking_id") or idempotency_key or str(uuid.uuid4())
                )
                status: Status = payload.get("status") or (
                    "PENDING" if r.status_code == 202 else "ACCEPTED"
                )
    ...
    def poll(self, tracking_id: str) -> Status:
        ...
        try:
            r = self.session.get(
                ep.status_url(), params={"tracking_id": tracking_id}, timeout=10
            )
